...
export const chapters: GuideChapter[] = [
  ...
  },
  {
    ...
    prerequisites: ['Fiches techniques et Production installées.'],
    steps: [
  ...
  },
  {
    ...
    prerequisites: ['RH recommandé : collaborateurs, services et postes renseignés.'],
    steps: [
  ...
  },
  {
    ...
    steps: [
      'Semaine 1 : préparez les utilisateurs, rôles et référentiels produits.',
...
export const screensByChapter: Record<string, GuideScreen[]> = {
  demarrer: [
    ...
    { title: 'Premier démarrage', purpose: 'Guider la mise en place progressive de l’établissement.', actions: 'Suivre les étapes proposées avant d’activer les parcours avancés.' },
  ],
  referentiels: [
...
export const operationalFlows: Record<string, OperationalFlow> = {
  demarrer: { when: 'Lors de la première prise en main ou de l’arrivée d’un nouvel utilisateur.', prerequisites: ['Un compte individuel actif.', 'L’adresse de l’instance locale.'], steps: ['Se connecter avec son compte personnel.', 'Vérifier l’établissement et les modules disponibles.', 'Lire les alertes et ouvrir le tableau de bord général.', 'Commencer par les référentiels avant toute saisie métier.', 'Demander le bon rôle si une action est absente.'], checks: ['Le nom de l’établissement est correct.', 'Aucun compte partagé n’est utilisé.'], result: 'L’utilisateur accède uniquement aux outils utiles à sa mission.', pitfalls: ['Créer des données métier avec un compte administrateur partagé.'] },
  ...
  parcours: { when: 'Lors de l’adoption progressive de ToqueHub dans un établissement.', prerequisites: ['Référent métier nommé pour chaque domaine.', 'Temps de formation et données de départ disponibles.'], steps: ['Installer et sécuriser les accès.', 'Construire les référentiels.', 'Démarrer les Stocks avec un périmètre pilote.', 'Créer les fiches techniques essentielles.', 'Passer à Production puis Menus.', 'Déployer les routines HACCP.', 'Ajouter RH, Planning, mobile et IoT selon la maturité.', 'Mesurer les écarts et améliorer les procédures.'], checks: ['Chaque étape possède un responsable.', 'Les données de l’étape précédente sont validées avant la suivante.'], result: 'Le déploiement reste progressif et les modules reposent sur des données fiables.', pitfalls: ['Activer tous les modules sans référentiels ni formation.', 'Recréer les mêmes données dans plusieurs modules.'] },
};
