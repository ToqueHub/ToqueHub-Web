...
function menuPortions(menu: MenuPlan, item: NonNullable<MenuPlan['items']>[number]) {
  ...
  return menu.activity === 'RESTAURANT_CAFE'
    ? guests
    : guests * Number(item.servingQuantity ?? 1);
}
  ...
  const operationMinutes = (campaign.batches ?? []).reduce(
    (total, batch) =>
      ...
      (batch.operations ?? []).reduce(
        (batchTotal, operation) =>
          batchTotal + Math.max(Number(operation.activeMinutes ?? 0), 0),
        0,
      ),
    0,
  );
...
function campaignShortages(campaign: ProductionCampaign) {
  return (campaign.requirements ?? []).filter((requirement) =>
    ['INSUFFICIENT_STOCK', 'UNIT_NOT_CONVERTIBLE', 'PRODUCT_ARCHIVED'].includes(
      requirement.status,
    ),
  );
}

function campaignMenuQuantities(campaign: ProductionCampaign) {
  const lines = (campaign.menuProductionLinks ?? []).flatMap(
    (link) => link.snapshot?.lines ?? [],
  );
  return {
    targetAvailable: lines.reduce(
      (sum, line) => sum + Number(line.targetPortions ?? line.portions ?? 0),
      0,
    ),
    openingCarryOver: lines.reduce(
      (sum, line) => sum + Number(line.openingCarryOverPortions ?? 0),
      0,
    ),
  };
}
...
export function ProductionFabricationCalendar({
  ...
  const [catalogCarryOver, setCatalogCarryOver] = useState<ProductionCarryOver | null>(null);
  const [assignmentCampaign, setAssignmentCampaign] = useState<ProductionCampaign | null>(
    null,
  );
  ...
  const [validationCampaign, setValidationCampaign] =
    useState<ProductionCampaign | null>(null);
  ...
  const visibleCampaigns = useMemo(
    () =>
      campaigns.filter(
        (campaign) =>
          campaign.status !== 'CANCELLED' && (!siteId || campaign.siteId === siteId),
      ),
    [campaigns, siteId],
  );
  ...
  const selectedCatalog =
    catalogs.find((catalog) => catalog.id === catalogDraft.catalogId) ?? null;
  ...
  const carryOverByProduct = useMemo(
    () =>
      new Map(
        (catalogCarryOver?.items ?? []).map((item) => [
    ...
    return Object.fromEntries(
      (catalog?.items ?? [])
        ...
        .map((item) => {
          ...
          const referencePortions = Number(
            item.technicalSheet?.referencePortions ?? 0,
          );
          const recipeReady =
            Number.isFinite(referencePortions) && referencePortions > 0;
          return [
            item.id!,
            {
  ...
  function openCreate(date: string) {
    ...
    const firstProfile = availableProfiles.find((profile) => profile.siteId === selectedSiteId);
    const kitchen =
      departments.find((department) => /cuisine|pâtisserie|patisserie|production/i.test(department.name)) ??
      departments[0];
    setDraft({
  ...
  function selectCatalogSite(selectedSiteId: string) {
    const catalog = catalogs.find(
      (candidate) => candidate.id === catalogDraft.catalogId,
    );
    setCatalogDraft((current) => ({
      ...current,
      siteId: selectedSiteId,
  ...
  useEffect(() => {
    if (!catalogPlannerDate || !catalogDraft.siteId) {
      setCatalogCarryOver(null);
      return;
    }
    let active = true;
    api
      .productionDayCarryOver(token, {
        siteId: catalogDraft.siteId,
        date: catalogPlannerDate,
      })
      .then((result) => {
        if (active) setCatalogCarryOver(result);
      })
      .catch(() => {
        if (active) setCatalogCarryOver(null);
      });
    return () => {
      active = false;
    };
  }, [catalogDraft.siteId, catalogPlannerDate, token]);
  ...
  async function planCatalogProducts(event: FormEvent) {
    ...
    if (
      ...
      lines.some(
        (line) =>
          !Number.isFinite(line.targetPortions) || line.targetPortions <= 0,
      )
    ) {
      setError('Sélectionnez au moins un produit avec un objectif supérieur à zéro.');
      return;
    }
  ...
  async function planMenuProducts(event: FormEvent) {
    ...
    if (!lines.length || lines.some((line) => !Number.isFinite(line.portions) || line.portions <= 0)) {
  ...
  async function createCampaign(event: FormEvent) {
    event.preventDefault();
    if (!draft.siteId || !draft.profileId || !draft.serviceId || Number(draft.target) <= 0) {
      setError('Choisissez le site, le produit, le service et un objectif supérieur à zéro.');
      return;
    }
    setCreateBusy(true);
    setError('');
    try {
      await api.createProductionCampaign(token, {
        ...
        grossRequirement: String(Number(draft.target)),
  ...
  function openCampaignEditor(campaign: ProductionCampaign) {
    const { targetAvailable } = campaignMenuQuantities(campaign);
    const requestedPortions = Number(
      campaign.grossRequirement ?? campaign.plannedPortions,
    );
    ...
    setEditDraft({
      ...
      targetPortions: String(
        Math.max(targetAvailable || requestedPortions || 1, 1),
      ),
    });
    setEditError('');
  }
  ...
  async function saveCampaignChanges(event: FormEvent) {
    ...
    const { targetAvailable, openingCarryOver } =
      campaignMenuQuantities(editingCampaign);
    const grossRequirement =
      targetAvailable > 0
        ? targetPortions - openingCarryOver
        : targetPortions;
    ...
    if (
      ...
      setEditError(
        targetAvailable > 0
          ? 'L’objectif doit être supérieur au report de la veille.'
          : 'L’objectif doit être supérieur à zéro.',
      );
      return;
    }
    ...
    try {
      await api.updateProductionCampaign(token, editingCampaign.id, {
        ...
        targetPortions:
          targetAvailable > 0 ? String(targetPortions) : undefined,
      });
  ...
  async function validateCampaign(
    ...
    try {
      ...
      if (
        !allowShortage &&
        /stock|ingrédient|composant|rupture|insuffisant/i.test(message)
      ) {
  ...
  async function openAssignment(campaign: ProductionCampaign) {
    if (!campaign.serviceId) {
      setError(
        'Choisissez d’abord le service responsable de cette fabrication.',
      );
      return;
    }
    ...
    const endsAt = new Date(
      startsAt.getTime() + campaignDurationMinutes(campaign) * 60_000,
    );
    ...
    try {
      for (const assignment of existingAssignments) {
        if (!selected.has(assignment.employeeId)) {
          await api.removeProductionAssignment(
            token,
            assignmentCampaign.id,
            assignment.id,
          );
        }
      }
  ...
  return (
    <>
      <section className="fabrication-calendar-panel">
        <header className="fabrication-calendar-toolbar">
          <div>
            ...
            <h2>Production à réaliser depuis le menu</h2>
            <p>
              Choisissez les produits et quantités du menu. Une fois validées, leurs étapes
              alimentent le planning opérationnel des collaborateurs.
            </p>
          </div>
          <div className="fabrication-calendar-actions">
            ...
            <button type="button" className="production-btn-glass" onClick={() => openCreate(anchor)}>
              <Plus size={17} /> Production hors menu
            </button>
            {mode === 'day' && (
              <button
                type="button"
              ...
              >
                {closureLoading ? <Loader2 size={17} className="spin" /> : <CheckCircle2 size={17} />}
                Clôturer la journée
              </button>
            )}
          </div>
        ...
        <div className="fabrication-calendar-navigation">
          ...
          <div className="fabrication-calendar-period">
            <button type="button" onClick={() => navigate(-1)}><ChevronLeft size={18} /></button>
            <button type="button" onClick={() => setAnchor(dayKey(new Date()))}>Aujourd’hui</button>
            <strong>{periodLabel}</strong>
            <button type="button" onClick={() => navigate(1)}><ChevronRight size={18} /></button>
          </div>
        </div>

        {error && (
          <div className="fabrication-calendar-error"><AlertCircle size={17} /> {error}</div>
        )}
        ...
        <div className={`fabrication-calendar-grid mode-${mode}`}>
          {calendarDays.map((day) => {
            ...
            const otherMonth = mode === 'month' && parseDay(day).getMonth() !== parseDay(anchor).getMonth();
            return (
              <article
                key={day}
              ...
              >
                <div className="fabrication-calendar-day-head">
                  <button type="button" onClick={() => { setAnchor(day); setMode('day'); }}>
                    {formatDay(day, mode !== 'day')}
                  </button>
                  <button type="button" aria-label="Planifier depuis la carte" onClick={() => openCatalogPlanner(day)}>
                    <Plus size={15} />
                  </button>
                </div>
                <div className="fabrication-calendar-events">
                    ...
                    if (mode === 'month') {
                      return (
                        ...
                        >
                          <BookOpen size={12} />
                          <span>{menu.name}</span>
                          <strong>{linked.size}/{technicalItems.length}</strong>
                        </button>
                      );
                    }
                    return (
                      <div key={menu.id} className="fabrication-menu-card">
                        <div className="fabrication-menu-card-head">
                          <span><BookOpen size={13} /> {menuServiceLabel(menu.service)}</span>
                              ...
                              <div key={item.id ?? `${item.section}-${menuItemName(item)}`}>
                                ...
                                {item.technicalSheetId ? (
                                  <small className={planned ? 'planned' : ''}>
                                    {planned ? <><Check size={11} /> Planifié</> : `${menuPortions(menu, item)} port.`}
                                  </small>
                                ) : (
                            ...
                            : !['VALIDATED', 'PUBLISHED'].includes(menu.status)
                              ...
                              : !menu.siteId
                                ...
                                : !technicalItems.length
                                  ...
                                  : <><Plus size={14} /> Ajouter {remaining.length} produit{remaining.length > 1 ? 's' : ''}</>}
                        </button>
                      </div>
                    );
                  })}
                  {dayCampaigns.map((campaign) => {
                    const unassigned = !campaign.assignments?.length && !campaign.responsibleEmployeeId;
                    const { targetAvailable, openingCarryOver } =
                      campaignMenuQuantities(campaign);
                    return (
                      <div key={campaign.id} className={`fabrication-calendar-event status-${campaign.status.toLowerCase()}`}>
                        <button
                          type="button"
                        ...
                        <div className="fabrication-calendar-event-content">
                          ...
                          <span>
                            ...
                            {targetAvailable > 0 ? (
                              <>
                                {' · '}Vitrine {targetAvailable.toLocaleString('fr-FR')}
                                {' · '}Report {openingCarryOver.toLocaleString('fr-FR')}
                                {' · '}À produire {Number(campaign.plannedPortions).toLocaleString('fr-FR')}
                              </>
                            ) : (
                              <>
                                {' · '}Objectif {Number(campaign.grossRequirement ?? campaign.plannedPortions).toLocaleString('fr-FR')}
                              </>
                            )}
                          </span>
                        </div>
                        <div className="fabrication-calendar-event-footer">
                          ...
                          {['DRAFT', 'PROPOSED', 'PLANNED'].includes(campaign.status) && (
                            ...
                            >
                              {validateBusy === campaign.id ? <Loader2 size={13} className="spin" /> : <Check size={13} />}
                              Valider
                            </button>
                          )}
                        </div>
                  ...
                  {!loading && !menusLoading && !dayCampaigns.length && !dayMenus.length && mode !== 'month' && (
                    <div className="fabrication-calendar-empty-actions">
                      <button type="button" className="fabrication-calendar-empty" onClick={() => openCatalogPlanner(day)}>
                        <BookOpen size={17} />
                        <span><strong>Planifier depuis la carte</strong>Choisir les produits et les portions</span>
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        <Sparkles size={13} /> Production exceptionnelle hors menu
                      </button>
                    </div>
                  )}
                </div>
        ...
        {catalogPlannerDate && (
          ...
          >
            ...
            >
              <header>
                <div>
                  <span><BookOpen size={16} /> Production du {formatDay(catalogPlannerDate)}</span>
                  <h2>Que souhaitez-vous avoir en vitrine ?</h2>
                  <p>
                    Sélectionnez les produits de la carte et l’objectif disponible pour la journée.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="fabrication-catalog-planner-settings">
                <label>
                  <span><BookOpen size={15} /> Carte / menu proposé</span>
                  <select
                ...
                <label>
                  <span><Building2 size={15} /> Site</span>
                  <select
                ...
                <label>
                  <span><Users size={15} /> Service responsable</span>
                  <select
                ...
                <label>
                  <span><Clock3 size={15} /> Prêt pour</span>
                  <input
                    type="time"
              ...
              {catalogCarryOver?.previousClosureDate && (
                <div className="fabrication-carry-over-banner">
                  <CheckCircle2 size={18} />
                  <div>
                    <strong>Report de la dernière clôture pris en compte</strong>
                    <span>
                      Le restant conservé est automatiquement déduit de ce qu’il faut produire.
                    </span>
                  </div>
                </div>
              )}
              ...
              <div className="fabrication-catalog-products">
                {catalogPlannerItems.map((item) => {
                    ...
                    existingDailyItem &&
                      selectedCatalogDayMenu?.productionLinks?.some(
                        (link) =>
                          ...
                          (link.productionOrder?.technicalSheetId ===
                            item.technicalSheetId ||
                  ...
                  const carryOver = Number(
                    carryOverByProduct.get(
                      item.technicalSheet?.outputProductId ?? '',
                    ) ?? 0,
                  );
                  const targetPortions = Number(line?.targetPortions ?? 0);
                  const toProduce = Math.max(targetPortions - carryOver, 0);
                  return (
                    <article
                      key={item.id}
                    ...
                    >
                      ...
                      <div className="fabrication-catalog-product-name">
                        ...
                        <span>
                          ...
                          {alreadyPlanned && ' · Déjà planifié, objectif modifiable'}
                        </span>
                        <span
                      ...
                      <div className="fabrication-catalog-quantity-result">
                        <span>Report veille <strong>{carryOver.toLocaleString('fr-FR')}</strong></span>
                        <span>À produire <strong>{toProduce.toLocaleString('fr-FR')}</strong></span>
                      </div>
                    </article>
                  );
                })}
                {!catalogs.length && (
                  <div className="fabrication-closure-empty">
                    Aucun produit de carte n’est encore relié à une fiche technique.
                    Configurez d’abord la carte dans le module Menus.
                  </div>
                )}
              ...
              <footer>
                ...
                >
                  <Sparkles size={15} /> Production hors menu
                </button>
                <div>
                  <button
                    type="button"
        ...
        {editingCampaign && (
          ...
          >
            ...
            >
              <header>
                <div>
                  <span><Pencil size={16} /> Fabrication planifiée</span>
              ...
              <div className="fabrication-planner-form-grid">
                ...
                <label className="fabrication-edit-target">
                  <span><Sparkles size={15} /> {
                    campaignMenuQuantities(editingCampaign).targetAvailable > 0
                      ? 'Objectif disponible en vitrine'
                      : 'Objectif de portions à produire'
                  }</span>
                  <input
                    type="number"
                    min="1"
                ...
                {campaignMenuQuantities(editingCampaign).targetAvailable > 0 && (
                  <div className="fabrication-edit-calculation">
                    <span>Report de la veille</span>
                    <strong>
                      {campaignMenuQuantities(
                        editingCampaign,
                      ).openingCarryOver.toLocaleString('fr-FR')}
                    </strong>
                    <span>Besoin net à recalculer</span>
                    <strong>
                      {Math.max(
                        Number(editDraft.targetPortions || 0) -
                          campaignMenuQuantities(editingCampaign).openingCarryOver,
                        0,
        ...
        {assignmentCampaign && (
          ...
          >
            ...
            >
              <header>
                <div>
                  <span><Users size={16} /> Équipe de production</span>
              ...
              <div className="fabrication-assignment-guide">
                ...
                <div>
                  ...
                  <span>
                    Chaque personne sélectionnée retrouvera les étapes dans son planning.
                    La couronne désigne le responsable principal.
                  </span>
                </div>
              </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div>
                          ...
                          <span>
                            {employee.position?.name ?? employee.department?.name ?? 'Collaborateur'}
                            {' · '}
        ...
        {validationCampaign && (
          ...
          >
            ...
            >
              <header>
                <div>
                  <span><AlertCircle size={16} /> Contrôle avant validation</span>
                  <h2>Stock insuffisant pour cette fabrication</h2>
                  <p>
                    {validationCampaign.name} ne peut pas réserver tous les ingrédients
                    nécessaires.
                  </p>
                </div>
                <button
              ...
              <div className="fabrication-shortage-list">
                {campaignShortages(validationCampaign).length ? (
                  ...
                  <div className="fabrication-validation-warning">
                    Le contrôle serveur signale un manque de stock. Rechargez les stocks
                    ou validez avec une justification.
                  </div>
                )}
              </div>
              ...
              <label className="fabrication-validation-reason">
                ...
                />
                <small>
                  Cette justification sera enregistrée dans l’historique de production.
                </small>
              </label>
        ...
        {menuPlanner && (
          ...
          >
            ...
            >
              <header>
                <div>
                  <span><BookOpen size={16} /> Menu du {formatDay(dayKey(menuPlanner.date!))}</span>
              ...
              <footer>
                ...
                <button
                  ...
                  disabled={
                    menuPlanning ||
                    !Object.values(menuDraft.lines).some((line) => line.selected)
                  }
                >
                  {menuPlanning ? <Loader2 size={17} className="spin" /> : <CalendarDays size={17} />}
      ...
      <AnimatePresence>
        {createOpen && (
          <div className="fabrication-modal-overlay" onMouseDown={(event) => event.target === event.currentTarget && setCreateOpen(false)}>
            <motion.form
              initial={{ opacity: 0, scale: .96, y: 14 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: .96, y: 14 }}
            ...
            >
              <header>
                <div><span><ChefHat size={16} /> Production hors menu</span><h2>Ajouter une fabrication libre</h2></div>
                <button type="button" onClick={() => setCreateOpen(false)}><X size={18} /></button>
              </header>
              <div className="fabrication-planner-form-grid">
                <label><span><Building2 size={15} /> Site</span>
                  <select
                    value={draft.siteId}
                    onChange={(event) => {
                      ...
                      setDraft((current) => ({
                        ...
                        siteId: nextSite,
                        profileId:
                          availableProfiles.find((profile) => profile.siteId === nextSite)?.id ??
                          '',
                      }));
                    }}
                  >
                ...
                <label><span><ChefHat size={15} /> Produit / fiche technique</span>
                  <select value={draft.profileId} onChange={(event) => setDraft((current) => ({ ...current, profileId: event.target.value }))}>
                    <option value="">Choisir…</option>
                    {selectedSiteProfiles.map((profile) => (
                      <option key={profile.id} value={profile.id}>
                        {profile.technicalSheet?.name ?? profile.outputProduct?.name ?? 'Fabrication'}
                      </option>
                    ))}
                  </select>
                  {draft.siteId && !selectedSiteProfiles.length && <small>Aucun profil de production configuré pour ce site.</small>}
                </label>
                <label><span><Users size={15} /> Service</span>
                  <select value={draft.serviceId} onChange={(event) => setDraft((current) => ({ ...current, serviceId: event.target.value }))}>
                    <option value="">Choisir…</option>
                    {departments.map((department) => <option key={department.id} value={department.id}>{department.name}</option>)}
                  </select>
                </label>
                <label><span><CalendarDays size={15} /> Date</span>
                  <input type="date" value={draft.date} onChange={(event) => setDraft((current) => ({ ...current, date: event.target.value }))} />
                </label>
                <label><span><Clock3 size={15} /> Heure souhaitée</span>
                  <input type="time" value={draft.time} onChange={(event) => setDraft((current) => ({ ...current, time: event.target.value }))} />
                </label>
                <label><span><Sparkles size={15} /> Objectif de portions</span>
                  <input type="number" min="1" step="1" value={draft.target} onChange={(event) => setDraft((current) => ({ ...current, target: event.target.value }))} />
                </label>
              </div>
              <footer>
                <button type="button" onClick={() => setCreateOpen(false)}>Annuler</button>
                <button type="submit" className="production-btn-primary" disabled={createBusy}>
      ...
      <AnimatePresence>
        {closure && (
          <div className="fabrication-modal-overlay" onMouseDown={(event) => event.target === event.currentTarget && setClosure(null)}>
            <motion.section
              initial={{ opacity: 0, scale: .96, y: 14 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: .96, y: 14 }}
            ...
            >
              <header>
                <div>
                  <span><CheckCircle2 size={16} /> Clôture de la vitrine</span>
                  <h2>{formatDay(dayKey(closure.date))}</h2>
                  <p>{closure.site?.name} · {closure.items.length} produit{closure.items.length > 1 ? 's' : ''}</p>
                </div>
                <button type="button" onClick={() => setClosure(null)}><X size={18} /></button>
              </header>
              {!closure.items.length ? (
                <div className="fabrication-closure-empty">Aucune fabrication à clôturer pour cette journée.</div>
              ) : (
                  ...
                  {closure.items.map((item, index) => (
                    <article key={item.orderId}>
                      <div className="fabrication-closure-product">
                        <span>Produit {index + 1} sur {closure.items.length}</span>
                        <h3>{item.productName}</h3>
                        <small>{item.orderNumber}{item.plannedTime ? ` · ${item.plannedTime}` : ''}</small>
                      </div>
                      <div className="fabrication-closure-summary">
                        <div><span>Objectif</span><strong>{item.targetPortions}</strong></div>
                        <div><span>Report précédent</span><strong>{item.openingCarryOverPortions}</strong></div>
                        <div><span>Fabriqué</span><strong>{item.producedPortions}</strong></div>
                        <div className="total"><span>Total disponible</span><strong>{item.totalAvailablePortions}</strong></div>
                      </div>
                      <div className="fabrication-closure-inputs">
                        <label>Restant physique
                          <input
                            type="number"
                            min="0"
                        ...
                        <label>Jeté / perdu
                          <input
                            ...
                            onChange={(event) => updateClosureItem(item.orderId, { discardedPortions: Number(event.target.value) })}
                          />
                        </label>
                        <label>Report proposé demain
                          <input
                            type="number"
                            min="0"
                            ...
                            onChange={(event) => updateClosureItem(item.orderId, { carryOverNextPortions: Number(event.target.value) })}
                          />
                        </label>
                        <label className="calculated">Sorti estimé<strong>{item.estimatedOutPortions}</strong></label>
                      </div>
                      {item.discardedPortions > 0 && (
                        <label className="fabrication-closure-reason">Motif de la perte
                          <input
                            value={item.lossReason ?? ''}
                            disabled={closure.status === 'CLOSED'}
                            onChange={(event) => updateClosureItem(item.orderId, { lossReason: event.target.value })}
              ...
              <footer>
                <button type="button" onClick={() => setClosure(null)}>Fermer</button>
                {closure.status !== 'CLOSED' && closure.items.length > 0 && (
                  <button type="button" className="production-btn-primary" onClick={() => void closeDay()} disabled={closureSaving}>
                    {closureSaving ? <Loader2 size={17} className="spin" /> : <CheckCircle2 size={17} />}
                    Clôturer la journée
                  </button>
                )}
                {closure.status === 'CLOSED' && <span className="fabrication-closure-closed"><Check size={16} /> Journée clôturée</span>}
              </footer>
            </motion.section>
          </div>
        )}
