...
type SettingKey = 'needs' | 'presets' | 'availability' | 'rules' | 'payroll' | 'costs';
...
export function PlanningApp({ token, tab, session, collaborators, departments, positions, rotations, sites, canWrite, onNavigate }: Props) {
  ...
  return (
    <div className="hr-shell planning-shell">
      ...
      {['dashboard', 'planning', 'attendance'].includes(tab) ? (
        <PeriodWorkflowPanel
          status={periodStatus}
          canWrite={canWrite}
...
function PlanningDashboard({ dashboard, alerts, setup, period, setPeriod, hoursByDepartment, actions, history }: { dashboard: ReturnType<typeof buildDashboard>; alerts: PlanningAlert[]; setup: ReturnType<typeof buildPlanningSetup>; period: DashboardPeriod; setPeriod: (value: DashboardPeriod) => void; hoursByDepartment?: Array<Record<string, any>>; actions?: Array<Record<string, any>>; history?: Array<Record<string, any>> }) {
  const distribution = normalizeHoursByDepartment(hoursByDepartment);
  const actionRows = actions?.length ? actions : [];
  return (
    <>
      {!setup.complete ? <PlanningSetupCompact setup={setup} /> : null}
      <div className="planning-toolbar">
        <div>
          <span className="section-tagline">Période dashboard</span>
          <h2>{period === 'week' ? 'Semaine' : period === 'year' ? 'Année' : 'Mensuel'} sélectionné</h2>
        </div>
        <div className="planning-segmented">
          {(['week', 'month', 'year'] as DashboardPeriod[]).map((value) => <button key={value} className={period === value ? 'active' : ''} onClick={() => setPeriod(value)}>{value === 'week' ? 'Semaine' : value === 'year' ? 'Année' : 'Mensuel'}</button>)}
        </div>
      </div>

      <div className="stats-grid planning-kpi-grid">
        {[
          { label: 'Heures planifiées', value: `${dashboard.plannedHours} h`, Icon: Clock },
          { label: 'Coût estimé', value: `${dashboard.estimatedCost} €`, Icon: Coins },
          { label: 'Heures supplémentaires', value: `${dashboard.overtimeHours} h`, Icon: CalendarRange },
          { label: 'Alertes actives', value: dashboard.activeAlerts, Icon: Bell },
        ].map(({ label, value, Icon }) => <motion.div className="stat-card-modern" key={label} whileHover={{ y: -2 }}><div className="stat-icon"><Icon size={20} /></div><div><span>{label}</span><strong>{value}</strong></div></motion.div>)}
      </div>

      <div className="planning-dashboard-grid">
        <div className="card-modern">
          <div className="section-header-modern"><span className="card-title"><ShieldAlert size={18} /> Alertes prioritaires</span><span className="section-tagline">{dashboard.priorityAlerts} prioritaire(s)</span></div>
        ...
        <div className="card-modern planning-actions-panel">
          <div className="section-header-modern"><span className="card-title"><ListChecks size={18} /> Actions à traiter</span><span className="section-tagline">{dashboard.actionsToProcess} action(s)</span></div>
...
function MonthlyCalendar({ selectedDate, onDayClick, assignments, requirements, collaborators, selectedEmployeeId, days, activeQuickMode, onEditAssignment }: { selectedDate: string; onDayClick: (day: string) => void | Promise<void>; assignments: PlanningAssignment[]; requirements: PlanningRequirement[]; collaborators: HrCollaborator[]; selectedEmployeeId: string; days: string[]; activeQuickMode: boolean; onEditAssignment: (assignment: PlanningAssignment) => void }) {
  const selectedMonth = selectedDate.slice(0, 7);
  return (
    <div className="card-modern planning-calendar-card">
      <div className="planning-month-grid planning-month-grid-final">
        ...
        {days.map((day) => {
          ...
          return (
            <div key={day} role="button" tabIndex={0} className={`month-cell planning-month-cell ${tone} ${outsideMonth ? 'outside' : ''} ${sameDay(day, selectedDate) ? 'selected' : ''} ${activeQuickMode ? 'assignment-target' : ''}`} onClick={() => void onDayClick(day)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); void onDayClick(day); } }}>
              <b>{dayNumber(day)}</b>
              <div className="month-shift-list">
                {assignmentGroups.slice(0, 4).map((group) => {
                  ...
                  return <span key={group.employeeId} role="button" tabIndex={0} className={`assignment-pill status-${status.className} ${group.hasConflict ? 'conflict' : ''} ${dimmed ? 'dimmed' : ''}`} onClick={(event) => { event.stopPropagation(); onEditAssignment(group.assignments[0]); }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); event.stopPropagation(); onEditAssignment(group.assignments[0]); } }}><strong>{shortName(group.collaborator)}</strong>{group.assignments.map((assignment) => <small key={assignment.id}>{assignmentDisplayRange(assignment)}</small>)}<em>{businessStatusLabel(group.businessStatus)}</em></span>;
                })}
                {assignmentGroups.length > 4 ? <small>+{assignmentGroups.length - 4} autre(s)</small> : null}
...
function PlanningSettings({ selected, setSelected, requirements, templates, rotations, legacyRotations, dayPresets, employeeTemplateAssignments, absences, departments, positions, sites, collaborators, settings, setup, onOpenInitialSetup, selectedDate, canWrite, onSaveRequirement, onDeleteRequirement, onSaveDayPreset, onDeleteDayPreset, onSaveWeeklyRotation, onDeleteWeeklyRotation, onSaveEmployeeTemplateAssignment }: { selected: SettingKey; setSelected: (value: SettingKey) => void; requirements: PlanningRequirement[]; templates: PlanningTemplate[]; rotations: PlanningTemplate[]; legacyRotations: HrRotation[]; dayPresets: PlanningTemplate[]; employeeTemplateAssignments: PlanningEmployeeTemplateAssignment[]; absences: Array<Record<string, any>>; departments: HrDepartment[]; positions: HrPosition[]; sites: Site[]; collaborators: HrCollaborator[]; settings?: Record<string, any>; setup: ReturnType<typeof buildPlanningSetup>; onOpenInitialSetup: () => void; selectedDate: string; canWrite: boolean; onSaveRequirement: (payload: RequirementPayload, id?: string) => Promise<void>; onDeleteRequirement: (id: string) => Promise<void>; onSaveDayPreset: (payload: PlanningDayPresetPayload, id?: string) => Promise<void>; onDeleteDayPreset: (id: string) => Promise<void>; onSaveWeeklyRotation: (payload: PlanningWeeklyRotationPayload, id?: string) => Promise<void>; onDeleteWeeklyRotation: (id: string) => Promise<void>; onSaveEmployeeTemplateAssignment: (payload: PlanningEmployeeTemplateAssignment) => Promise<void> }) {
  const cards: Array<{ key: SettingKey; title: string; description: string; count: string; status: PlanningSetupStatus; Icon: typeof ClipboardList }> = [
    ...
    { key: 'payroll', title: 'Règles paie & majorations', description: 'Profils France / Finlande configurables par entreprise.', count: 'Placeholder', status: 'partial', Icon: FileSignature },
    { key: 'costs', title: 'Coûts employeur', description: 'Salaire brut, coefficient employeur et mode avancé futur.', count: 'Placeholder', status: 'partial', Icon: Coins },
  ];
  return (
    <>
...
function SettingsDetail({ selected, requirements, rotations, legacyRotations, dayPresets, employeeTemplateAssignments, absences, departments, positions, sites, collaborators, settings, selectedDate, canWrite, onSaveRequirement, onDeleteRequirement, onSaveDayPreset, onDeleteDayPreset, onSaveWeeklyRotation, onDeleteWeeklyRotation, onSaveEmployeeTemplateAssignment }: { selected: SettingKey; requirements: PlanningRequirement[]; templates: PlanningTemplate[]; rotations: PlanningTemplate[]; legacyRotations: HrRotation[]; dayPresets: PlanningTemplate[]; employeeTemplateAssignments: PlanningEmployeeTemplateAssignment[]; absences: Array<Record<string, any>>; departments: HrDepartment[]; positions: HrPosition[]; sites: Site[]; collaborators: HrCollaborator[]; settings?: Record<string, any>; selectedDate: string; canWrite: boolean; onSaveRequirement: (payload: RequirementPayload, id?: string) => Promise<void>; onDeleteRequirement: (id: string) => Promise<void>; onSaveDayPreset: (payload: PlanningDayPresetPayload, id?: string) => Promise<void>; onDeleteDayPreset: (id: string) => Promise<void>; onSaveWeeklyRotation: (payload: PlanningWeeklyRotationPayload, id?: string) => Promise<void>; onDeleteWeeklyRotation: (id: string) => Promise<void>; onSaveEmployeeTemplateAssignment: (payload: PlanningEmployeeTemplateAssignment) => Promise<void> }) {
  ...
  if (selected === 'payroll') return <PayrollRulesSettings profile={settings?.payrollRuleProfiles as Record<string, any> | undefined} />;
  return <EmployerCostsSettings />;
}
...
function PresetsRotationsSettings({ dayPresets, weeklyRotations, legacyRotations, assignments, departments, positions, sites, collaborators, canWrite, onSaveDayPreset, onDeleteDayPreset, onSaveWeeklyRotation, onDeleteWeeklyRotation, onSaveEmployeeTemplateAssignment }: { dayPresets: PlanningTemplate[]; weeklyRotations: PlanningTemplate[]; legacyRotations: HrRotation[]; assignments: PlanningEmployeeTemplateAssignment[]; departments: HrDepartment[]; positions: HrPosition[]; sites: Site[]; collaborators: HrCollaborator[]; canWrite: boolean; onSaveDayPreset: (payload: PlanningDayPresetPayload, id?: string) => Promise<void>; onDeleteDayPreset: (id: string) => Promise<void>; onSaveWeeklyRotation: (payload: PlanningWeeklyRotationPayload, id?: string) => Promise<void>; onDeleteWeeklyRotation: (id: string) => Promise<void>; onSaveEmployeeTemplateAssignment: (payload: PlanningEmployeeTemplateAssignment) => Promise<void> }) {
  ...
  return (
    <>
      <div className="section-header-modern"><span className="card-title">Presets & roulements</span><span className="section-tagline">Stockage actuel : planning_templates</span></div>
      <div className="planning-settings-controls planning-settings-split">
        ...
        <div className="settings-list planning-settings-list">
          {dayPresets.map((preset) => <div key={preset.id} className="planning-settings-item"><div><strong>{preset.name}</strong><span>{presetRangeLabel(preset)} - {businessStatusLabel(presetBusinessStatus(preset) ?? 'work')} - {preset.employeeIds?.length ?? 0} attribution(s)</span></div><div className="planning-item-actions"><button className="btn btn-secondary btn-compact" type="button" onClick={() => editPreset(preset)}>Modifier</button><button className="btn btn-secondary btn-compact danger" type="button" disabled={!canWrite} onClick={() => void onDeleteDayPreset(preset.id)}>Archiver</button></div></div>)}
          {!dayPresets.length ? <p className="muted">Aucun preset jour Planning.</p> : null}
        </div>
      </div>
      <div className="planning-settings-controls planning-settings-split">
        ...
        <div className="settings-list planning-settings-list">
          {weeklyRotations.map((rotation) => <div key={rotation.id} className="planning-settings-item"><div><strong>{rotation.name}</strong><span>{weeklyRotationSummary(rotation)} - {rotation.employeeIds?.length ?? 0} attribution(s)</span></div><div className="planning-item-actions"><button className="btn btn-secondary btn-compact" type="button" onClick={() => editRotation(rotation)}>Modifier</button><button className="btn btn-secondary btn-compact danger" type="button" disabled={!canWrite} onClick={() => void onDeleteWeeklyRotation(rotation.id)}>Archiver</button></div></div>)}
      ...
      <form className="planning-need-form" onSubmit={(event) => void submitAssignment(event)}>
        <strong>Attribution collaborateurs</strong>
        ...
        <p className="muted">Les attributions sont conservées temporairement dans le JSON du template, sans duplication RH.</p>
      </form>
      <div className="settings-list">
...
function AttendanceView({ rows, assignments, collaborators, selectedMonth, employeeFilter }: { rows: Array<Record<string, any>>; assignments: PlanningAssignment[]; collaborators: HrCollaborator[]; selectedMonth: string; employeeFilter: string }) {
  ...
  const displayRows = rows.length ? rows : assignments.map((assignment) => ({ assignmentId: assignment.id, employeeId: assignment.employeeId ?? assignment.collaboratorId, date: assignment.date, plannedStartTime: assignment.startTime, plannedEndTime: assignment.endTime, plannedMinutes: Math.round(assignmentHours(assignment) * 60), declaredMinutes: null, varianceMinutes: null, status: 'NON_SIGNE' }));
  const filteredRows = displayRows.filter((row) => String(row.date ?? '').slice(0, 7) === selectedMonth && (!employeeFilter || row.employeeId === employeeFilter) && (!statusFilter || row.status === statusFilter));
  return (
    <div className="card-modern attendance-card">
      <div className="section-header-modern">
        <div><span className="card-title"><FileSignature size={18} /> Émargement</span><span className="section-tagline">Prévu vs réel, signature salarié future et validation manager.</span></div>
        <span className="status-pill">Placeholder architecture</span>
      </div>
      <div className="planning-settings-controls">
      ...
      <div className="planning-table attendance-table">
        {filteredRows.map((row, index) => {
          const collaborator = findCollaborator(collaborators, row.employeeId);
          const variance = row.varianceMinutes == null ? null : minutesToHours(Number(row.varianceMinutes));
          return <div className="planning-row planning-row-actions" key={row.assignmentId ?? index}><strong>{collaboratorName(collaborator)}</strong><span>{formatAttendanceDate(row.date)} - {timeLabel(row.plannedStartTime)} à {timeLabel(row.plannedEndTime)}</span><span>{formatHours(minutesToHours(row.plannedMinutes))} prévues / {row.declaredMinutes == null ? 'signature à venir' : `${formatHours(minutesToHours(row.declaredMinutes))} déclarées`}</span><span>{variance == null ? 'Écart n.c.' : `${formatHours(variance)} d’écart`}</span><span className="status-pill">{statusLabel[row.status] ?? row.status ?? 'Non signé'}</span><button className="btn btn-secondary btn-compact" type="button" disabled>Valider</button></div>;
        })}
        {!filteredRows.length ? <GuidedEmptyState title="Aucune ligne d’émargement" description="Les heures planifiées alimentent cette vue. Les signatures salariés et heures déclarées réelles seront ajoutées plus tard." /> : null}
      </div>
      <p className="muted">Aucune signature réelle n’est simulée. Les statuts prévus sont : non signé, signé, à valider, validé, anomalie.</p>
    </div>
  );
}
...
function DepartmentHours({ distribution }: { distribution: Array<{ name: string; hours: number; percent: number }> }) {
  ...
  return <div className="department-hours-list">{distribution.map((item) => <div key={item.name} className="department-hour-row"><div><strong>{item.name}</strong><span>{item.hours} h - {item.percent}%</span></div><div className="hour-bar"><i style={{ width: `${item.percent}%` }} /></div></div>)}</div>;
}
...
function buildDashboard(data: PlanningBootstrap | undefined, assignments: PlanningAssignment[], alerts: PlanningAlert[], replacements: PlanningReplacementProposal[], requirements: PlanningRequirement[]) {
  const fallbackAlerts = alerts.length ? alerts.length : requirements.length + replacements.length;
  return {
    plannedHours: data?.summary?.plannedHours ?? Math.round(assignments.reduce((sum, assignment) => sum + assignmentHours(assignment), 0) * 10) / 10,
    estimatedCost: data?.summary?.estimatedCost ?? 0,
    overtimeHours: data?.summary?.overtimeHours ?? 0,
    activeAlerts: data?.summary?.activeAlerts ?? fallbackAlerts,
    priorityAlerts: data?.summary?.priorityAlerts ?? alerts.filter((alert) => ['critique', 'attention', 'critical', 'warning'].includes(String(alert.level))).length,
...
function buildPlanningSetup({ collaborators, departments, requirements, templates, planningProfile, onNavigate, openSetting }: { collaborators: HrCollaborator[]; departments: HrDepartment[]; requirements: PlanningRequirement[]; templates: PlanningTemplate[]; planningProfile: string; onNavigate: (tab: PlanningTab) => void; openSetting: (setting: SettingKey) => void }) {
  ...
  const steps: PlanningSetupStep[] = [
    { key: 'profile', title: 'Profil pays', description: planningProfile ? profileLabel(planningProfile) : 'France, Finlande ou personnalisé.', status: planningProfile ? 'done' : 'todo', actionLabel: 'Choisir', action: () => openSetting('payroll') },
...
function consolidateAssignmentsByEmployee(assignments: PlanningAssignment[], collaborators: HrCollaborator[]): CalendarAssignmentGroup[] {
  ...
  return Array.from(groups.entries()).map(([employeeId, items]) => {
    ...
    return { employeeId, collaborator: findCollaborator(collaborators, employeeId), assignments: sorted, businessStatus: assignmentBusinessStatus(sorted[0]), hasConflict };
  });
}
...
function assignmentHours(assignment: PlanningAssignment) {
  ...
  return Math.max(0, (end - start - Number(assignment.breakMinutes ?? 0)) / 60);
}
...
function formatHours(value: number) { return `${Number.isFinite(value) ? value : 0} h`; }
