...
const apps = [
  {
    ...
    size: 'Temps réel',
    tagline:
      'Suivez les cours du marché alimentaire FranceAgriMer et analysez l’évolution des prix de milliers de produits.',
    description:
      'Cours des Produits transforme ToqueHub en centre de veille économique alimentaire. Les données RNM FranceAgriMer sont consultées en temps réel via le backend ToqueHub, sans import ni duplication dans vos référentiels métier.\n\nFonctionnalités clés :\n- Catalogue RNM, recherche, secteurs, catégories et pagination.\n- Fiches produits avec dernières cotations et historique graphique.\n- Favoris personnels persistés, conservés après désinstallation.\n- Historique global filtrable pour suivre les tendances de marché.',
  ...
  },
  {
    ...
    version: 'v1.0.0',
    compatibility:
      'ToqueHub Core v0.1.0+ + Stocks et Fiches Techniques requis, RH/Planning optionnels',
    status: 'Disponible',
  },
  {
    id: 'haccp',
  ...
  },
  {
    ...
    description:
      'Gérez vos commandes à partir des fournisseurs et produits Stocks.\n\nFonctionnalités clés :\n- Suggestions de réassort sans création automatique.\n- Envoi des commandes PDF par Resend.\n- Import et rapprochement des bons de livraison.\n- Réceptions partielles et intégration contrôlée dans Stocks.',
    screenshots: ['Bon de commande', 'Réception livraison', 'Analyses achats'],
    changelog:
      'Lancement V1 : brouillons, envoi Resend, référentiels Stocks, BL, écarts et réceptions Stocks idempotentes.',
    version: 'v1.0.0',
    compatibility: 'ToqueHub Core v0.1.0+ + Stocks obligatoire',
    status: 'Disponible',
  },
  ...
  },
  {
    ...
    description:
      'RH devient le référentiel humain central de l’établissement : collaborateurs avec ou sans compte ToqueHub, services, postes, organigramme, historique et liaison unique avec les utilisateurs Core.',
    screenshots: ['Tableau de bord RH', 'Collaborateurs', 'Organigramme'],
    changelog:
      'Lancement V1 avec services et postes de départ créés automatiquement à l’installation.',
    version: 'v1.0.0',
    compatibility: 'ToqueHub Core v0.1.0+',
    status: 'Disponible',
  },
  {
    ...
    size: '2.1 Mo',
    tagline:
      'Pilotez plannings jour/semaine/mois, affectations, absences RH, remplacements et besoins opérationnels.',
    description:
...
const isStocksSettingsRoute = (tab: ActiveTab): tab is StocksSettingsTab =>
  STOCKS_SETTINGS_TABS.includes(tab as StocksSettingsTab);
...
export function Dashboard({ session, onLogout, onSessionSwitch }: DashboardProps) {
  ...
  const [productImportReturnToStocksOnboarding, setProductImportReturnToStocksOnboarding] =
    useState(false);
  ...
  const [autoUpdateOperation, setAutoUpdateOperation] = useState<SystemUpdateOperation | null>(
    null,
  );
  ...
  function addAppNotification(type: AppNotification['type'], message: string) {
    setNotifications((current) => {
      ...
      if (
        ...
        now.getTime() - newest.createdAt.getTime() < 3000
      ) {
        return current;
      }
      return [
  ...
  async function refresh() {
    ...
    try {
      ...
      if (Array.isArray(usersResult)) {
        ...
        if (typeof usersResult.devSwitchEnabled === 'boolean')
          setDevSwitchEnabled(usersResult.devSwitchEnabled);
      }
  ...
  async function checkForUpdateNotification() {
    ...
    try {
      ...
      const operationRunning = Boolean(
        status.runtime.lastOperation &&
        ['queued', 'running', 'rollback'].includes(status.runtime.lastOperation.status),
      );
      if (
        status.updateAvailable &&
        status.runtime.updaterAvailable &&
        !operationRunning &&
        !hasDismissed
      ) {
        ...
        addAppNotification(
          'success',
          `Mise à jour ToqueHub disponible : ${status.latest?.tag ?? status.latest?.version}.`,
        );
      }
    } catch {
  ...
  useEffect(() => {
    if (
      !autoUpdateOperation ||
      !['queued', 'running', 'rollback'].includes(autoUpdateOperation.status)
    )
      return;
      ...
      try {
        ...
        if (!['queued', 'running', 'rollback'].includes(next.status)) {
          ...
          if (next.status === 'success')
            addAppNotification('success', 'Mise à jour ToqueHub terminée.');
          if (next.status === 'error' || next.status === 'rollback')
            addAppNotification('error', 'La mise à jour ToqueHub nécessite une vérification.');
        }
      } catch (err) {
  ...
  const rhPlanningReadiness = getRhPlanningReadiness(
    hrInstalled,
    hrDepartments,
    hrPositions,
    hrCollaborators,
  ...
  const isRnmTab = useMemo(
    () => ['rnm-dashboard', 'rnm-history', 'rnm-favorites', 'rnm-about'].includes(activeTab),
    [activeTab],
  );
  const isHrTab = useMemo(
    () =>
      [
        'hr-dashboard',
        'hr-collaborators',
  ...
  const isTechnicalSheetsTab = useMemo(
    () =>
      [
        'technical-sheets-dashboard',
        'technical-sheets-recipes',
  ...
  const allApps = useMemo(
    () => [
      ...
      },
      {
        ...
        title: 'Achats',
        icon: ShoppingCart,
        installed: purchasingInstalled,
        expanded: purchasingMenuExpanded,
        setExpanded: setPurchasingMenuExpanded,
        isActive: isPurchasingTab,
        defaultTab: 'purchasing-dashboard',
        submenu: [
    ...
    [
      ...
      isPlanningTab,
      technicalSheetsInstalled,
      technicalSheetsMenuExpanded,
      isTechnicalSheetsTab,
      productionInstalled,
      productionMenuExpanded,
      isProductionTab,
      menusInstalled,
      menusMenuExpanded,
      isMenusTab,
  ...
  const completed = dashboardSummary?.progress
    ? Math.round((dashboardSummary.progress.percent / 100) * progressItems.length)
    : progressItems.filter((item) => item.done).length;
  const progress =
    dashboardSummary?.progress?.percent ?? Math.round((completed / progressItems.length) * 100);
  const activeUsersCount =
    dashboardSummary?.counts.activeUsers ??
    users.filter((user) => user.status === 'ACTIVE' || (user as any).isActive === true).length;
  const canWriteHr =
    isAdmin || ['MANAGER', 'RESPONSABLE'].includes(session.user.role?.toUpperCase());
  const hrActiveCollaboratorsCount =
    dashboardSummary?.counts.hrCollaborators ??
    dashboardSummary?.counts.collaborators ??
    hrSummary?.counts?.collaborators ??
    hrCollaborators.filter((collaborator) => !isArchived(collaborator)).length;
      ...
      stocks.reduce(
        (sum, stock) =>
          ...
          numeric(
            stock.stockValue ??
              stock.value ??
              numeric(stock.currentQuantity ?? stock.quantity) *
                numeric(
                  stock.product.averagePrice ??
                    stock.product.averagePurchasePrice ??
                    stock.product.weightedAveragePrice,
                ),
          ),
        0,
      ),
  ...
  const movementsThisMonth = useMemo(
    () => movements.filter((m) => movementEffectiveDate(m) >= monthStart).length,
    [movements, monthStart],
  );
  const topConsumed = useMemo(
    () =>
      Object.values(
        movements
          .filter((m) =>
            ['LOSS', 'OUT', 'EXIT', 'PRODUCTION', 'CORRECTION', 'INVENTORY'].includes(m.type),
          )
          .reduce<Record<string, { name: string; qty: number; unit?: string }>>((acc, m) => {
  ...
  async function toggleDashboardWidget(widgetId: string, visible: boolean) {
    ...
    const hiddenWidgetIds = visible
      ? currentHidden.filter((id) => id !== widgetId)
      : Array.from(new Set([...currentHidden, widgetId]));
  ...
  async function toggleDashboardPin(widgetId: string) {
    ...
    const pinnedWidgetIds = currentPinned.includes(widgetId)
      ? currentPinned.filter((id) => id !== widgetId)
      : [...currentPinned, widgetId];
  ...
  async function handleCreateUser(payload: {
    ...
    await submit(
      ...
      'Utilisateur créé. Le mot de passe temporaire ne sera plus affiché.',
    );
  ...
  async function handleDisableUser(user: CoreUser) {
    if (
      !window.confirm(
        `Désactiver ${displayUserName(user)} ? Le compte restera visible pour l’historique.`,
      )
    )
      return;
  ...
  async function handleUpdateRolePermissions(roleKey: string, nextPermissions: string[]) {
    await submit(
      () => api.updateRolePermissions(token, roleKey, nextPermissions),
  ...
  async function installStocks() {
    ...
    try {
      ...
      setSuccess(
        'L’application Stocks a été installée. Les menus métier sont maintenant visibles pour toute l’organisation.',
      );
      await refresh();
      setActiveTab('stocks-dashboard');
      setShowStocksOnboarding(true);
  ...
  async function uninstallStocks() {
    ...
    try {
      ...
      setSuccess(
        'L’application Stocks a été supprimée de l’interface. Les données métier existantes sont conservées.',
      );
      await refresh();
    } catch (err) {
  ...
  async function uninstallRnmPrices() {
    ...
    try {
      ...
      setInstalledApps(
        summary.installedApplications ?? installedApps.filter((app) => app !== 'rnm-prices'),
      );
      if (isRnmTab) setActiveTab('applications');
      setSuccess(
        'L’application Cours des Produits a été retirée de l’interface. Vos favoris RNM sont conservés.',
      );
      await refresh();
    } catch (err) {
      setError(
        err instanceof Error ? err.message : 'Suppression de Cours des Produits impossible.',
      );
    } finally {
      setAppActionLoading(false);
      setConfirmation(null);
  ...
  async function uninstallPlanning() {
    ...
    try {
      ...
      setDashboardSummary((prev) => ({ ...prev, ...summary }) as DashboardSummary);
      setInstalledApps(
        summary.installedApplications ?? installedApps.filter((app) => app !== 'planning'),
      );
      if (isPlanningTab) setActiveTab('applications');
      setSuccess(
        'L’application Planning a été retirée de l’interface. Les plannings et historiques existants sont conservés.',
      );
      await refresh();
    } catch (err) {
  ...
  async function uninstallTechnicalSheets() {
    ...
    try {
      ...
      setDashboardSummary((prev) => ({ ...prev, ...summary }) as DashboardSummary);
      setInstalledApps(
        summary.installedApplications ?? installedApps.filter((app) => app !== 'technical-sheets'),
      );
      if (isTechnicalSheetsTab) setActiveTab('applications');
      setShowTechnicalSheetsOnboarding(false);
      setSuccess(
        'L’application Fiches Techniques a été retirée de l’interface. Les fiches, historiques, coûts et simulations sont conservés.',
      );
      await refresh();
    } catch (err) {
  ...
  async function uninstallProduction() {
    ...
    try {
      ...
      setDashboardSummary((prev) => ({ ...prev, ...summary }) as DashboardSummary);
      setInstalledApps(
        summary.installedApplications ?? installedApps.filter((app) => app !== 'production'),
      );
      if (isProductionTab) setActiveTab('applications');
      setSuccess(
        'L’application Production a été retirée de l’interface. Les ordres et historiques existants sont conservés.',
      );
      await refresh();
    } catch (err) {
  ...
  async function uninstallMenus() {
    ...
    try {
      ...
      setDashboardSummary((prev) => ({ ...prev, ...summary }) as DashboardSummary);
      setInstalledApps(
        summary.installedApplications ?? installedApps.filter((app) => app !== 'menus'),
      );
      if (isMenusTab) setActiveTab('applications');
      setSuccess(
        'L’application Menus a été retirée de l’interface. Les menus, cycles, convives, exports et historiques sont conservés.',
      );
      await refresh();
    } catch (err) {
  ...
  async function uninstallPurchasing() {
    ...
    setSuccess(undefined);
    try {
      const summary = await api.uninstallPurchasing(token);
      setInstalledApps(
        summary.installedApplications ?? installedApps.filter((app) => app !== 'purchasing'),
      );
      if (isPurchasingTab) setActiveTab('applications');
      setSuccess(
        'L’application Achats a été retirée de la navigation. Commandes, réceptions, paramètres et historiques sont conservés.',
      );
      await refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Suppression d’Achats impossible.');
    } finally {
      setAppActionLoading(false);
      setConfirmation(null);
    }
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        if (appId === 'purchasing' && !stocksInstalled)
          throw new Error(
            'Installez Stocks avant Achats. Produits, fournisseurs, unités, sites et stocks restent la source de vérité.',
          );
        const summary =
          appId === 'rnm-prices'
            ? await api.installRnmPrices(token)
            : appId === 'hr'
              ? await api.installHr(token)
              : appId === 'planning'
                ? await api.installPlanning(token)
                : appId === 'technical-sheets'
                  ? await api.installTechnicalSheets(token)
                  : appId === 'production'
        ...
        setSuccess(
          appId === 'rnm-prices'
            ? 'L’application Cours des Produits a été installée. La navigation RNM est maintenant visible.'
            : appId === 'hr'
              ? 'L’application RH a été installée. Services et postes de départ sont disponibles.'
              : appId === 'planning'
                ? 'L’application Planning a été installée. Les vues opérationnelles consomment désormais le référentiel RH.'
                : appId === 'technical-sheets'
                  ? 'L’application Fiches Techniques a été installée. La configuration guidée est prête.'
                  : appId === 'production'
  ...
  async function refreshHr() {
    ...
    const activeCollaboratorsCount = (data.collaborators ?? []).filter(
      (collaborator) => !isArchived(collaborator),
    ).length;
  ...
  async function handleUploadHrCollaboratorDocument(
    employeeId: string,
    payload: { file: File; category: string; notes?: string; expiresAt?: string },
  ) {
    await submit(
      () => api.uploadHrCollaboratorDocument(token, employeeId, payload),
      'Document RH enregistré.',
    );
    await refreshHr();
  }
  ...
  async function handleDeleteHrCollaboratorDocument(employeeId: string, documentId: string) {
    ...
    await submit(
      () => api.deleteHrCollaboratorDocument(token, employeeId, documentId),
      'Document RH supprimé.',
    );
    await refreshHr();
  }

  async function handleReplaceHrCollaboratorDocument(
    employeeId: string,
    documentId: string,
    file: File,
  ) {
    const document = (await submit(
      () => api.replaceHrCollaboratorDocument(token, employeeId, documentId, file),
      'Document RH remplacé.',
    )) as HrDocument;
    await refreshHr();
    return document;
  }
  ...
  async function handleArchiveHrCollaborator(id: string) {
    if (
      !window.confirm(
        'Archiver ce collaborateur ? Son historique et ses relations seront conservés.',
      )
    )
      return;
  ...
  async function handleCreateHrDepartmentsBulk(names: string[]) {
    const result = await submit(
      () => api.createHrDepartmentsBulk(token, names),
      `${names.length} services sélectionnés.`,
    );
    const created =
      typeof (result as any)?.created === 'number' ? (result as any).created : names.length;
    const skipped = typeof (result as any)?.skipped === 'number' ? (result as any).skipped : 0;
    setSuccess(
      `${created} service${created > 1 ? 's' : ''} créé${created > 1 ? 's' : ''}${skipped ? `, ${skipped} déjà présent${skipped > 1 ? 's' : ''}` : ''}.`,
    );
    await refreshHr();
  }
  ...
  async function handleCreateHrPositionsBulk(items: HrReferencePayload[]) {
    const result = await submit(
      () => api.createHrPositionsBulk(token, items),
      `${items.length} postes sélectionnés.`,
    );
    const created =
      typeof (result as any)?.created === 'number' ? (result as any).created : items.length;
    const skipped = typeof (result as any)?.skipped === 'number' ? (result as any).skipped : 0;
    setSuccess(
      `${created} poste${created > 1 ? 's' : ''} créé${created > 1 ? 's' : ''}${skipped ? `, ${skipped} déjà présent${skipped > 1 ? 's' : ''}` : ''}.`,
    );
    await refreshHr();
  }
  ...
  async function handleDeleteCategory(categoryId: string) {
    await submit(
      () => api.archiveCategory(token, categoryId),
  ...
  async function handleCreateMovement(payload: {
    productId: string;
    supplierId?: string;
  ...
  async function handleUploadStocksOcr(files: File[]) {
    ...
    if (!config.configured) {
      ...
      setActiveTab('organization-general');
      setError(
        'Ajoutez une clé API Mistral dans Organisation > Général > Clés API avant de lancer un import OCR Stocks.',
      );
      return;
    }
    const uploaded = await api.uploadStocksOcrDocuments(token, files);
    const initialStatuses = uploaded.documents.map((document) => ({
      document,
      ocr: null,
      extraction: null,
      state: 'upload',
    ...
    const refreshed = await Promise.all(
      uploaded.documents.map((document) =>
        api
          .stocksOcrStatus(token, document.id)
          .catch(() => ({ document, ocr: null, extraction: null, state: 'en attente' })),
      ),
    );
    ...
    void refreshMyDocuments();
    setSuccess(
      `${uploaded.documents.length} document${uploaded.documents.length > 1 ? 's' : ''} envoyé${uploaded.documents.length > 1 ? 's' : ''} en analyse.`,
    );
  }
  ...
  async function handleRenameDocument(document: MyDocument, newName: string) {
    await submit(
      () => api.renameDocument(token, document.id, newName),
      'Document renommé avec succès.',
    );
    void refreshMyDocuments();
  }
  ...
  async function handleCreateOcrReception(payload: StocksOcrExtraction['data']) {
    ...
    if (validatedDocumentId)
      setOcrStatuses((current) =>
        current.filter((item) => item.document.id !== validatedDocumentId),
      );
  ...
  async function handleCreateOcrProductFromLine(line: StocksOcrLine, supplierId?: string | null) {
    ...
    if (!unitId)
      throw new Error(
        `Unité OCR "${unitLabel || 'non renseignée'}" introuvable. Sélectionnez une unité sur la ligne ou ajoutez-la au référentiel.`,
      );
    ...
    if (existing) {
      const shouldUpdatePrice = Boolean(
        unitPrice &&
        numeric(
          existing.averagePrice ?? existing.averagePurchasePrice ?? existing.weightedAveragePrice,
        ) <= 0,
      );
      const shouldUpdateCategory = Boolean(
        categoryId && !(existing.categoryId ?? existing.category?.id),
      );
      const shouldUpdateSupplier = Boolean(
        supplierId &&
        !(
          existing.primarySupplierId ??
          existing.supplierId ??
          existing.primarySupplier?.id ??
        ...
        return (await submit(
          () =>
            api.updateProduct(token, existing.id, {
              ...
              primarySupplierId:
                existing.primarySupplierId ?? existing.supplierId ?? supplierId ?? undefined,
              averagePrice: shouldUpdatePrice
                ? unitPrice
                : numeric(
          ...
          shouldUpdateSupplier
            ...
            : shouldUpdatePrice
              ? 'Prix produit mis à jour depuis l’OCR.'
              : 'Catégorie produit mise à jour depuis l’OCR.',
        )) as Product;
      }
      return existing;
    }
  ...
  async function handleCreateOcrSupplier(name: string) {
    ...
    const existing = suppliers.find(
      (supplier) => normalizeLookup(supplier.name) === normalizeLookup(supplierName),
    );
    if (existing) return existing;
    return (await submit(
      () => api.createSupplier(token, { name: supplierName }),
      'Fournisseur créé depuis l’OCR.',
    )) as Supplier;
  }
  ...
  async function handleCreateInventory(payload: {
    name: string;
    date?: string;
    comment?: string;
    siteId?: string;
    ...
    const created = await submit(
      () => api.createInventory(token, payload),
      'Inventaire complet créé. Vous pouvez saisir les quantités comptées.',
    );
  ...
  async function handleSaveInventoryCounts(
    ...
    await submit(
      () => api.updateInventoryCounts(token, inventoryId, lines),
      'Comptage inventaire enregistré.',
    );
  }
  ...
  async function handleValidateInventory(
    ...
    await submit(
      () => api.validateInventory(token, inventoryId, lines),
      'Inventaire validé. Les corrections de stock ont été générées.',
    );
  }

  async function handlePrefillStocks(payload: {
    categories?: boolean;
    units?: boolean;
    sites?: boolean;
    locations?: boolean;
  ...
  const activeProducts = useMemo(
    () => products.filter((p) => showArchived || !isArchived(p)),
    [products, showArchived],
  );
  const categoriesWithUncategorizedLast = useMemo(
  ...
  const activeSites = useMemo(
    () => sites.filter((s) => showArchived || !isArchived(s)),
    [sites, showArchived],
  );
  const activeLocations = useMemo(
    () => locations.filter((l) => showArchived || !isArchived(l)),
  ...
  const ocrConfigured = Boolean(
    dashboardSummary?.organization?.apiKeys?.mistral.configured ??
    session.user.apiKeys?.mistral.configured,
  );
  ...
  const filteredStocks = useMemo(() => {
    return stocks.filter((stock) => {
      const matchesSearch =
        ...
        (stock.lot?.lotNumber &&
          stock.lot.lotNumber.toLowerCase().includes(inventorySearch.toLowerCase())) ||
        (stock.site?.name &&
          stock.site.name.toLowerCase().includes(inventorySearch.toLowerCase())) ||
        (stock.location?.name &&
    ...
    () =>
      activeLocations.filter(
        (l) =>
          ...
          l.site?.name?.toLowerCase().includes(locationSearch.toLowerCase()),
      ),
  ...
  const filteredInventories = useMemo(
    () =>
      inventories.filter(
        (i) =>
          i.name.toLowerCase().includes(inventorySessionSearch.toLowerCase()) ||
          (i.status ?? '').toLowerCase().includes(inventorySessionSearch.toLowerCase()),
      ),
    [inventories, inventorySessionSearch],
  );
  ...
  const filteredAudit = useMemo(
    () =>
      auditEntries.filter((a) =>
        `${a.action} ${a.entityType ?? ''} ${a.user?.email ?? ''}`
          .toLowerCase()
  ...
  const filteredMovements = useMemo(() => {
    return sortMovementsByRecency(movements).filter((m) => {
      const matchesSearch =
        m.product.name.toLowerCase().includes(movementSearch.toLowerCase()) ||
        ...
        (m.createdBy?.email &&
          m.createdBy.email.toLowerCase().includes(movementSearch.toLowerCase()));
    ...
    return activeSuppliers.filter((s) => {
      return (
        ...
        (s.purchasingProfile?.orderEmail &&
          s.purchasingProfile.orderEmail.toLowerCase().includes(supplierSearch.toLowerCase())) ||
    ...
    >
      ...
      <header className="mobile-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          />
          <span className="sidebar-title" style={{ fontSize: '1.1rem' }}>
            TOQUE<span>HUB</span>
          </span>
        </div>
          ...
          <div className="sidebar-notifications-container">
            ...
            >
              ...
              {unreadNotifications > 0 ? (
                <span className="sidebar-notifications-count">
                  ...
                  <div className="sidebar-notifications-list">
                    {notifications.length === 0 ? (
                      <div className="sidebar-notifications-empty">
                        Les messages de l'application apparaîtront ici.
                      </div>
                    ) : (
                      notifications.map((notification) => (
                        <div
                          key={notification.id}
                          className={`sidebar-notification-item ${notification.type}`}
                        >
                ...
                return (
                  <div key={app.id} className="sidebar-app-group">
                    ...
                    >
                      ...
                      {app.expanded ? (
                        <ChevronDown size={14} className="expand-indicator" />
                      ) : (
                    ...
                    {app.expanded && (
                      <div className="sidebar-submenu">
                        {app.submenu.map((sub) => {
                          ...
                          const isSubActive =
                            activeTab === sub.tab || matchingTabs.includes(activeTab);
                          return (
                            <div
                  ...
                  {profileDropdownMode === 'main' ? (
                    <div className="profile-dropdown-inner">
                      ...
                      {devSwitchEnabled && isAdmin && (
                        <button
                          className="dropdown-item"
                          onClick={() => setProfileDropdownMode('users')}
                        >
                          <RefreshCw size={14} />
        ...
        <header className="topbar-modern">
          ...
          <div className="topbar-actions">
            ...
            <button
              ...
              onClick={() => void openChangelogModal()}
            >
              <History size={13} />
              Changelog
            </button>
            ...
            >
                    ...
                    <>
                      <div className={`welcome-hero theme-${dashboardTheme}`}>
                        ...
                        <p className="welcome-desc">
                          Bienvenue dans l'espace de gestion de <strong>{organizationName}</strong>.
                          Votre ERP de cuisine open source local et souverain est entièrement
                          fonctionnel.
                        </p>
                      </div>
                          ...
                          {stocksInstalled ? (
                            <>
                              <Metric
                                ...
                                value={products.filter((p) => !isArchived(p)).length}
                            ...
                            <>
                              ...
                              <Metric
                                icon={<UsersRound size={20} />}
                                value={hrActiveCollaboratorsCount}
                      ...
                      <div className="dashboard-widget-grid" style={{ marginTop: '1.5rem' }}>
                        {stocksInstalled && (
                          <>
                            ...
                            <div className="card-modern dashboard-widget-card">
                              <span className="card-title">
                                <TrendingUp size={18} /> Produits consommés
                              </span>
                                ...
                                {topConsumed.length ? (
                                  topConsumed.map((p) => (
                                    <div className="progress-item-modern" key={p.name}>
                                      ...
                                      <span className="progress-val-modern">
                                  ...
                                  <EmptyMini
                                    ...
                                    text="Les mouvements de sortie alimenteront ce classement."
                                    icon="📈"
                                  />
                                )}
                              </div>
                          ...
                          {productionInstalled ? (
                            <div className="installed-app-summary">
                              ...
                              <div>
                                ...
                                <p className="muted">
                                  productions aujourd'hui · portions prévues et retards.
                                </p>
                              </div>
                            </div>
                          ) : (
                            <EmptyMini
                              title="Production"
                              text="Installez le module Production pour planifier vos ordres de fabrication."
                              icon="🚧"
                            />
                          )}
                          <button
                            ...
                            onClick={() =>
                              goToTab(productionInstalled ? 'production-dashboard' : 'applications')
                            }
                          >
                        ...
                        <div className="card-modern dashboard-widget-card">
                          ...
                          {menusInstalled ? (
                            ...
                            <EmptyMini
                              title="Menus"
                              text="Installez le module Menus pour planifier les repas et convives."
                              icon="🚧"
                            />
                          )}
                          <button
                            ...
                            style={{ marginTop: 'auto' }}
                            onClick={() =>
                              goToTab(menusInstalled ? 'menus-dashboard' : 'applications')
                            }
                          >
                        ...
                        <div className="card-modern dashboard-widget-card">
                          <span className="card-title">
                            <UsersRound size={18} /> Collaborateurs
                          </span>
                          <div className="installed-app-summary">
                            <div
                              className={`metric-icon-wrapper ${hrInstalled ? 'purple' : 'gray'}`}
                            >
                              <UsersRound />
                            </div>
                            <div>
                              <strong style={{ fontSize: '1.7rem' }}>
                                {hrActiveCollaboratorsCount}
                              </strong>
                              <p className="muted">
                                {hrInstalled
                                  ? 'collaborateurs actifs dans le référentiel RH.'
                                  : 'Installez RH pour centraliser vos collaborateurs.'}
                              </p>
                            </div>
                          </div>
                          <button
                            ...
                            style={{ marginTop: 'auto' }}
                            onClick={() =>
                              goToTab(hrInstalled ? 'hr-collaborators' : 'applications')
                            }
                          >
                          ...
                          <div className="card-modern dashboard-widget-card">
                            ...
                            <div className="progress-list">
                              {progressItems.map((item, idx) => (
                                <div
                                  key={idx}
                                  className={`progress-item ${item.done ? 'done' : ''}`}
                                  style={{
                                    display: 'flex',
                                    alignItems: 'center',
                                    gap: '0.5rem',
                        ...
                        {visibleWidgets.apps && (
                          <div className="card-modern dashboard-widget-card">
                            <span className="card-title" style={{ marginBottom: '1.25rem' }}>
                              <Boxes size={18} /> Applications
                            </span>
                            <div className="installed-app-summary">
                              ...
                              <div>
                                <strong style={{ fontSize: '1.7rem' }}>
                                  {installedApps.length} active{installedApps.length > 1 ? 's' : ''}
                                </strong>
                                <p className="muted">
                                  {stocksInstalled
                                    ? 'Stocks est actif pour votre organisation.'
                                    : 'Installez votre première application.'}
                                </p>
                              </div>
                            </div>
                            <button
              ...
              {isRnmTab && rnmInstalled && (
                <CoursProduitsApp
                  token={token}
                  initialTab={
                    activeTab === 'rnm-history'
                      ? 'history'
                      : activeTab === 'rnm-favorites'
                        ? 'favorites'
                        : activeTab === 'rnm-about'
                          ? 'about'
                          : 'dashboard'
              ...
              {isHrTab && hrInstalled && (
                <HrApp
                  tab={
                    activeTab === 'hr-collaborators'
                      ? 'collaborators'
                      : activeTab === 'hr-departments'
                        ? 'departments'
                        : activeTab === 'hr-positions'
                          ? 'positions'
                          : activeTab === 'hr-orgchart'
                            ? 'orgchart'
                            : 'dashboard'
                  }
                  summary={hrSummary}
              ...
              {isPlanningTab && planningInstalled && (
                <PlanningApp
                  ...
                  session={session}
                  tab={
                    activeTab === 'planning-settings' ||
                    activeTab === 'planning-templates' ||
                    activeTab === 'planning-requirements' ||
                    activeTab === 'planning-absences'
                      ? 'settings'
              ...
              {isTechnicalSheetsTab && technicalSheetsInstalled && (
                <TechnicalSheetsApp
                  token={token}
                  tab={
                    activeTab === 'technical-sheets-recipes'
                      ? 'recipes'
                      : activeTab === 'technical-sheets-categories'
                        ? 'categories'
              ...
              {isPurchasingTab && purchasingInstalled && (
                <Suspense
                  fallback={<div className="dashboard-loading">Chargement du module Achats…</div>}
                >
                  <PurchasingApp
                    token={token}
                    tab={
              ...
              {activeTab === 'settings' && (
                <SettingsPage
                  ...
                  sites={sites}
                  focusApiKeys={apiKeysPanelHint}
                  focusSiteAddress={siteAddressModalHint}
                  onSiteAddressOpened={() => setSiteAddressModalHint(false)}
                  onApiKeysSaved={() => {
                    setApiKeysPanelHint(false);
                    void refresh();
                  }}
                  onSettingsSaved={() => {
                    void refresh();
              ...
              {activeTab === 'organization-documents' && (
                <MyDocumentsPage
                  ...
                  onDownload={(document) =>
                    api.downloadDocument(token, document.id, document.originalName)
                  }
              ...
              {activeTab === 'users' && !isAdmin && (
                <div className="card-modern">
                  <span className="card-title">Accès réservé</span>
                  <p>Seul un Administrateur peut gérer les utilisateurs.</p>
                </div>
              )}

              {/* TAB ARCHITECTURE */}
              {activeTab === 'architecture' && isAdmin && <ArchitectureCenter session={session} />}
                    ...
                    <div className="section-header-modern">
                      <div className="section-info">
                        ...
                        <span className="section-tagline">
                          Familles de produits conservées dans votre organisation ToqueHub. Les
                          éléments archivés restent dans les historiques.
                        </span>
                      </div>
                      <button
                        className="btn btn-primary"
                          ...
                          >
                            ...
                            <p>
                              {category.description ||
                                `${products.filter((product) => (product.categoryId ?? product.category?.id) === category.id).length} produit(s)`}
                            </p>
                          </motion.div>
                        ))
                      )}
                          ...
                          {filteredStocks.length === 0 ? (
                            <tr>
                              <td colSpan={4}>
                                <div className="empty-state">
                                  ...
                                  <span className="empty-state-desc">
                                    Aucun mouvement n’a encore créé de quantité. Les produits du
                                    catalogue sans stock restent visibles dans « Catalogue produits
                                    ».
                                  </span>
                                  <button
                                    className="btn btn-primary"
                            ...
                            filteredStocks.map((stock) => (
                              <tr key={stock.id}>
                                ...
                                <td>
                                  {stock.lot?.lotNumber ?? '—'}{' '}
                                  {stock.lot?.expiresAt || stock.lot?.expirationDate ? (
                                    <span className="badge badge-correction">
                                      DLC{' '}
                                      {new Date(
                                ...
                                <td>
                                  {stock.product.minimumStock ?? stock.product.minStock ?? '—'}
                                </td>
                                <td>
                                  <span className={`badge ${stockStatus(stock).className}`}>
                            ...
                            <tr>
                              <td colSpan={6}>
                                <div className="empty-state">
                                  ...
                                  <span className="empty-state-desc">
                                    Aucun mouvement de stock n'a été effectué ou aucun ne correspond
                                    à vos filtres.
                                  </span>
                                </div>
                              </td>
                                ...
                                <tr key={m.id}>
                                  ...
                                  <td
                                    style={{
                                      ...
                                      fontWeight: 700,
                                      color:
                                        m.type === 'LOSS' ? 'var(--danger)' : 'var(--text-main)',
                                    }}
                                  >
                                    {movementSign(m.type)}
                                    {m.quantity} {m.product.unit?.symbol ?? ''}
                                  </td>
                                  <td>
                                    {m.supplier?.name || (
                  ...
                  <div className="card-modern">
                    <div className="section-header-modern">
                      <div className="section-info">
                        ...
                        <span className="section-tagline">
                          Références utilisées ou achetées, même sans quantité en stock.
                        </span>
                      </div>
                      <div className="row-actions">
                        <button
                          className="btn btn-primary"
                          onClick={() => setShowAddImportModal(true)}
                        >
                          ...
                          {filteredProducts.length === 0 ? (
                            <tr>
                              <td colSpan={7}>
                                <div className="empty-state">
                                  ...
                                  <span className="empty-state-desc">
                                    Ajustez la recherche ou les filtres fournisseur/catégorie.
                                  </span>
                                  <button
                                    className="btn btn-primary"
                                    onClick={() => setShowAddImportModal(true)}
                            ...
                            filteredProducts.map((p) => (
                              ...
                              >
                                ...
                                <td style={{ textAlign: 'right' }}>
                                  {numeric(p.minimumStock ?? p.minStock)
                                    ? parseFloat(
                                        numeric(p.minimumStock ?? p.minStock).toFixed(2),
                                      ).toString()
                                    : '—'}
                                </td>
                              </tr>
              ...
              {activeTab === 'inventories' && (
                <>
                  ...
                  <InventoriesPage
                    ...
                    products={products}
                    search={inventorySessionSearch}
                    setSearch={setInventorySessionSearch}
                    onCreate={() => setShowInventoryModal(true)}
                    onOpen={(inventory) => setSelectedInventoryId(inventory.id)}
                  />
                </>
              )}
                  ...
                  <div className="card-modern">
                    <div className="section-header-modern">
                      <div className="section-info">
                        ...
                        <span className="section-tagline">
                          Coordonnées de vos partenaires fournisseurs pour les réceptions de
                          marchandises.
                        </span>
                      </div>
                      <button
                            ...
                            <tr>
                              <td colSpan={2}>
                                <div className="empty-state">
                                  ...
                                  <span className="empty-state-desc">
                                    Ajoutez votre premier fournisseur pour tracer les livraisons de
                                    marchandises dans votre cuisine.
                                  </span>
                                  <button
                                    className="btn btn-primary"
                                    onClick={() => setShowSupplierModal(true)}
      ...
      {/* Product Modal */}
      {showAddImportModal ? (
        <div className="drawer-backdrop" onClick={() => setShowAddImportModal(false)}>
          <aside className="side-drawer" onClick={(event) => event.stopPropagation()}>
            <div className="side-drawer-header">
              <div>
                <span className="stocks-onboarding-kicker">Articles</span>
                <h2>Ajouter / importer</h2>
                <span>Ajoutez une référence ou réceptionnez vos achats.</span>
              </div>
              <button className="icon-btn" onClick={() => setShowAddImportModal(false)}>
                <X size={18} />
              </button>
            </div>
      ...
      <Modal
        ...
        title="Créer un inventaire complet"
        size="product"
      >
        <InventoryForm
          sites={sites}
      ...
      >
        {editingUser && (
          <UserForm
            user={editingUser}
            roles={roles}
            onSubmitUpdate={(payload) => handleUpdateUser(editingUser.id, payload)}
            onClose={() => setEditingUser(null)}
          />
        )}
      </Modal>
      ...
      <Modal
        isOpen={selectedStoreApp !== null}
        onClose={() => setSelectedStoreApp(null)}
        title="Fiche Module - Toque Store"
      >
        {selectedStoreApp && (
          <AppStoreDetailSheet
            ...
            prerequisiteMessage={
              selectedStoreApp.id === 'planning' && !installedApps.includes('planning')
            ...
            onOpen={() => {
              ...
              if (selectedStoreApp.id === 'technical-sheets')
                setActiveTab('technical-sheets-dashboard');
            ...
            onUninstall={() => {
              if (selectedStoreApp.id === 'menus') setConfirmation('uninstall-menus');
                ...
                setConfirmation('uninstall-technical-sheets');
      ...
      <Modal
        isOpen={showUpdateAvailableModal}
        onClose={closeUpdateAvailableModal}
        title={autoUpdateOperation ? 'Installation de la mise à jour' : 'Mise à jour disponible'}
        size="lg"
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {autoUpdateOperation ? (
            <div
              className={`alert-modern ${autoUpdateOperation.status === 'success' ? 'success' : autoUpdateOperation.status === 'error' || autoUpdateOperation.status === 'rollback' ? 'error' : 'info'}`}
              style={
                autoUpdateOperation.status === 'running' || autoUpdateOperation.status === 'queued'
                  ? { background: '#f8fafc', borderColor: '#dbe4ef', color: '#334155' }
                  : undefined
              }
            >
              {isSystemUpdateRunning(autoUpdateOperation) ? (
                <RefreshCw size={17} className="spin" />
              ) : autoUpdateOperation.status === 'success' ? (
            ...
            <div
              ...
              style={{ background: '#f8fafc', borderColor: '#dbe4ef', color: '#334155' }}
            >
              <Download size={17} />
              <span>Une nouvelle version stable de ToqueHub est prête à être installée.</span>
            </div>
          )}
          ...
          <div className="settings-grid-premium">
            <div className="info-card-premium">
              ...
              <div className="info-card-premium-value">
                {autoUpdateStatus?.current.version ?? '-'}
              </div>
              <span
                className="badge badge-reception"
                style={{ width: 'fit-content', marginTop: '0.65rem' }}
              >
                {autoUpdateStatus?.current.imageTag ?? 'local'}
              </span>
            </div>
            <div className="info-card-premium">
              <div className="info-card-premium-header">
              ...
              <div className="info-card-premium-value">
                {autoUpdateStatus?.latest?.tag ?? autoUpdateStatus?.latest?.version ?? '-'}
              </div>
              {autoUpdateStatus?.latest?.source ? (
                <span
                  className="badge badge-reception"
                  style={{ width: 'fit-content', marginTop: '0.65rem' }}
                >
                  {autoUpdateStatus.latest.source === 'release' ? 'GitHub Release' : 'Tag GitHub'}
                </span>
              ) : null}
            </div>
          </div>
          ...
          {autoUpdateOperation ? (
            <>
              <div>
                ...
                >
                  <strong style={{ color: 'var(--text-main)', fontSize: '0.9rem' }}>
                    {systemUpdateProgress(autoUpdateOperation).label}
                  </strong>
                  <span className="badge badge-reception">
                    {systemUpdateProgress(autoUpdateOperation).percent}%
                  </span>
                </div>
                <div
                  className="progress-bar-bg"
                ...
                >
                  <div
                    ...
                    style={{
                      height: '100%',
                      width: `${systemUpdateProgress(autoUpdateOperation).percent}%`,
                      background:
                        autoUpdateOperation.status === 'error' ||
                        autoUpdateOperation.status === 'rollback'
                          ? '#ef4444'
                          : '#10b981',
                      borderRadius: '999px',
              ...
              <div className="settings-list">
                <div>
                  <span>Statut</span>
                  <strong>{updateStatusLabel(autoUpdateOperation.status)}</strong>
                </div>
                <div>
                  <span>Cible</span>
                  <strong>
                    {autoUpdateOperation.targetTag ?? autoUpdateStatus?.latest?.tag ?? '-'}
                  </strong>
                </div>
                <div>
                  <span>Démarrée</span>
              ...
              {autoUpdateOperation.error ? (
                <div className="alert-modern error">
                  <AlertCircle size={16} /> {autoUpdateOperation.error}
                </div>
              ) : null}
              ...
              >
                {(autoUpdateOperation.logs?.length
                  ? autoUpdateOperation.logs
            ...
            <p className="muted" style={{ margin: 0, lineHeight: 1.6 }}>
              L’installation crée d’abord une sauvegarde locale, télécharge les nouvelles images
              Docker, redémarre l’API et le web, puis vérifie l’état de santé. En cas d’échec, le
              service tente un rollback automatique.
            </p>
          )}

          {autoUpdateError ? (
            <div className="alert-modern error">
              <AlertCircle size={16} /> {autoUpdateError}
            </div>
          ) : null}
          ...
          <div className="modal-footer" style={{ margin: '0 -1.75rem -1.75rem' }}>
            <button
              className="btn btn-secondary"
              onClick={closeUpdateAvailableModal}
            ...
            autoUpdateOperation.status === 'rollback' ? (
              <button
                className="btn btn-primary"
                onClick={() => void applyUpdateFromModal()}
                disabled={autoUpdateApplying || !autoUpdateStatus?.runtime.updaterAvailable}
              >
                <Download size={15} />
                {autoUpdateApplying
                  ? 'Lancement...'
      ...
      <ConfirmationModal
        isOpen={confirmation === 'uninstall-purchasing'}
        title="Supprimer l’application Achats ?"
        text="L’entrée disparaîtra de la navigation, mais les commandes, offres fournisseur, bons de livraison, réceptions et historiques seront conservés."
        confirmLabel="Supprimer"
        danger
        loading={appActionLoading}
        onCancel={() => setConfirmation(null)}
      ...
      <div className="instance-info-grid">
        ...
        <InstanceInfoCard icon={<ExternalLink size={18} />} title="Adresses">
          ...
          <InstanceInfoRow
            label="Backend"
            value={
              <a href={info.api.url} target="_blank" rel="noreferrer">
                {info.api.url}
              </a>
            }
          />
          ...
          <InstanceInfoRow
            label="Origines CORS"
            value={info.frontend.configuredOrigins.join(', ') || '-'}
          />
        </InstanceInfoCard>
          ...
          <InstanceInfoRow
            ...
            value={
              <InstanceStatusPill
                ...
                label={info.database.connected ? 'Connecté' : 'Erreur'}
              />
            }
          />
        ...
        <InstanceInfoCard icon={<Wifi size={18} />} title="Mosquitto / MQTT">
          <InstanceInfoRow
            ...
            value={
              <InstanceStatusPill
                ok={info.mqtt.configured}
                label={info.mqtt.configured ? 'Configuré' : 'Non configuré'}
              />
            }
          />
          ...
          <InstanceInfoRow
            label="Zigbee2MQTT"
            value={
              info.mqtt.zigbee2mqttFrontendUrl ? (
                <a href={info.mqtt.zigbee2mqttFrontendUrl} target="_blank" rel="noreferrer">
                  {info.mqtt.zigbee2mqttFrontendUrl}
                </a>
              ) : null
            }
          ...
          <InstanceInfoRow
            ...
            value={
              <InstanceStatusPill
                ok={zigbeeAdapterPresent}
                label={zigbeeAdapterPresent ? 'Détectée' : 'Introuvable'}
              />
            }
          />
          ...
          <InstanceInfoRow
            label="Détectée"
            value={detectedSerialPorts.join(', ') || info.mqtt.suggestedZigbeeAdapterPath}
          />
        </InstanceInfoCard>
        ...
        <InstanceInfoCard icon={<HardDrive size={18} />} title="Docker & stockage">
          <InstanceInfoRow
            ...
            value={
              <InstanceStatusPill
                ok={info.docker.containerized}
                label={info.docker.containerized ? 'Conteneur' : 'Hors conteneur'}
              />
            }
          />
        ...
        <InstanceInfoCard icon={<Cpu size={18} />} title="Machine">
          ...
          <InstanceInfoRow
            label="Mémoire"
            value={`${formatInstanceBytes(info.host.freeMemoryBytes)} libres / ${formatInstanceBytes(info.host.totalMemoryBytes)}`}
          />
...
function ChangelogPanel({
  ...
  return (
    <div className="changelog-panel">
      ...
      {error ? (
        <div className="alert-modern error">
          <AlertCircle size={16} /> GitHub : {error}
        </div>
      ) : null}
      ...
      {!changelog.entries.length ? (
        <div className="instance-empty">
          ...
          <span>
            Publiez un tag vX.Y.Z pour générer automatiquement une release et l’afficher ici.
          </span>
        </div>
      ) : (
            ...
            <article key={entry.tag} className="changelog-entry">
              <div className="changelog-entry-header">
                ...
                <div className="changelog-entry-actions">
                  ...
                  {entry.isInstalled ? (
                    <span className="changelog-pill installed">Installée</span>
                  ) : null}
              ...
              <pre>
                {entry.notes?.trim() || 'Aucune note de version publiée pour cette release.'}
              </pre>
            </article>
          ))}
        </div>
...
function getRhPlanningReadiness(
  ...
  const activeCollaborators = collaborators.filter(
    (collaborator) => !isArchived(collaborator) && collaborator.status === 'ACTIVE',
  );
  ...
  const incompleteCollaborators = activeCollaborators.filter((collaborator) => {
    ...
    return (
      ...
      !activePositionIds.has(positionId)
    );
  });

  if (incompleteCollaborators.length) {
    const names = incompleteCollaborators
      .slice(0, 3)
      .map(
        (collaborator) =>
          `${collaborator.firstName ?? ''} ${collaborator.lastName ?? ''}`.trim() ||
    ...
    const suffix =
      incompleteCollaborators.length > 3 ? `, +${incompleteCollaborators.length - 3}` : '';
    missing.push(
      `un nom affichable, un service principal et un poste principal pour ${names.join(', ')}${suffix}`,
    );
  }
...
function normalizeProductSearchText(value?: string | null) {
  return normalizeSearchText(value)
    .replace(
      /\b(?:lot|dlc|ddm|prix|total|montant|tva|ht|ttc|net|brut|colis|carton|cartons|pieces|piece|unite|unites|kg|kgs|g|gr|l|litre|litres|ml|cl|x)\b/g,
      ' ',
    )
...
function scoreProductForOcrLine(line: StocksOcrLine, product: Product, supplierId?: string | null) {
  ...
  let score = Math.max(
    tokenSimilarity(label, productName),
    tokenSimilarity(normalizeProductSearchText(label), normalizeProductSearchText(productName)),
  );
...
const CATEGORY_KEYWORDS: Array<{ name: string; hints: string[]; aliases: string[] }> = [
  {
    ...
    aliases: ['viande', 'viandes', 'boucherie', 'volaille', 'volailles', 'produits frais'],
  },
  {
    name: 'Poissons',
    hints: [
    ...
    aliases: ['poisson', 'poissons', 'maree', 'marée', 'produits de la mer', 'produits frais'],
  },
  {
    ...
    aliases: ['cremerie', 'crèmerie', 'produits laitiers', 'laitier', 'fromage', 'produits frais'],
  },
  {
  ...
  },
  {
    ...
    aliases: ['boulangerie', 'patisserie', 'pâtisserie', 'pain', 'produits frais'],
  },
  {
    name: 'Épicerie',
    hints: [
    ...
    aliases: ['epicerie', 'épicerie', 'sec', 'produits secs'],
  },
  {
    name: 'Surgelés',
    hints: [
    ...
    aliases: ['surgeles', 'surgelés', 'surgelé', 'congelé'],
  },
  {
    name: 'Boissons',
    hints: [
  ...
  },
  {
    ...
    aliases: ['hygiene', 'hygiène', 'entretien', 'non alimentaire', 'consommables'],
  },
  {
    name: 'Emballages',
    hints: [
    ...
    aliases: ['emballage', 'emballages', 'non alimentaire', 'consommables'],
  },
  {
    name: 'Nutrition médicale',
    hints: [
      'clinutren',
      'thickenup',
      'resource',
...
function ocrKeywordCategoryRule(line: StocksOcrLine) {
  const text = normalizeProductSearchText(
    [line.ocrLabel, line.label, line.reference, line.categoryName, line.suggestedCategoryName]
      .filter(Boolean)
      .join(' '),
  );
  return CATEGORY_KEYWORDS.find((rule) =>
    rule.hints.some((hint) => {
      const normalizedHint = normalizeProductSearchText(hint);
      return (
        normalizedHint &&
        (text.includes(normalizedHint) || tokenSimilarity(text, normalizedHint) >= 0.72)
      );
    }),
  );
}
...
function isGenericOcrCategoryName(name?: string | null) {
  return [
    ...
    'divers',
    'autres',
  ].includes(normalizeSearchText(name));
}
...
function inferOcrCategoryId(line: StocksOcrLine, categories: Category[], products: Product[]) {
  ...
  if (aiCategoryId && activeCategories.some((category) => category.id === aiCategoryId))
    return aiCategoryId;
  ...
  if (closestProduct?.categoryId || closestProduct?.category?.id)
    return closestProduct.categoryId ?? closestProduct.category?.id ?? undefined;

  const keywordRule = ocrKeywordCategoryRule(line);
  if (keywordRule) {
    const category =
      findCategoryByBusinessName(activeCategories, keywordRule.name) ??
      keywordRule.aliases
        .map((alias) => findCategoryByBusinessName(activeCategories, alias))
        .find(Boolean);
  ...
  const rankedCategory = categories
    ...
    .map((category) => {
      ...
      return {
        category,
        score: Math.max(
          tokenSimilarity(label, haystack),
          normalizeProductSearchText(label).includes(normalizeProductSearchText(category.name))
            ? 0.78
            : 0,
        ),
      };
...
async function resolveOcrCategoryIdForCreate(
  token: string,
  line: StocksOcrLine,
  categories: Category[],
  products: Product[],
  ...
  const inferred =
    ...
    supplierMajorityCategoryId(supplierId, products);
  ...
  const firstUsefulCategory = categories.find(
    (category) =>
      !isArchived(category) &&
      !['aclasser', 'sanscategorie', 'nonclasse'].includes(normalizeLookup(category.name)),
  );
...
function enrichOcrProductMatches(
  data: StocksOcrExtraction['data'],
  products: Product[],
): StocksOcrExtraction['data'] {
  ...
  return {
    ...
    lines: (data.lines || []).map((line) => {
      ...
      const candidates = ranked
        ...
        .slice(0, 8)
        .map((candidate) => productCandidateFromProduct(candidate.product, candidate.score));
      if (!best || best.score < 0.58)
        return {
          ...line,
          productCandidates: candidates,
          matchingStatus: line.matchingStatus ?? 'NOT_FOUND',
          matchingScore: line.matchingScore ?? 0,
        };
...
function resolveOcrUnitId(units: Unit[], unitId?: string | null, unitLabel?: string | null) {
  ...
  for (const wanted of [key, ...(aliases[key] ?? [])]) {
    const found = units.find(
      (unit) => normalizeLookup(unit.symbol) === wanted || normalizeLookup(unit.name) === wanted,
    );
...
function resolveOcrReceptionUnits(
  data: StocksOcrExtraction['data'],
  units: Unit[],
): StocksOcrExtraction['data'] {
  return {
...
function ocrProductName(line: StocksOcrLine) {
  return String(line.ocrLabel || line.label || '')
    .trim()
    .slice(0, 180);
}
...
function stockStatus(stock: Stock) {
  ...
  if (minimum > 0 && quantity <= minimum)
    return { label: 'Stock faible', className: 'badge-correction' };
...
function ModularDashboardOverview({
  zones,
  theme,
  firstName,
  organizationName,
  ...
  zones: ModularDashboard['zones'];
  theme: string;
  firstName: string;
  organizationName: string;
  refreshLabel: string;
  onNavigate: (tab: ActiveTab) => void;
  onTogglePin: (widgetId: string) => Promise<void>;
}) {
  const zoneMeta = {
    kpi: {
      title: 'KPI',
      icon: TrendingUp,
      desc: 'Indicateurs synthétiques Core et modules installés.',
    },
    activity: {
      title: 'Activité',
      icon: History,
      desc: 'Flux récents, actions à suivre et états vides.',
    },
    analytics: {
      title: 'Analyses',
      icon: LineChart,
      desc: 'Tendances disponibles sans logique métier dans le Core.',
    },
    ...
    <div className="modular-dashboard-shell">
      <div className={`welcome-hero theme-${theme}`}>
        <span className="welcome-tag">
          Dashboard modulaire · Refresh global 5 min · {refreshLabel}
        </span>
        <h1 className="welcome-title">Bonjour {firstName} 👋</h1>
        <p className="welcome-desc">
          Vue intelligente de <strong>{organizationName}</strong>. Les widgets sont fournis par le
          registry Core, filtrés par applications installées et permissions de lecture.
        </p>
      </div>
        ...
        return (
          <section className={`dashboard-zone dashboard-zone-${zone}`} key={zone}>
            ...
            {widgets.length ? (
              ...
              <div className="card-modern">
                <EmptyMini
                  title={`Zone ${meta.title} vide`}
                  text="Aucun widget visible avec vos applications installées et permissions actuelles."
                  icon="🧩"
                />
              </div>
            )}
...
function DashboardCockpitOverview({
  ...
  const getUrgentIcon = (module: string) => {
    ...
    if (mod.includes('temp') || mod.includes('haccp-temp') || mod.includes('relev'))
      return <Thermometer size={20} />;
    if (mod.includes('clean') || mod.includes('nettoy') || mod.includes('surfac'))
    ...
    <div className="cockpit-dashboard">
      ...
      >
        <div className="cockpit-header-left">
          ...
          <h1>
            Bonjour {firstName} <span className="waving-hand">👋</span>
          </h1>
          <p>
        ...
        >
          <div className="cockpit-weather-card">
            ...
            <div className="cockpit-weather-actions">
              ...
              <button
                className="cockpit-btn-outline"
                onClick={onRefresh}
                title="Actualiser le cockpit"
              >
                <RefreshCw size={14} /> Actualiser{' '}
                <small style={{ opacity: 0.8, fontSize: '0.68rem', marginLeft: '0.2rem' }}>
                  {refreshedAt}
                </small>
              </button>
            </div>
        ...
        <section className="cockpit-urgent-banner">
          <div className="cockpit-urgent-header-block">
            ...
            <div className="cockpit-urgent-header-text">
              ...
              <h3>
                {cockpit.urgent.length} alerte{cockpit.urgent.length > 1 ? 's' : ''} opérationnelle
                {cockpit.urgent.length > 1 ? 's' : ''}
              </h3>
            </div>
          </div>
      ...
      <section className="cockpit-section">
        ...
        <div className="cockpit-news-grid cockpit-news-grid-single">
          <NewsCard
            title={
              cockpit.weather.city
                ? `Actualités · ${cockpit.weather.city.split(',')[0]}`
                : 'Actualités locales'
            }
...
function CockpitCard({
  ...
  const renderSparkline = () => {
    ...
    if (title.includes('effectif')) {
      const current =
        parseInt(String(card.value)) || collaborators.filter((c) => !c.isArchived).length || 0;
      const history = [];
      for (let i = 6; i >= 0; i--) {
      ...
      return (
        <svg
          className="cockpit-card-sparkline effectif"
          viewBox="0 0 100 30"
          width="80"
          height="24"
        >
    ...
    if (title.includes('stock')) {
      ...
      const sorted = [...movements].sort(
        (a, b) => movementEffectiveDate(a).getTime() - movementEffectiveDate(b).getTime(),
      );
      ...
      for (let i = 0; i < 7; i++) {
        ...
        const subsequentMovements = sorted.filter((m) => movementEffectiveDate(m) > targetDate);
  ...
  const renderCornerElement = () => {
    ...
    if (sectionType === 'overview') {
      ...
      if (title.includes('stock')) {
        return (
          <span className="cockpit-card-corner-icon">
            <ShoppingBag size={15} />
          </span>
        );
      }
...
function NewsCard({
  ...
  return (
    <motion.div
      className={`cockpit-news-card-modern ${large ? 'large' : ''}`}
    ...
    >
      ...
      <div className="cockpit-news-image-side" style={{ backgroundImage: `url(${image})` }}>
...
function dashboardWidgetModule(
  widget: Pick<DashboardWidget, 'module'> & { appId?: string; moduleLabel?: string; id?: string },
) {
  return String(
    widget.module ?? widget.appId ?? widget.moduleLabel ?? widget.id?.split('.')[0] ?? 'core',
  ).toLowerCase();
}
...
function DashboardWidgetCard({
  ...
  const value =
    widget.data?.value ?? widget.data?.count ?? widget.data?.total ?? widget.data?.label ?? '—';
  const subtitle = String(
    widget.data?.subtitle ??
      widget.data?.description ??
      widget.emptyMessage ??
      (widget.comingSoon ? 'Module à venir' : 'Données disponibles dès première utilisation.'),
  );
  ...
  if (compact) {
    return (
      <Metric
        ...
        value={String(value)}
        label={widget.title}
        tone={widgetTone(moduleKey)}
      />
    );
  }
  return (
    <motion.div
      className="card-modern dashboard-widget-card"
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
    >
      <div className="card-title-container">
        <span className="card-title">
          <Icon size={18} /> {widget.title}
        </span>
      </div>
      {widget.empty || widget.comingSoon ? (
        <EmptyMini
          title={widget.emptyTitle ?? (widget.comingSoon ? 'Bientôt disponible' : 'Aucune donnée')}
          text={subtitle}
          icon={widget.comingSoon ? '🚧' : '🧩'}
        />
      ) : (
        <div className="installed-app-summary">
          <div className={`metric-icon-wrapper ${widgetTone(moduleKey)}`}>
            <Icon />
          </div>
          <div>
            <strong style={{ fontSize: '1.7rem' }}>{String(value)}</strong>
      ...
      <div className="documents-toolbar-modern card-modern">
        <div className="search-input-wrapper-modern">
          ...
          <input
            ...
            placeholder="Rechercher fournisseur, numéro, fichier..."
          />
        </div>
      ...
      <div className="documents-layout">
            ...
            {items.length ? (
              viewMode === 'grid' ? (
                  ...
                  {items.map((document) => (
                    ...
                    >
                      <div className="doc-card-header">
                        ...
                        <span
                          className={`badge-pill ${documentStateBadge(document.processingState)}`}
                        >
                      ...
                      <div className="doc-card-footer">
                        <div
                          className="doc-supplier"
                          title={document.supplierName || 'Fournisseur non identifié'}
                        >
                ...
                >
                  <div className="card-title-container">
                    <span className="card-title">
                      <FileText size={18} /> Documents importés
                    </span>
                    <span className="section-tagline">
              ...
              >
                <EmptyMini
                  title={loading ? 'Chargement...' : 'Aucun document'}
                  text="Les bons de commande, factures et BL importés via l’OCR Stocks apparaîtront ici."
                  icon="📄"
                />
              </motion.div>
            )}
        ...
        <aside className="documents-insights-modern">
          <div className="card-modern insight-card-modern">
            <span className="card-title">
              <UsersRound size={18} /> Par fournisseur
            </span>
...
function computeStocksReadiness(
  categories: Category[],
  units: Unit[],
  products: Product[],
  sites: Site[],
  ...
  const foundationReady = Boolean(
    activeCategories.length && activeUnits.length && activeSites.length && activeLocations.length,
  );
  ...
  const hasValidatedOcr = ocrStatuses.some((status) => {
    ...
    return Boolean(
      ...
      (status as any).reception?.status === 'VALIDATED',
    );
  });
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        >
          ...
          <div style={{ color: 'white', fontSize: '1rem', fontWeight: 800, marginTop: '0.2rem' }}>
            {readiness.progress}% Métier Prêt
          </div>
          <p
            style={{ color: '#94a3b8', fontSize: '0.75rem', marginTop: '0.25rem', lineHeight: 1.4 }}
          >
            {activeProducts.length} produit(s), {activeSuppliers.length} fournisseur(s),{' '}
...
function StocksOnboardingWizard({
  ...
  const [step, setStep] = useState<StocksOnboardingStep>(() =>
    readiness.progress ? readiness.nextStep : 'welcome',
  );
...
function StocksOnboardingWelcome({ onNext, onClose }: { onNext: () => void; onClose: () => void }) {
  return (
    <GuidedWelcome
      title={
        <>
          Bienvenue sur le module <span>Stocks & Réceptions</span>
        </>
      }
      description="Le socle de catégories, unités et sites est préparé automatiquement. Importez ensuite votre catalogue CSV ou analysez un bon de commande, une facture ou un BL."
      benefits={[
        {
          icon: <Warehouse size={16} />,
          text: 'Socle créé automatiquement (catégories, unités, sites)',
        },
        {
          icon: <Package size={16} />,
          text: 'Importer le catalogue CSV et créer automatiquement les références manquantes',
        },
        {
      ...
      illustration={<StocksIllustration />}
      onNext={onNext}
      onClose={onClose}
    />
  );
...
function StocksCatalogStep({
  ...
  const makeLine = (): StocksCatalogDraftLine => ({
    id: randomLocalId(),
    name: '',
    unitId: units[0]?.id ?? '',
  ...
  async function createQuickSupplier() {
    ...
    try {
      ...
      if (created?.id) {
        ...
        setLines((current) =>
          current.map((line) => ({ ...line, supplierId: line.supplierId || created.id })),
        );
      }
      setSupplierName('');
    } catch (err) {
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', minHeight: 0 }}>
        <div>
          ...
          >
            Créez quelques produits réellement utilisés. Vous pourrez compléter le catalogue plus
            tard ou créer des produits manquants depuis une facture OCR.
          </p>
        </div>
        {error ? (
          ...
          <input
            ...
            placeholder={
              localSuppliers.length
                ? 'Ajouter un fournisseur rapide (ex. Transgourmet)'
          ...
          {lines.map((line, index) => (
            <div
              ...
              style={{
                display: 'grid',
                gridTemplateColumns:
                  '32px minmax(180px, 2fr) minmax(100px, 1fr) minmax(120px, 1.2fr) minmax(120px, 1.2fr) minmax(90px, 0.9fr) minmax(90px, 0.9fr) 40px',
                gap: '0.5rem',
                alignItems: 'center',
                background: '#f8fafc',
      ...
      >
        ...
        <div className="row-actions" style={{ display: 'flex', gap: '0.75rem' }}>
          ...
          <button
            type="button"
            className="btn btn-primary"
            disabled={submitting || !lines.some((line) => line.name.trim() && line.unitId)}
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        <div>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-main)', margin: 0 }}>
            Importer vos produits ou analyser un document
          </h2>
          <p
            style={{
              color: 'var(--text-muted)',
          ...
          >
            Ajoutez votre catalogue via CSV ou Excel, ou déposez un bon de commande, une facture ou
            un bon de livraison. L’import catalogue ne crée aucun stock ; seul le parcours de
            réception validé crée des mouvements.
          </p>
        </div>
        {!ocrConfigured ? (
          <div
          ...
          >
            <span>
              <AlertCircle size={16} /> Ajoutez une clé Mistral pour utiliser l’import OCR Stocks.
            </span>
            <button type="button" className="btn btn-secondary btn-sm" onClick={onOpenApiKeys}>
              <KeyRound size={13} /> Configurer
            </button>
          </div>
        ) : null}
        ...
        <div className="onboarding-options-grid">
          <div className="onboarding-option-card blue" onClick={onImportCsv}>
            ...
            <div className="onboarding-option-content">
              ...
              <span className="onboarding-option-desc">
                CSV, Excel ou historique fournisseur : l’assistant consolide le catalogue puis
                revient ici.
              </span>
            </div>
          </div>
          ...
          <div className="onboarding-option-card orange" onClick={onImportCreator}>
            ...
            <div className="onboarding-option-content">
              ...
              <span className="onboarding-option-desc">
                Déposez vos factures, BL ou fiches fournisseur : l’IA prépare votre catalogue
                produits.
              </span>
            </div>
          </div>
          ...
          <div className="onboarding-option-card emerald" onClick={onImportOcr}>
            ...
            <div className="onboarding-option-content">
              ...
              <span className="onboarding-option-desc">
                Facture, BL ou commande : l'IA lit le document et crée la réception en brouillon.
              </span>
            </div>
          </div>
          ...
          <div className="onboarding-option-card" onClick={onCreateMovement}>
            ...
            <div className="onboarding-option-content">
              ...
              <span className="onboarding-option-desc">
                Alternative ponctuelle rapide si aucun document physique n’est disponible.
              </span>
            </div>
          </div>
        </div>
        {ocrStatuses.length ? (
          <StocksOcrDashboardStatusBar
            statuses={ocrStatuses}
            onOpenExtraction={async () => undefined}
            onImportOcr={onImportOcr}
          />
        ) : null}
      </div>
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        <div>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-main)', margin: 0 }}>
            {readiness.progress === 100 ? 'Stocks est prêt à exploiter' : 'Résumé de configuration'}
          </h2>
          <p
            style={{
              color: 'var(--text-muted)',
          ...
          >
            Votre configuration reste modifiable depuis les onglets Stocks. Les prochains imports
            facture/BL enrichiront le catalogue et l’historique.
          </p>
        </div>
        <div
          style={{
      ...
      >
        ...
        <div className="row-actions" style={{ display: 'flex', gap: '0.75rem' }}>
          ...
          <button
            type="button"
            className="btn btn-primary"
            onClick={readiness.flowReady ? onOpenStocks : onClose}
            style={{ borderRadius: '10px', padding: '0.5rem 1.5rem' }}
          >
            {readiness.flowReady ? 'Voir stocks' : 'Terminer'}
          </button>
        </div>
      </div>
    </div>
...
function ProductCsvCreator({
  ...
  const patch = (index: number, key: keyof ProductImportPreviewFields, value: string) =>
    setRows((current) =>
      current.map((row, i) =>
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const combined = [
        ...rows.filter((row) => String(row.fields.name ?? '').trim()),
        ...result.preview.rows,
      ];
      setProgress(100);
      await new Promise((r) => setTimeout(r, 220));
      ...
      <div className="modal-card product-csv-creator-modal">
        ...
        <div className="product-csv-header">
          <div>
            ...
            <p style={{ margin: 0, color: 'var(--text-muted)' }}>
              Ajoutez des lignes, ou déposez vos catalogues, fiches et étiquettes. Rien n’est créé
              avant validation.
            </p>
          </div>
        ...
        {error ? (
          <div className="product-csv-toast error">
            <AlertCircle size={14} />
            <span>{error}</span>
            <button className="toast-close-btn" onClick={() => setError(undefined)}>
              ×
            </button>
          </div>
        ) : null}
        {message ? (
          <div className="product-csv-toast success">
            ...
            <span>{message}</span>
            <button className="toast-close-btn" onClick={() => setMessage(undefined)}>
              ×
            </button>
          </div>
        ) : null}
        ...
        <div className="product-csv-actions-row">
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            {files.length > 0 && (
              <button
                className="btn btn-secondary btn-sm-premium"
                disabled={busy !== null}
            ...
            >
              <ShieldCheck size={15} />{' '}
              {busy === 'preview' ? 'Vérification…' : 'Vérifier les lignes'}
            </button>
              ...
              {rows.map((row, index) => {
                ...
                return (
                  <tr
                    ...
                    className={`${hasError ? 'row-has-error' : ''} ${hasWarning ? 'row-has-warning' : ''}`}
                  >
        ...
        <div
          className="hr-catalog-actions sticky"
          style={{ marginTop: '0.5rem', borderTop: '1px solid #e8edf3', paddingTop: '1rem' }}
        >
          ...
          <div className="row-actions">
            ...
            <button
              className="btn btn-primary btn-sm-premium"
              disabled={busy !== null || !rows.some((row) => row.selected)}
              onClick={() => void commit()}
            >
...
function AddImportChooser({
  ...
  return (
    ...
    >
      ...
      >
        Choisissez votre source. Le catalogue peut contenir des produits sans stock ; seuls les
        réceptions et mouvements modifient le stock physique.
      </p>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
          ...
          <small style={{ fontSize: '0.78rem', color: '#64748b', lineHeight: 1.3 }}>
            Ajouter une référence au catalogue, sans créer de stock.
          </small>
        </div>
      </button>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
          ...
          <small style={{ fontSize: '0.78rem', color: '#64748b', lineHeight: 1.3 }}>
            Importer une base produit ou un historique fournisseur, avec consolidation avant
            création.
          </small>
        </div>
      </button>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
          <span style={{ fontSize: '0.92rem', fontWeight: 800, color: '#0f172a' }}>
            Créer un CSV produits
          </span>
          <small style={{ fontSize: '0.78rem', color: '#64748b', lineHeight: 1.3 }}>
            Saisir des lignes ou lire des photos de catalogue avec l’IA.
          </small>
        </div>
      </button>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
          <span style={{ fontSize: '0.95rem', fontWeight: 850, color: '#ffffff' }}>
            Analyser un document OCR
          </span>
          <small
          ...
          <div className="product-import-shell">
            ...
            <main className="product-import-main">
              <div className="product-import-topbar">
                <div>
                  <span className="badge badge-reception">
                    Étape {currentIndex + 1} / {PRODUCT_IMPORT_STEPS.length}
                  </span>
                  <div className="progress-bar-bg">
                ...
                <button
                  type="button"
                  className="product-import-close"
                  onClick={onClose}
                  aria-label="Fermer"
                >
                  <X size={20} />
              ...
              {error ? (
                <div className="alert-modern error product-import-error">
                  <AlertCircle size={16} /> {error}
                </div>
              ) : null}
              <AnimatePresence mode="wait">
                <motion.div
                  key={step}
                  className="product-import-panel"
...
function ProductImportWelcome({ onClose, onNext }: { onClose: () => void; onNext: () => void }) {
  return (
    <div className="product-import-welcome">
      <button
        type="button"
        className="product-import-close welcome"
        onClick={onClose}
        aria-label="Fermer"
      >
        <X size={20} />
      ...
      <div>
        <span className="badge badge-reception product-import-badge">
          <Sparkles size={14} color="#10b981" /> Configuration Guidée
        </span>
        <h1>
          Bienvenue sur l'importation de <span>produits</span>
        </h1>
        <p>
          Ajoutez vos produits depuis un fichier CSV ou Excel existant. L’assistant lit la
          structure, consolide les historiques fournisseur compatibles et vous laisse tout vérifier
          avant la création.
        </p>
        <div className="product-import-bullets">
          <span>
            <Download size={16} /> Utiliser un CSV ou un classeur Excel
          </span>
          <span>
            <UploadCloud size={16} /> Importer votre fichier produit
          </span>
          <span>
            <CheckCircle2 size={16} /> Vérifier avant création
          </span>
        </div>
  ...
  return (
    <>
      <div className="product-import-rail-content">
        <div className="product-import-brand">
          <Package size={28} /> TOQUE<span>HUB</span> STOCKS
        </div>
        <div>
      ...
      <div className="product-import-rail-card">
        ...
        <strong>
          {result
            ? `${result.created} produit(s) créé(s)`
            : summary
              ? `${summary.selected} ligne(s) sélectionnée(s)`
...
function ProductImportStructureStep({
  onBack,
  onNext,
  onDownload,
  onCreateFromDocuments,
  ...
  return (
    <div className="product-import-step">
      <div className="product-import-copy">
        ...
        <p>
          Un CSV ou un classeur Excel classique doit contenir au minimum un nom de produit et une
          unité. Les historiques fournisseur reconnus peuvent aussi permettre de calculer
          automatiquement l’unité et le prix moyen.
        </p>
      </div>
      ...
      <div className="alert-modern">
        <Info size={16} /> Formats acceptés : CSV et Excel .xlsx. Depuis Numbers, exportez en Excel
        ou en CSV.
      </div>
...
function ProductImportUploadStep({
  onBack,
  onAnalyze,
  busy,
}: {
  ...
  return (
    <div className="product-import-step">
      <div className="product-import-copy">
        ...
        <p>
          Déposez votre CSV ou votre fichier Excel. L’assistant lit directement les cellules,
          prépare les correspondances et signale les produits à vérifier.
        </p>
      </div>
      <label
      ...
      {file ? (
        <div className="stocks-ocr-file-row product-import-file-row">
          ...
          <button type="button" className="stocks-ocr-file-remove" onClick={() => setFile(null)}>
            <X size={14} />
          </button>
        </div>
      ) : null}
...
function ProductImportMappingStep({
  preview,
  onBack,
  onNext,
  onUploadAgain,
    ...
    return (
      <div className="product-import-step">
        <EmptyMini
          ...
          text="Importez un CSV avant d’associer les colonnes."
        />
        <div className="hr-catalog-actions sticky product-import-footer">
          <button type="button" className="btn btn-secondary" onClick={onBack}>
            Retour
          </button>
          <button type="button" className="btn btn-primary" onClick={onUploadAgain}>
            Importer
          </button>
        </div>
      </div>
  ...
  return (
    <div className="product-import-step">
      <div className="product-import-copy">
        ...
        <p>
          Les colonnes reconnues sont prêtes
          {preview.sourceSheet ? ` depuis la feuille « ${preview.sourceSheet} »` : ''}. Les colonnes
          non reconnues restent ignorées pour éviter de créer des informations incorrectes.
        </p>
      </div>
      {preview.processingNotes?.length ? (
        <div className="alert-modern" style={{ margin: 0, alignItems: 'flex-start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
            {preview.processingNotes.map((note) => (
      ...
      {unmapped.length ? (
        <div className="alert-modern" style={{ margin: 0 }}>
          <Info size={16} /> Colonnes ignorées : {unmapped.slice(0, 6).join(', ')}
          {unmapped.length > 6 ? '…' : ''}
        </div>
      ) : null}
      {preview.ai?.warnings?.length ? (
        <div className="alert-modern">
          <Info size={16} /> {preview.ai.warnings[0]}
        </div>
      ) : null}
      <div className="hr-catalog-actions sticky product-import-footer">
        <button type="button" className="btn btn-secondary" onClick={onBack}>
...
function ProductImportReviewStep({
  ...
  if (!preview || !summary) {
    return (
      <div className="product-import-step">
        <EmptyMini
          title="Aucune prévisualisation"
          text="Analysez un fichier produit avant de valider."
        />
        <div className="hr-catalog-actions sticky product-import-footer">
          <button type="button" className="btn btn-secondary" onClick={onBack}>
            Retour
  ...
  return (
    <div className="product-import-step">
      <div className="product-import-copy">
        ...
        <p>
          Sélectionnez les lignes à importer. Les doublons et les lignes en erreur ne sont pas
          créés.
        </p>
      </div>
      ...
      <div className="product-import-options">
        ...
        <label>
          <input
            ...
            onChange={(event) =>
              onOptionsChange({ ...options, createMissingCategories: event.target.checked })
            }
          />{' '}
        ...
        <label>
          <input
            ...
            onChange={(event) =>
              onOptionsChange({ ...options, createMissingSuppliers: event.target.checked })
            }
          />{' '}
              ...
              return (
                <tr key={row.rowNumber}>
                  <td>
                    <input
                      type="checkbox"
                      checked={Boolean(row.selected && selectable)}
                      disabled={!selectable}
                      onChange={(event) => onPatchRow(row.rowNumber, event.target.checked)}
                    />
                  </td>
                  <td>{row.rowNumber}</td>
                  <td>
                    <span className={`product-import-status ${row.status}`}>
                      {productImportStatusLabel(row.status)}
                    </span>
                  </td>
                  ...
                  <td
                    style={{
                      textAlign: 'right',
                      fontWeight: row.fields.averagePrice != null ? 700 : 400,
                    }}
                  >
      ...
      {invalidCount ? (
        <div className="alert-modern">
          <Info size={16} /> {invalidCount} ligne(s) seront ignorée(s) tant qu’elles restent en
          doublon ou en erreur.
        </div>
      ) : null}
      <div className="hr-catalog-actions sticky product-import-footer">
        <button type="button" className="btn btn-secondary" onClick={onBack}>
...
function ProductImportDoneStep({
  result,
  onClose,
  onRestart,
}: {
        ...
        <div className="product-import-sheet-grid">
          {rows.flatMap((row, rowIndex) =>
            row.map((cell, cellIndex) => (
              <span key={`${rowIndex}-${cellIndex}`} className={rowIndex === 0 ? 'header' : ''}>
                {cell}
              </span>
            )),
          )}
...
function summarizeProductImportRows(rows: ProductImportPreviewRow[]) {
  return rows.reduce<Record<ProductImportStatus | 'total' | 'selected', number>>(
    (acc, row) => {
      acc.total += 1;
      acc[row.status] += 1;
  ...
  return (
    <div className="hr-tabs stocks-module-tabs" role="tablist" aria-label="Navigation Stocks">
      {STOCKS_NAV_TABS.map((item) => {
        const isActive =
          item.tab === activeTab || (item.tab === 'categories' && isStocksSettingsRoute(activeTab));
        return (
          <button
            key={item.tab}
            type="button"
    ...
    const timer = window.setTimeout(
      async () => {
        ...
        try {
          ...
          if (active)
            setQueryError(
              err instanceof Error ? err.message : 'Chargement des produits impossible.',
            );
        } finally {
    ...
    <div className="stocks-dashboard-grid articles-page">
      ...
      <div className="stocks-metrics-strip">
        ...
        <div
          ...
          title="Filtrer : Seuils à surveiller"
        >
          <div className="metric-icon-wrapper">
      ...
      <div className="articles-page-layout">
        <div className="articles-table-card">
          ...
          {queryError ? (
            <div className="alert-modern error" style={{ margin: '0 1.25rem 1rem' }}>
              <AlertCircle size={16} /> {queryError}
            </div>
          ) : null}
          <div
            className="table-wrapper"
            style={{ opacity: loading ? 0.62 : 1, transition: 'opacity 0.15s' }}
            aria-busy={loading}
          >
                  ...
                  <tr>
                    <td colSpan={8}>
                      <EmptyMini
                        ...
                        text="Les produits sans stock sont inclus dans cette vue."
                        icon="📦"
                      />
                    </td>
                  </tr>
          ...
          >
            <span style={{ color: 'var(--text-muted)', fontSize: '0.82rem' }}>
              {totalFiltered
                ? `${(page - 1) * 25 + 1}–${Math.min(page * 25, totalFiltered)} sur ${totalFiltered} produit${totalFiltered > 1 ? 's' : ''}`
                : 'Aucun produit'}
            </span>
            ...
            {article.lots?.length ? (
              <div className="article-detail-list">
                {article.lots.map((lot, index) => (
                  <div
                    className="article-detail-row-premium lot-row"
                    key={`${lot.lotNumber ?? 'lot'}-${index}`}
                  >
                    <div className="lot-info">
                      <span className="lot-number">{lot.lotNumber || 'Lot sans numéro'}</span>
                      <span className="lot-subtext">{lot.siteName || 'Tous sites'}</span>
                    </div>
            ...
            {article.lastMovement ? (
              <div className="article-detail-row-premium">
                <span className="row-label">
                  {movementLabels[article.lastMovement.type] || article.lastMovement.type}
                </span>
                <strong className="row-value">
                  {new Date(
      ...
      <div className="double-panel">
        ...
        >
          <div className="card-title-container">
            ...
            <button className="btn btn-secondary btn-sm" onClick={onOpenStocks}>
              Voir stocks
            </button>
          </div>
          <MiniMovements movements={recentMovements} />
        ...
        >
          <span className="card-title">
            <TrendingUp size={18} /> Produits les plus consommés
          </span>
          <div className="progress-list" style={{ marginTop: '1rem' }}>
            {topConsumed.length ? (
              topConsumed.map((p) => (
              ...
              <EmptyMini
                title="Aucune consommation"
                text="Les sorties, pertes, productions et corrections négatives alimenteront ce classement."
                icon="📈"
              />
            )}
          </div>
...
function StocksOcrDashboardStatusBar({
  statuses,
  onOpenExtraction,
  onImportOcr,
  onDismiss,
  ...
  const featured =
    statuses.find(isOcrStatusReady) ?? statuses.find(isOcrStatusWorking) ?? statuses[0];
  const progressClass = isOcrStatusError(featured)
    ? 'error'
    : isOcrStatusReady(featured)
      ? 'success'
      : isOcrStatusAnalyzing(featured)
        ? 'analyzing'
        : isOcrStatusPending(featured)
          ? 'pending'
          : 'uploading';
  ...
  return (
    ...
    >
      ...
      <div
        className="stocks-ocr-dashboard-status-actions"
        style={onDismiss ? { marginRight: '1.25rem' } : undefined}
      >
        {featured.extraction ? (
          <button
            className="btn btn-primary btn-sm"
            onClick={() => void onOpenExtraction(featured.extraction!.id)}
          >
...
function StocksMarginsPage({
  ...
  const [subTab, setSubTab] = useState<
    'dashboard' | 'analytics' | 'optimization' | 'alerts' | 'reports'
  >('dashboard');
  ...
  useEffect(() => {
    void load();
  }, [token, period, productId, supplierId, categoryId, search]);
  ...
  return (
    <div className="stocks-dashboard-grid margins-dashboard">
      ...
      >
        ...
        <p className="welcome-desc">
          Centre de pilotage des achats, fournisseurs, coûts matières et variations de prix,
          consolidé uniquement depuis les réceptions validées.
        </p>
        <div className="stocks-reception-actions">
          {subTab === 'reports' && (
            <button
      ...
      <div className="margins-subtabs-nav">
        ...
        <button
          className={`subtab-btn ${subTab === 'analytics' ? 'active' : ''}`}
          onClick={() => setSubTab('analytics')}
        >
          <Search size={16} /> Fiches Achat
        </button>
        <button
          className={`subtab-btn ${subTab === 'optimization' ? 'active' : ''}`}
          onClick={() => setSubTab('optimization')}
        >
          <Scale size={16} /> Optimisations & Marché
        </button>
        <button
          className={`subtab-btn ${subTab === 'alerts' ? 'active' : ''}`}
          onClick={() => setSubTab('alerts')}
        >
          <AlertCircle size={16} /> Alertes et Seuils{' '}
          {kpis?.openAlerts ? (
            <span
              className="badge badge-correction"
              style={{
                marginLeft: '4px',
        ...
        <button
          className={`subtab-btn ${subTab === 'reports' ? 'active' : ''}`}
        ...
        <div className="filter-bar">
          {subTab === 'analytics' && (
            <div className="search-input-wrapper">
              ...
              <input
                className="search-input"
                placeholder="Rechercher produit, facture, lot, fournisseur..."
                value={search}
          ...
          <div className="filter-select-wrapper">
            <select value={supplierId} onChange={(event) => setSupplierId(event.target.value)}>
              <option value="">Tous fournisseurs</option>
              {suppliers
                .filter((supplier) => !isArchived(supplier))
                .map((supplier) => (
                  <option key={supplier.id} value={supplier.id}>
                    {supplier.name}
                  </option>
        ...
        <div className="margins-dashboard-container">
          <div className="metrics-grid">
            <Metric
              ...
              value={loading ? '...' : formatCurrency(kpis?.monthlyPurchases)}
              label="Achats du mois"
              tone="blue"
            />
            <Metric
              icon={<Calculator size={20} />}
              value={loading ? '...' : formatCurrency(kpis?.materialCost)}
              label="Coût matière théorique"
              tone="emerald"
            />
            <Metric
              icon={<FileText size={20} />}
              value={loading ? '...' : (kpis?.invoiceCount ?? 0)}
              label="Factures reçues"
              tone="purple"
            />
            <Metric
              icon={<UsersRound size={20} />}
              value={loading ? '...' : (kpis?.supplierCount ?? 0)}
              label="Fournisseurs actifs"
              tone="orange"
            />
          </div>
          <div className="metrics-grid">
            <Metric
              icon={<TrendingUp size={20} />}
              value={loading ? '...' : formatPct(kpis?.averageIncreasePct)}
              label="Hausse moyenne"
              tone="orange"
            />
            <Metric
              icon={<Scale size={20} />}
              value={loading ? '...' : formatCurrency(kpis?.potentialSavings)}
              label="Économies potentielles"
              tone="emerald"
            />
            <Metric
              icon={<LineChart size={20} />}
              value={loading ? '...' : formatPct(kpis?.monthlyEvolutionPct)}
              label="Évolution mensuelle"
              tone="blue"
            />
            <Metric
              icon={<AlertCircle size={20} />}
              value={loading ? '...' : (kpis?.openAlerts ?? 0)}
          ...
          <div className="double-panel">
            <section className="card-modern widget-card-modern">
              <span className="card-title">
                <LineChart size={18} /> Évolution des achats
              </span>
              <MarginsLineChart data={purchases} />
            </section>
            <section className="card-modern widget-card-modern">
              <span className="card-title">
                <Layers size={18} /> Répartition familles produits
              </span>
          ...
          <div className="double-panel">
            <section className="card-modern widget-card-modern">
              <span className="card-title">
                <ShoppingBag size={18} /> Top produits achetés
              </span>
              <MarginsBarList data={topProducts} />
            </section>
            <section className="card-modern widget-card-modern">
              <span className="card-title">
                <UsersRound size={18} /> Top fournisseurs
              </span>
          ...
          <section className="card-modern widget-card-modern">
            <span className="card-title">
              <Sparkles size={18} /> Suggestions d'optimisation IA
            </span>
            <div
              style={{
                display: 'grid',
            ...
            >
              {dashboard?.suggestions?.length ? (
                ...
                <EmptyMini
                  ...
                  text="Les recommandations apparaîtront avec davantage d'historique d'achats."
                  icon="✨"
                />
              )}
            </div>
      ...
      {subTab === 'analytics' && (
        <div className="margins-dashboard-container">
          {search.trim() && searchResults ? (
            <section className="card-modern widget-card-modern">
              <span className="card-title">
                <Search size={18} /> Résultats de recherche globale
              </span>
              ...
              <div className="double-panel" style={{ marginTop: '2rem' }}>
                <div>
                  ...
                  >
                    <Package size={16} /> Produits ({searchResults.products.length})
                  </h4>
                  <div className="table-wrapper">
                    <table className="table-modern">
                      <thead>
                ...
                <div>
                  ...
                  >
                    <UsersRound size={16} /> Fournisseurs ({searchResults.suppliers.length})
                  </h4>
                  <div className="table-wrapper">
                    <table className="table-modern">
                      <thead>
            ...
            !supplierId && (
              <div className="margins-search-placeholder">
                <div className="margins-search-icon-circle">
                  <Search size={32} />
                </div>
                <h2>Analyses & Fiches Achat</h2>
                <p>
                  Sélectionnez un produit ou un fournisseur dans les listes déroulantes ci-dessus,
                  ou tapez une recherche globale pour analyser l'historique de vos achats.
                </p>
              </div>
            )
          )}
          ...
          {(productDetail || supplierDetail) && (
            <div className="double-panel">
              {productDetail && (
                <section className="card-modern widget-card-modern">
                  <span className="card-title">
                    <Package size={18} /> Fiche achat produit : {productDetail.product.name}
                  </span>
                  <div
                    className="metrics-grid"
                    style={{ gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', marginTop: '1rem' }}
                  >
                    <Metric
                      icon={<Calculator size={18} />}
                      value={formatCurrency(Number(productDetail.stats.averagePrice ?? 0))}
                      label="Prix moyen"
                      tone="blue"
                    />
                    <Metric
                      icon={<Clock size={18} />}
                      value={formatCurrency(Number(productDetail.stats.lastPrice ?? 0))}
                      label="Dernier prix"
                      tone="emerald"
                    />
                    <Metric
                      icon={<TrendingUp size={18} />}
                      value={formatCurrency(Number(productDetail.stats.maxPrice ?? 0))}
                      label="Prix max"
                      tone="orange"
                    />
                    <Metric
                      icon={<Scale size={18} />}
                      value={formatCurrency(Number(productDetail.stats.minPrice ?? 0))}
                      label="Prix min"
                      tone="purple"
                    />
                  </div>
                  ...
                  {productDetail.rnmComparison && (
                    <div className="settings-list" style={{ marginTop: '1rem' }}>
                      ...
                      >
                        ...
                        <div>
                          <strong>
                            Comparaison RNM : {productDetail.rnmComparison.rnmProductName}
                          </strong>
                          <span
                            style={{
                              display: 'block',
                          ...
                          >
                            {productDetail.rnmComparison.message}
                          </span>
                          <small>
                            Payé {formatCurrency(productDetail.rnmComparison.paidPrice)} · Cours RNM{' '}
                            {formatCurrency(productDetail.rnmComparison.rnmPrice)} · écart{' '}
                            {formatPct(productDetail.rnmComparison.gapPct)}
                          </small>
                        </div>
                      </div>
                    </div>
              ...
              {supplierDetail && (
                <section className="card-modern widget-card-modern">
                  <span className="card-title">
                    <UsersRound size={18} /> Fiche fournisseur : {supplierDetail.supplier.name}
                  </span>
                  <div
                    className="metrics-grid"
                    style={{ gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', marginTop: '1rem' }}
                  >
                    <Metric
                      ...
                      value={`${supplierDetail.score}/100`}
                      label="Score global"
                      tone="emerald"
                    />
                    <Metric
                      icon={<FileText size={18} />}
                      value={Number(supplierDetail.stats.orderCount ?? 0)}
                      label="Factures reçues"
                      tone="blue"
                    />
                    <Metric
                      icon={<ShoppingCart size={18} />}
                      value={formatCurrency(Number(supplierDetail.stats.monthlyAmount ?? 0))}
                      label="Volume mensuel"
                      tone="purple"
                    />
                    <Metric
                      icon={<Package size={18} />}
                      value={Number(supplierDetail.stats.productsCount ?? 0)}
      ...
      {subTab === 'optimization' && (
        <div className="margins-dashboard-container">
          <div className="double-panel">
            <section className="card-modern widget-card-modern">
              <span className="card-title">
                <CalendarDays size={18} /> Prévision des achats & Risques de rupture
              </span>
                      ...
                      <tr key={forecast.productId}>
                        <td>
                          ...
                          <small style={{ color: 'var(--text-muted)' }}>
                            {forecast.supplierName ?? 'Fournisseur à désigner'}
                          </small>
                        </td>
                        <td>
                          {forecast.daysUntilRupture == null ? (
                            ...
                            >
                              Imminente ({Math.ceil(forecast.daysUntilRupture)} j)
                            </span>
                          ) : (
                            <span
                              className="badge badge-production"
                            ...
                            >
                              {Math.ceil(forecast.daysUntilRupture)} jours
                            </span>
                          )}
                        </td>
                        <td style={{ fontWeight: 600 }}>
                        ...
                        <td style={{ fontWeight: 700 }}>
                          {formatCurrency(forecast.estimatedBudget)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              {!forecasts.length && (
                <EmptyMini
                  title="Aucune rupture estimée"
                  text="Les mouvements récents n'indiquent aucun risque de rupture imminent."
                  icon="📅"
                />
              )}
            </section>
            ...
            <section className="card-modern widget-card-modern">
              <span className="card-title">
                <Scale size={18} /> Croisement des Prix de Marché (RNM)
              </span>
              <div className="settings-list" style={{ marginTop: '1rem' }}>
                {rnmComparisons.length ? (
                  rnmComparisons.map((item) => (
                    <div
                      key={item.productId}
                      className="suggestion-card-premium"
                      style={{ marginBottom: '0.75rem', gap: '0.75rem', alignItems: 'center' }}
                    >
                      <div style={{ color: item.gapPct > 0 ? 'var(--danger)' : 'var(--success)' }}>
                        <TrendingUp size={20} />
                      </div>
                      <div style={{ flexGrow: 1 }}>
                        ...
                        >
                          ...
                          <span
                            className={`badge ${item.gapPct > 0 ? 'badge-correction' : 'badge-production'}`}
                            style={
                              item.gapPct > 0
                                ? {
                                    background: 'var(--danger-bg)',
                        ...
                        >
                          Payé : <strong>{formatCurrency(item.paidPrice)}</strong> · Cours RNM :{' '}
                          <strong>{formatCurrency(item.rnmPrice)}</strong>
                        </p>
                  ...
                  <EmptyMini
                    title="Aucun cours de comparaison"
                    text="Installez le module Cours des Produits pour analyser vos écarts par rapport au marché."
                    icon="≋"
                  />
                )}
              </div>
          ...
          <section className="card-modern widget-card-modern">
            <span className="card-title">
              <Calculator size={18} /> Impact sur le coût matière des Recettes (Fiches Techniques)
            </span>
            <div className="metrics-grid" style={{ marginTop: '1.25rem' }}>
              <Metric
                icon={<Utensils size={18} />}
                value={sheetImpact?.impactedRecipesCount ?? 0}
                label="Recettes concernées"
                tone="blue"
              />
              <Metric
                icon={<Calculator size={18} />}
                value={formatCurrency(sheetImpact?.totalMaterialCost)}
                label="Coût matière global"
                tone="emerald"
              />
              <Metric
                icon={<TrendingUp size={18} />}
                value={formatCurrency(sheetImpact?.totalDelta)}
                label="Variation totale"
                tone="orange"
              />
              <Metric
                icon={<Calendar size={18} />}
                value={formatCurrency((sheetImpact?.totalDelta ?? 0) * 52)}
            ...
            <div className="table-wrapper" style={{ marginTop: '1.5rem' }}>
              <table className="table-modern">
                <thead>
                  <tr>
                    <th>Recette</th>
                    <th>Ingrédient concerné</th>
                    <th>Ancien coût portion</th>
                    <th>Nouveau coût portion</th>
                    <th>Impact annuel</th>
                  </tr>
                </thead>
                <tbody>
                  {(sheetImpact?.impacts ?? []).map((impact) => (
                    <tr key={`${impact.technicalSheetId}-${impact.productId}`}>
                      ...
                      <td
                        style={{
                          fontWeight: 700,
                          color: impact.annualImpact > 0 ? 'var(--danger)' : 'var(--success)',
                        }}
                      >
                        {impact.annualImpact > 0 ? '+' : ''}
                        {formatCurrency(impact.annualImpact)}
                      </td>
                    </tr>
                  ))}
                  {!(sheetImpact?.impacts ?? []).length && (
                    <tr>
                      <td colSpan={5} style={{ textAlign: 'center', color: 'var(--text-muted)' }}>
                        Aucune variation de coût détectée sur les fiches techniques.
                      </td>
                    </tr>
                  )}
                </tbody>
        ...
        <div className="margins-dashboard-container">
          <div className="double-panel">
            <section className="card-modern widget-card-modern">
              <div className="card-title-container">
                <span className="card-title">
                  <SlidersHorizontal size={18} /> Configuration des seuils de sensibilité
                </span>
                <button
                  className="btn btn-primary btn-sm"
                ...
                >
                  {savingSettings ? 'Sauvegarde...' : 'Sauvegarder les seuils'}
                </button>
              </div>
              <div className="thresholds-config-grid">
                ...
                >
                  ...
                  <input
                    ...
                    value={settings?.priceIncreaseThresholdPct ?? 10}
                    onChange={(event) =>
                      setSettings((current) =>
                        current
                          ? { ...current, priceIncreaseThresholdPct: Number(event.target.value) }
                          : current,
                      )
                    }
                  />
                  <small style={{ fontWeight: 400, color: 'var(--text-muted)' }}>
                    Signale les hausses modérées sous forme de Warning.
                  </small>
                </label>
                <label
                  style={{
                ...
                >
                  ...
                  <input
                    ...
                    max="1000"
                    style={{ padding: '0.6rem 0.8rem', borderRadius: '10px' }}
                    value={settings?.anomalyThresholdPct ?? 35}
                    onChange={(event) =>
                      setSettings((current) =>
                        current
                          ? { ...current, anomalyThresholdPct: Number(event.target.value) }
                          : current,
                      )
                    }
                  />
                  <small style={{ fontWeight: 400, color: 'var(--text-muted)' }}>
                    Déclenche une alerte Critique pour les écarts extrêmes.
                  </small>
                </label>
                <label
                  style={{
                ...
                >
                  ...
                  <input
                    ...
                    max="1000"
                    style={{ padding: '0.6rem 0.8rem', borderRadius: '10px' }}
                    value={settings?.quantityAnomalyThresholdPct ?? 60}
                    onChange={(event) =>
                      setSettings((current) =>
                        current
                          ? { ...current, quantityAnomalyThresholdPct: Number(event.target.value) }
                          : current,
                      )
                    }
                  />
                  <small style={{ fontWeight: 400, color: 'var(--text-muted)' }}>
                    Détecte les écarts importants de quantités livrées par rapport aux moyennes.
                  </small>
                </label>
              </div>
            </section>
            ...
            <section className="card-modern widget-card-modern">
              <span className="card-title">
                <AlertCircle size={18} /> Journal des alertes et anomalies ouvertes
              </span>
              <div
                className="settings-list"
                style={{ marginTop: '1.25rem', maxHeight: '480px', overflowY: 'auto' }}
              >
                {(dashboard?.alerts ?? []).length ? (
                    ...
                    >
                      <div>
                        ...
                        >
                          ...
                          <span
                            className={`badge ${alert.severity === 'CRITICAL' ? 'badge-correction' : 'badge-production'}`}
                            style={
                              alert.severity === 'CRITICAL'
                                ? {
                                    background: 'var(--danger-bg)',
                        ...
                        <small style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                          ...
                          {alert.supplier?.name ?? 'Fournisseur'}{' '}
                          {alert.variationPct != null
                            ? `· Écart : ${formatPct(alert.variationPct)}`
                            : ''}
                        </small>
                      </div>
                    </div>
                  ...
                  <EmptyMini
                    ...
                    text="Toutes les factures et réceptions récentes respectent vos seuils de tolérance."
                    icon="⚑"
                  />
                )}
              </div>
      ...
      {subTab === 'reports' && (
        <div className="margins-dashboard-container">
          <section className="card-modern widget-card-modern">
            <div className="card-title-container">
              <span className="card-title">
                <FileText size={18} /> Synthèses financières historisées
              </span>
              <button
                className="btn btn-primary"
                onClick={generateReport}
              ...
              >
                <Sparkles size={16} />{' '}
                {generatingReport ? 'Génération...' : 'Générer une synthèse périodique'}
              </button>
            </div>
            <div
              style={{
            ...
            >
              {(dashboard?.reports ?? []).length ? (
                  ...
                  >
                    ...
                    >
                      <button
                        className="btn btn-secondary btn-sm"
                        onClick={() =>
                          void api.downloadMarginReportCsv(
                            token,
                            report.id,
                ...
                <EmptyMini
                  ...
                  text="Générez un rapport pour figer l'analyse et l'exporter en CSV."
                  icon="📄"
                />
              )}
            </div>
  ...
  return (
    <div className="rnm-svg-chart-container" style={{ height: 220 }}>
      {data.length ? (
        ...
        <EmptyMini
          title="Aucune donnée"
          text="Les réceptions validées alimenteront ce graphique."
          icon="📈"
        />
      )}
    </div>
...
function MarginsBarList({ data }: { data: MarginChartPoint[] }) {
  ...
  if (!data.length)
    return (
      <EmptyMini
        title="Aucune donnée"
        text="Aucun achat validé pour les filtres sélectionnés."
        icon="📊"
      />
    );
  return (
    <div className="progress-list" style={{ marginTop: '1rem' }}>
      {data.slice(0, 8).map((item, index) => (
        <div
          ...
          key={`${item.name ?? item.productName ?? item.date}-${index}`}
        >
          <span className="progress-text-modern">
            {item.name ?? item.productName ?? item.date ?? 'Element'}
          </span>
...
function MiniMovements({ movements }: { movements: StockMovement[] }) {
  return movements.length ? (
    ...
    <EmptyMini
      ...
      text="Enregistrez une réception, sortie, perte, correction, inventaire, production ou transfert."
    />
  );
}
...
function InventoriesPage({
  ...
  return (
    <ReferencePage
      title="Inventaires"
      subtitle="Inventaires complets: comptage réel, écarts, corrections automatiques et verrouillage après validation."
      search={search}
      setSearch={setSearch}
      onCreate={onCreate}
      createLabel="Créer un inventaire"
    >
      <div className="table-wrapper">
        <table className="table-modern">
          <thead>
            ...
            {inventories.map((inventory) => {
              ...
              return (
                <tr key={inventory.id} className="clickable-row" onClick={() => onOpen(inventory)}>
                  ...
                  <td>
                    <span
                      className={`badge ${inventory.status === 'VALIDATED' ? 'badge-reception' : 'badge-inventory'}`}
                    >
                      {inventory.status === 'VALIDATED' ? 'Validé' : 'Brouillon'}
                    </span>
                  </td>
                  <td>{inventory.site?.name ?? 'Tous sites'}</td>
                  <td>{lines.length || products.length}</td>
                  <td>
                    {counted}/{lines.length || products.length}
                  </td>
                </tr>
              );
            })}
      ...
      {!inventories.length && (
        <EmptyMini
          ...
          text="Créez un inventaire complet pour charger les produits actifs et saisir les quantités comptées."
        />
      )}
    </ReferencePage>
  );
...
function InventoryDetailModal({
  ...
  const filteredLines = useMemo(() => {
    ...
    return lines.filter((line) =>
      `${line.product?.name ?? ''} ${line.product?.sku ?? ''} ${line.product?.category?.name ?? ''}`
        .toLowerCase()
        .includes(query),
    );
  }, [lines, search]);
  ...
  async function validateInventory() {
    ...
    if (
      missing > 0 &&
      !window.confirm(
        `${missing} ligne(s) n’ont pas de quantité comptée. Valider uniquement les lignes renseignées ?`,
      )
    )
      return;
    setSubmitting(true);
    ...
    >
      {inventory ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div
            className="alert-modern info"
            style={{ margin: 0, background: '#f8fafc', borderColor: '#dbe4ef', color: '#334155' }}
          >
            <ClipboardList size={16} />
            <span>
              {inventory.site?.name ?? 'Tous sites'} · {lines.length} produit(s) à compter.
            </span>
          </div>
          <div className="filter-bar" style={{ margin: 0 }}>
            <div className="search-input-wrapper" style={{ maxWidth: '100%' }}>
              ...
              <input
                ...
                value={search}
                onChange={(event) => setSearch(event.target.value)}
                placeholder="Filtrer les produits de l’inventaire..."
              />
            </div>
            {!isLocked ? (
              <button
                ...
                onClick={() =>
                  setCounts(
                    Object.fromEntries(
                      lines.map((line) => [
                        inventoryLineKey(line),
                        String(numeric(line.theoreticalQuantity)),
                      ]),
                    ),
                  )
                }
                ...
                {filteredLines.map((line) => {
                  ...
                  const variance =
                    counted === null || !Number.isFinite(counted)
                      ? numeric(line.varianceQuantity ?? line.variance)
                      : counted - theoretical;
                    ...
                    <tr key={line.id}>
                      ...
                      <td>
                        {line.product?.category?.name ?? (
                          <span style={{ color: 'var(--text-muted)' }}>Sans catégorie</span>
                        )}
                      </td>
                      <td style={{ textAlign: 'right' }}>
                        {formatStockNumber(theoretical)} {unit}
                      </td>
                      <td style={{ textAlign: 'right' }}>
                        {isLocked ? (
                          <strong>
                          ...
                          <input
                            ...
                            value={countedRaw}
                            onChange={(event) =>
                              setCounts((current) => ({ ...current, [key]: event.target.value }))
                            }
                      ...
                      >
                        {countedRaw === '' && !isLocked
                          ? '—'
                          : `${variance > 0 ? '+' : ''}${formatStockNumber(variance)} ${unit}`}
                      </td>
                    </tr>
                  );
                })}
...
function LocationsPage({
  ...
  return (
    <ReferencePage
      title="Sites"
      subtitle="Choisissez le site de stock. Les détails internes sont masqués pour la V1."
      search={search}
      setSearch={setSearch}
      showArchived={showArchived}
      setShowArchived={setShowArchived}
      onCreate={onCreateLocation}
      createLabel="Configurer un site"
      secondaryAction={
        <button className="btn btn-secondary" onClick={onCreateSite}>
          <Warehouse size={16} /> Nouveau site
        </button>
      }
    >
      <div className="apps-grid compact-grid">
        {sites.map((s) => (
          <div className="app-card compact-card" key={s.id}>
            <div className="app-card-icon">
              <Warehouse size={20} />
            </div>
            <h3>{s.name}</h3>
            <p>{isArchived(s) ? 'Archivé' : 'Actif'}</p>
          </div>
        ))}
      </div>
      <div className="table-wrapper">
        <table className="table-modern">
          ...
          <tbody>
            {sites.map((s) => (
              <tr key={s.id}>
                <td>{s.name}</td>
                <td>{isArchived(s) ? 'Archivé' : 'Actif'}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {!sites.length && (
        <EmptyMini title="Aucun site" text="Ajoutez Restaurant principal ou Cuisine centrale." />
      )}
    </ReferencePage>
  );
}
...
function AuditPage({
  ...
  return (
    <ReferencePage
      title="Audit"
      subtitle="Journal métier consultable par administrateurs et managers, exportable en CSV."
      search={search}
      setSearch={setSearch}
      onCreate={onExport}
      createLabel="Exporter CSV"
    >
      <div className="table-wrapper">
        <table className="table-modern">
          <thead>
      ...
      {!entries.length && (
        <EmptyMini
          title="Audit vide"
          text="Les créations, modifications, archivages, mouvements, inventaires et transferts seront historisés ici."
        />
      )}
    </ReferencePage>
  );
...
function ApplicationsPage({
  ...
  const featuredPrerequisiteMessage =
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="store-featured-content">
                <span className="store-featured-tag">
                  À LA UNE · {featuredApp.category.toUpperCase()}
                </span>
                <h1 className="store-featured-title">Module {featuredApp.title}</h1>
                <p className="store-featured-desc">{featuredApp.tagline}</p>
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  ...
                  {installedApps.includes(featuredApp.id) ? (
                    ...
                    <button
                      className="btn btn-secondary"
                      disabled
                      title={featuredPrerequisiteMessage}
                      style={{
        ...
        <div className="dev-switch-menu">
          ...
          {users.map((user) => (
            <button
              ...
              disabled={switching || user.id === currentUserId || user.status === 'DISABLED'}
            >
          ...
          <button onClick={onCreate} className="dev-switch-create">
...
function BackupRestorePage({
  ...
  const [cloudForm, setCloudForm] = useState({
    clientId: '',
    clientSecret: '',
    redirectUri: api.backupCloudDefaultRedirectUri(),
  });
  ...
  async function load() {
    setError(undefined);
    try {
      const [nextState, nextSchedule, nextCloudStatus] = await Promise.all([
        api.backups(token),
        api.backupSchedule(token),
        api.backupCloudStatus(token),
      ]);
      setState(nextState);
      ...
      setCloudForm((current) => ({
        ...
        redirectUri:
          current.redirectUri ||
          nextCloudStatus.googleDrive.redirectUri ||
          api.backupCloudDefaultRedirectUri(),
      }));
      ...
      if ((event.data as { type?: string } | undefined)?.type === 'toquehub:backup-cloud-google') {
        setMessage(
          (event.data as { ok?: boolean }).ok
            ...
            : 'Connexion Google Drive interrompue.',
        );
        void load();
      }
    };
  ...
  const localRestoreReady = Boolean(
    selectedBackup && normalizedConfirmationPhrase === RESTORE_CONFIRMATION_PHRASE,
  );
  const uploadRestoreReady = Boolean(
    inspection && normalizedConfirmationPhrase === RESTORE_CONFIRMATION_PHRASE,
  );
  ...
  const googleDriveStatusLabel = googleDriveConnected
    ? 'Connecté'
        ...
        <div className="settings-hero-grid">
          ...
          <div className="glass-terminal">
            <div className="glass-terminal-header">
              <span className="glass-terminal-title">backup runtime</span>
            </div>
            <div className="glass-terminal-rows">
              {(state?.tools ?? []).map((tool) => (
                <div className="glass-terminal-row" key={tool.key}>
                  <span className="label">{tool.key.toUpperCase()} :</span>
                  <span className="value" style={{ color: tool.available ? '#10b981' : '#f97316' }}>
                    <span
                      className={`status-indicator-dot ${tool.available ? 'green' : 'orange'}`}
                    ></span>
                    {tool.available ? 'DISPONIBLE' : 'MANQUANT'}
                  </span>
                </div>
              ))}
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                      ...
                      <button
                        ...
                        onClick={() =>
                          void run(
                            () => api.sendBackupToGoogleDrive(token, backup.id),
                            'Sauvegarde envoyée vers Google Drive.',
                          )
                        }
                        title={
            ...
            <div className="settings-grid-premium">
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setCloudForm((current) => ({ ...current, clientId: event.target.value }))
                  }
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          >
            ...
            >
              Votre sauvegarde cloud en 5 minutes
            </h3>
            <p className="muted" style={{ margin: 0, lineHeight: 1.65 }}>
              ToqueHub crée d'abord une archive locale, puis l'envoie dans un dossier Google Drive
              autorisé par votre compte.
            </p>
          </div>
          ...
          >
            {[
              [
                '1',
                'Créer le projet Google',
                'Dans Google Cloud Console, créez ou sélectionnez un projet dédié à ToqueHub.',
              ],
              [
                '2',
                'Activer Drive API',
                'Dans Bibliothèque API, activez Google Drive API pour ce projet.',
              ],
              [
                '3',
                'Préparer OAuth',
                'Dans Écran de consentement OAuth, renseignez le nom de l’application et ajoutez votre compte en testeur si nécessaire.',
              ],
              [
                '4',
                'Passer l’audience en production',
                'Dans Audience, publiez l’application en production. En mode test, Google peut refuser la connexion ou limiter les comptes.',
              ],
              [
                '5',
                'Créer les identifiants',
                'Créez un Client OAuth de type Application Web, puis ajoutez exactement l’URI de redirection ToqueHub.',
              ],
              [
                '6',
                'Connecter et tester',
                'Copiez Client ID et secret ici, enregistrez OAuth, connectez Google Drive, puis lancez un test.',
              ],
            ].map(([step, title, text]) => (
              <div
                key={step}
              ...
              >
                ...
                <strong
                  style={{ display: 'block', marginTop: '0.8rem', color: 'var(--text-main)' }}
                >
                  {title}
                </strong>
                <p
                  className="muted"
                  style={{ margin: '0.35rem 0 0', lineHeight: 1.55, fontSize: '0.84rem' }}
                >
                  {text}
                </p>
              </div>
          ...
          >
            <div style={{ minWidth: 0 }}>
              <strong
                style={{ display: 'block', color: 'var(--text-main)', marginBottom: '0.25rem' }}
              >
                URI de redirection à coller dans Google
              </strong>
              <code
                style={{
                  display: 'block',
              ...
              >
                {cloudForm.redirectUri}
              </code>
            </div>
            <button type="button" className="btn btn-secondary" onClick={copyCloudRedirectUri}>
              <ClipboardList size={16} /> Copier
            </button>
          </div>
          ...
          >
            <div className="alert-modern info" style={{ margin: 0, background: '#f8fafc' }}>
              <ShieldCheck size={16} />
              <span>
                Le Client secret reste côté serveur. Si le champ est laissé vide après une première
                configuration, ToqueHub conserve le secret existant.
              </span>
            </div>
            <div
              className="alert-modern info"
              style={{ margin: 0, background: '#fff7ed', borderColor: '#fed7aa', color: '#9a3412' }}
            >
              <AlertCircle size={16} />
              <span>
                La variable serveur BACKUP_CLOUD_ENCRYPTION_KEY doit être configurée avant
                l’activation cloud.
              </span>
            </div>
          </div>
          ...
          <div
            ...
            style={{ margin: '0 -1.75rem -1.75rem', padding: '1rem 1.75rem' }}
          >
            <button
              type="button"
              className="btn btn-secondary"
            ...
            <button
              ...
              className="btn btn-primary"
              onClick={() => {
                copyCloudRedirectUri();
                setCloudHelpOpen(false);
              }}
            >
...
const establishmentTypeOptions: EstablishmentType[] = [
  'Restaurant',
  'EHPAD',
  'Collectivité',
  'Hôtel',
...
function teamSizeLabel(value?: TeamSize | string | null) {
  return (
    teamSizeOptions.find((option) => option.value === value)?.label ?? value ?? 'Non renseigné'
  );
}
...
function systemUpdateOperationMessage(operation: SystemUpdateOperation) {
  ...
  if (operation.status === 'running')
    return 'Installation en cours. Gardez cette fenêtre ouverte pour suivre l’avancement.';
  if (operation.status === 'success') return 'Mise à jour terminée avec succès.';
  if (operation.status === 'rollback')
    return 'Un problème a été détecté. Rollback automatique en cours ou terminé.';
...
function systemUpdateProgress(operation: SystemUpdateOperation) {
  ...
  if (tagChanged && logs.includes('pull api web'))
    return { percent: 62, label: 'Téléchargement des images Docker' };
  if (tagChanged) return { percent: 48, label: 'Préparation de la nouvelle version' };
  if (logs.includes('backup fichiers valid'))
    return { percent: 40, label: 'Sauvegarde fichiers validée' };
  if (logs.includes('backup fichiers')) return { percent: 32, label: 'Sauvegarde des fichiers' };
  if (logs.includes('backup logique postgresql valid'))
    return { percent: 26, label: 'Sauvegarde PostgreSQL validée' };
  ...
  if (logs.includes('vérification accès images docker'))
    return { percent: 14, label: 'Vérification des images Docker' };
  if (logs.includes('ghcr: authentification docker'))
    return { percent: 13, label: 'Authentification GHCR' };
  if (logs.includes('précontrôle service'))
    return { percent: 12, label: 'Vérification des services' };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      <div className="card-modern" style={{ padding: '1.5rem' }}>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '0.65rem', flexWrap: 'wrap' }}>
            ...
            <button
              ...
              onClick={onApply}
              disabled={
                !status?.updateAvailable || !updaterAvailable || applying || operationRunning
              }
            >
              <Download size={15} />
        ...
        {status?.github.error ? (
          <div className="alert-modern error" style={{ marginBottom: '1rem' }}>
            <AlertCircle size={16} /> GitHub : {status.github.error}
          </div>
        ) : null}
        {!updaterAvailable && status ? (
          <div className="alert-modern error" style={{ marginBottom: '1rem' }}>
            <Info size={16} />
            Updater indisponible. Lancez en SSH :{' '}
            <code>
              docker compose --env-file .env.docker pull && docker compose --env-file .env.docker up
              -d
            </code>
          </div>
        ) : null}
...
function SystemUpdateProgressModal({
  ...
  const progress = operation
    ? systemUpdateProgress(operation)
    : { percent: applying ? 8 : 0, label: applying ? 'Lancement de l’opération' : 'Prêt' };
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        <div
          className={`alert-modern ${operation?.status === 'success' ? 'success' : operation?.status === 'error' || operation?.status === 'rollback' || error ? 'error' : 'info'}`}
          style={
            !operation || running
              ? { background: '#f8fafc', borderColor: '#dbe4ef', color: '#334155' }
              : undefined
        ...
        >
          ...
          <span>
            {operation ? systemUpdateOperationMessage(operation) : 'Démarrage de la mise à jour...'}
          </span>
        </div>
        ...
        <div className="settings-grid-premium">
          ...
          <div className="info-card-premium">
            <div className="info-card-premium-header">
              <span className="info-card-premium-label">Cible</span>
              <span className="info-card-premium-icon">
                <Download size={16} />
              </span>
            </div>
            <div className="info-card-premium-value">
              {operation?.targetTag ?? status?.latest?.tag ?? status?.latest?.version ?? '-'}
            </div>
          </div>
        </div>
...
function SettingsPage({
  ...
  const hrCollaboratorCount =
    dashboardSummary?.counts.hrCollaborators ?? dashboardSummary?.counts.collaborators ?? null;
  const computedTeamSize =
    typeof hrCollaboratorCount === 'number'
      ? teamSizeFromCollaboratorCount(hrCollaboratorCount)
      : organizationTeamSize;
  const teamSizeDisplay =
    typeof hrCollaboratorCount === 'number'
      ? collaboratorCountLabel(hrCollaboratorCount)
      : teamSizeLabel(organizationTeamSize);
  const teamSizeMeta =
    typeof hrCollaboratorCount === 'number'
      ? hrCollaboratorCount > 0
        ...
        : 'Synchronisé RH'
      : 'Valeur onboarding';
  const regulatoryCountryCode =
    organization?.regulatoryCountryCode ?? session.user.regulatoryCountryCode ?? null;
  ...
  const primarySite =
    activeOrganizationSites.find((site) => site.id === primarySiteId) ??
    activeOrganizationSites.find(
      (site) => site.name === (organization?.mainSiteName ?? session.user.mainSiteName),
    ) ??
  ...
  const initialConfigured =
    organization?.apiKeys?.mistral.configured ?? session.user.apiKeys?.mistral.configured ?? false;
  const initialMasked =
    organization?.apiKeys?.mistral.masked ?? session.user.apiKeys?.mistral.masked;
  const initialGithubConfigured =
    organization?.apiKeys?.github?.configured ?? session.user.apiKeys?.github?.configured ?? false;
  const initialGithubMasked =
    organization?.apiKeys?.github?.masked ?? session.user.apiKeys?.github?.masked;
  const initialResendConfigured =
    organization?.apiKeys?.resend?.configured ?? session.user.apiKeys?.resend?.configured ?? false;
  const initialResendMasked =
    organization?.apiKeys?.resend?.masked ?? session.user.apiKeys?.resend?.masked;
  ...
  const [resendKeyConfigured, setResendKeyConfigured] = useState(initialResendConfigured);
  const [resendKeyMasked, setResendKeyMasked] = useState<string | null | undefined>(
    initialResendMasked,
  );
  ...
  const [regulatoryCountryDraft, setRegulatoryCountryDraft] = useState<string>(
    regulatoryCountryCode ?? '',
  );
  ...
  const [siteCountry, setSiteCountry] = useState<'FR' | 'FI'>(
    regulatoryCountryCode === 'FI' ? 'FI' : 'FR',
  );
  ...
  useEffect(() => {
    setResendKeyConfigured(initialResendConfigured);
    setResendKeyMasked(initialResendMasked);
  }, [initialResendConfigured, initialResendMasked]);
  ...
  async function loadUpdateStatus(force: boolean) {
    ...
    try {
      const status = force
        ? await api.systemUpdateCheck(token)
        : await api.systemUpdateStatus(token);
      setUpdateStatus(status);
      setUpdateOperation(status.runtime.lastOperation);
    } catch (err) {
      setUpdateError(
        err instanceof Error ? err.message : 'Impossible de vérifier les mises à jour.',
      );
    } finally {
  ...
  async function loadRemoteAccessStatus() {
    ...
    try {
      ...
      setRemoteAccessError(
        err instanceof Error ? err.message : 'Impossible de lire l’accès à distance.',
      );
    } finally {
  ...
  async function saveApiKey() {
    ...
    try {
      const saved = await api.updateOrganizationApiKeys(token, {
        mistralApiKey: mistralKey.trim() || (apiKeyConfigured ? '' : undefined),
      });
      setApiKeyConfigured(Boolean(saved?.mistral.configured));
      setApiKeyMasked(saved?.mistral.masked);
      setMistralKey('');
      setApiKeyMessage(
        saved?.mistral.configured ? 'Clé Mistral enregistrée.' : 'Clé Mistral supprimée.',
      );
      onApiKeysSaved?.();
    } catch (err) {
  ...
  async function saveResendKey() {
    ...
    setResendKeyError(undefined);
    setResendKeyMessage(undefined);
    try {
      const saved = await api.updateOrganizationApiKeys(token, {
        resendApiKey: resendKey.trim() || (resendKeyConfigured ? '' : undefined),
      });
      setResendKeyConfigured(Boolean(saved?.resend?.configured));
      setResendKeyMasked(saved?.resend?.masked);
      setResendKey('');
      setResendKeyMessage(
        saved?.resend?.configured ? 'Clé Resend enregistrée.' : 'Clé Resend supprimée.',
      );
      onApiKeysSaved?.();
    } catch (err) {
      setResendKeyError(
        err instanceof Error ? err.message : 'Impossible d’enregistrer la clé Resend.',
      );
    } finally {
  ...
  async function saveGithubToken() {
    ...
    try {
      const saved = await api.updateOrganizationApiKeys(token, {
        githubToken: githubToken.trim() || (githubTokenConfigured ? '' : undefined),
      });
      setGithubTokenConfigured(Boolean(saved?.github?.configured));
      setGithubTokenMasked(saved?.github?.masked);
      setGithubToken('');
      setGithubTokenMessage(
        saved?.github?.configured
          ? 'Token GitHub enregistré. Vérification des releases en cours...'
          : 'Token GitHub supprimé.',
      );
      if (saved?.github?.configured) {
        ...
        setGithubTokenMessage(
          status.github.error
            ? `Token enregistré, mais GitHub répond encore : ${status.github.error}`
            : 'Token GitHub enregistré. Releases et changelog accessibles.',
        );
      }
      onSettingsSaved?.();
    } catch (err) {
      setGithubTokenError(
        err instanceof Error ? err.message : 'Impossible d’enregistrer le token GitHub.',
      );
    } finally {
  ...
  async function saveOrganizationIdentity() {
    ...
    try {
      ...
      if (editingSetting === 'establishmentType')
        payload.establishmentType = identityDraft.establishmentType || null;
  ...
  async function saveRegulatoryCountry(closeOnSuccess = false) {
    ...
    if (regulatoryCountryCode && next && next !== regulatoryCountryCode) {
      const confirmed = window.confirm(
        'Changer le pays RH peut modifier le type de solde conges utilise pour les prochains calculs.',
      );
      if (!confirmed) return;
    }
    ...
    try {
      await api.updateOrganizationRegulatoryCountry(token, {
        regulatoryCountryCode: next as 'FR' | 'FI' | null,
      });
      setRegulatoryCountryMessage(
        next ? `Pays RH enregistre : ${countryLabel(next)}.` : 'Pays RH reinitialise.',
      );
      if (closeOnSuccess) setEditingSetting(null);
      onSettingsSaved?.();
    } catch (err) {
      setRegulatoryCountryError(
        err instanceof Error ? err.message : 'Impossible d enregistrer le pays RH.',
      );
    } finally {
  ...
  const remoteStatusLabel =
    effectiveRemoteStatus === 'active'
      ...
      : effectiveRemoteStatus === 'needs_login'
        ? 'Connexion requise'
        : effectiveRemoteStatus === 'unavailable'
          ? 'Agent indisponible'
          : 'Non activé';
      ...
      >
        <div className="settings-hero-grid">
          <div className="settings-hero-left">
            ...
            >
              Console d'administration locale et cloud souveraine. Vos applications partagent la
              même organisation, les mêmes utilisateurs et les mêmes données sécurisées.
            </p>
          </div>
          <div className="glass-terminal">
            <div className="glass-terminal-header">
        ...
        <div className="settings-content">
          {activeSubTab === 'general' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              <div className="card-modern" style={{ padding: '1.5rem' }}>
                ...
                >
                  <Building2 size={18} /> Détails de l'Organisation
                </span>
                <p className="muted" style={{ fontSize: '0.85rem', marginBottom: '1.5rem' }}>
                  Ces informations définissent l'identité et la taille de votre structure ToqueHub.
                </p>
                <div className="settings-grid-premium">
                  <div
                    role="button"
                    ...
                    onClick={() => setEditingSetting('name')}
                    onKeyDown={(event) => {
                      if (event.key === 'Enter' || event.key === ' ') setEditingSetting('name');
                    }}
                  >
                    <div className="info-card-premium-header">
                      <span
                    ...
                    <div className="info-card-premium-value">{organizationName}</div>
                  </div>
                  ...
                  <div
                    ...
                    tabIndex={0}
                    className="info-card-premium clickable"
                    onClick={() => setEditingSetting('establishmentType')}
                    onKeyDown={(event) => {
                      if (event.key === 'Enter' || event.key === ' ')
                        setEditingSetting('establishmentType');
                    }}
                  >
                    <div className="info-card-premium-header">
                      <span
                  ...
                  <div className="info-card-premium">
                    ...
                    <div className="info-card-premium-value">{teamSizeDisplay}</div>
                    <span
                      className="badge badge-reception"
                      style={{ width: 'fit-content', marginTop: '0.65rem' }}
                    >
                  ...
                  <div
                    ...
                    tabIndex={0}
                    className="info-card-premium clickable"
                    onClick={() => openSiteForm(primarySite, 'Modifier le site principal')}
                    onKeyDown={(event) => {
                      if (event.key === 'Enter' || event.key === ' ')
                        openSiteForm(primarySite, 'Modifier le site principal');
                    }}
                  >
                    <div className="info-card-premium-header">
                      <span
                  ...
                  <div
                    ...
                    tabIndex={0}
                    className="info-card-premium clickable"
                    onClick={() => setEditingSetting('secondarySites')}
                    onKeyDown={(event) => {
                      if (event.key === 'Enter' || event.key === ' ')
                        setEditingSetting('secondarySites');
                    }}
                  >
                    <div className="info-card-premium-header">
                      <span
                    ...
                    <div className="info-card-premium-value">
                      {secondarySites.length
                        ? `${secondarySites.length} site${secondarySites.length > 1 ? 's' : ''}`
                        : 'Aucun'}
                    </div>
                    {secondarySites.length ? (
                      <span className="muted" style={{ marginTop: '0.45rem', fontSize: '0.8rem' }}>
                  ...
                  <div
                    ...
                    tabIndex={0}
                    className="info-card-premium clickable"
                    onClick={() => setEditingSetting('regulatoryCountry')}
                    onKeyDown={(event) => {
                      if (event.key === 'Enter' || event.key === ' ')
                        setEditingSetting('regulatoryCountry');
                    }}
                  >
                    <div className="info-card-premium-header">
                      <span
                    ...
                    <span
                      className={`badge ${regulatoryCountryCode ? 'badge-reception' : 'badge-correction'}`}
                      style={{ width: 'fit-content', marginTop: '0.65rem' }}
                    >
                ...
                {identityMessage ? (
                  <div className="alert-modern success" style={{ marginTop: '1rem' }}>
                    <CheckCircle2 size={16} /> {identityMessage}
                  </div>
                ) : null}
              ...
              <div className="card-modern" style={{ padding: '1.5rem' }}>
                ...
                >
                  ...
                  <span
                    className={`badge ${githubTokenConfigured ? 'badge-reception' : 'badge-correction'}`}
                  >
                ...
                {githubTokenMasked ? (
                  ...
                  >
                    ...
                    <span style={{ fontSize: '0.85rem' }}>
                      Token GitHub actif : <code>{githubTokenMasked}</code>
                    </span>
                  </div>
                ) : null}
                {githubTokenError ? (
                  <div className="alert-modern error" style={{ marginBottom: '1rem' }}>
                    <AlertCircle size={16} /> {githubTokenError}
                  </div>
                ) : null}
                {githubTokenMessage ? (
                  <div className="alert-modern success" style={{ marginBottom: '1rem' }}>
                    <CheckCircle2 size={16} /> {githubTokenMessage}
                  </div>
                ) : null}
            ...
            <div className="card-modern" style={{ padding: '1.5rem' }}>
              ...
              >
                <div>
                  ...
                  <p className="muted" style={{ fontSize: '0.85rem', margin: 0 }}>
                    Accès privé via Tailscale, sans ouverture de port routeur ni exposition publique
                    de ToqueHub.
                  </p>
                </div>
                <div
                ...
                >
                  ...
                  >
                    <strong style={{ color: 'var(--text-main)' }}>
                      Comment activer l’accès hors de chez vous ?
                    </strong>
                    <ol style={{ margin: 0, paddingLeft: '1.1rem' }}>
                      ...
                      <li>
                        Installez Tailscale sur le téléphone, la tablette ou l’ordinateur qui
                        servira à ouvrir ToqueHub à distance.
                      </li>
                      <li>
                        Cliquez sur <strong>Activer l’accès à distance</strong> dans ToqueHub.
                      </li>
                      <li>
                        Si le bouton <strong>Se connecter à Tailscale</strong> apparaît, ouvrez-le
                        et validez l’ajout de cette machine.
                      </li>
                      <li>
                        Revenez ici, cliquez sur <strong>Rafraîchir</strong>, puis utilisez{' '}
                        <strong>Ouvrir l’accès</strong> quand le statut passe à Actif.
                      </li>
                    </ol>
                    <span>
                      Aucun port routeur n’est à ouvrir : l’accès reste privé via votre réseau
                      Tailscale.
                    </span>
                  </div>
                </div>
              ...
              >
                <strong style={{ fontSize: '1.2rem', color: 'var(--text-main)' }}>
                  {effectiveRemoteStatus === 'active'
                    ? 'Votre accès distant est prêt'
                    : effectiveRemoteStatus === 'needs_login'
                      ? 'Connexion Tailscale à finaliser'
                      : 'Activer ToqueHub hors de chez vous'}
                </strong>
              ...
              >
                ...
                <button
                  className="btn btn-secondary"
                  onClick={() => void refreshRemoteAccess()}
                  disabled={remoteAccessBusy || remoteStatusLoading}
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                ...
                >
                  <RotateCw size={15} className={remoteStatusLoading ? 'spin' : undefined} />{' '}
                  Rafraîchir
                </button>
              </div>
            </div>
          ...
          {activeSubTab === 'users' && (
            <div className="card-modern" style={{ padding: '1.5rem' }}>
              ...
              >
                <UsersRound size={18} /> Gestion des Accès
              </span>
              <p className="muted" style={{ fontSize: '0.85rem', marginBottom: '1.5rem' }}>
                Administrez les comptes des collaborateurs accédant à votre console de gestion
                ToqueHub.
              </p>
              ...
              >
                <div style={{ flex: '1 1 300px' }}>
                  ...
                  >
                    Ajoutez de nouveaux profils collaborateurs, attribuez des rôles ou révoquez les
                    accès temporaires.
                  </span>
                </div>
                <button
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    <KeyRound size={18} /> Clés API & IA
                  </span>
                  <p className="muted" style={{ fontSize: '0.82rem', margin: '0.25rem 0 0 0' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                  ...
                  >
                    ...
                    >
                      <span
                        style={{ width: '33.3%', background: '#002395', height: '100%' }}
                      ></span>
                      <span
                        style={{ width: '33.3%', background: '#FFFFFF', height: '100%' }}
                      ></span>
                      <span
                        style={{ width: '33.3%', background: '#ED2939', height: '100%' }}
                      ></span>
                    </span>
                    Souveraineté Française
                  </span>
                  <span
                    className={`badge ${apiKeyConfigured ? 'badge-reception' : 'badge-correction'}`}
                  >
              ...
              >
                ...
                >
                  <strong>Mistral AI</strong> est le leader français de l'IA. En configurant votre
                  clé API, vous activez l'OCR intelligent de ToqueHub pour déchiffrer instantanément
                  vos factures et bons de commande. Vos données restent hébergées en France.
                </p>
              </div>
              ...
              {apiKeyMasked ? (
                ...
                >
                  ...
                  <span style={{ fontSize: '0.85rem' }}>
                    Clé API active enregistrée : <code>{apiKeyMasked}</code>
                  </span>
                </div>
              ) : null}
              {focusApiKeys ? (
                <div className="alert-modern error" style={{ marginBottom: '1.25rem' }}>
                  <Info size={16} /> Veuillez ajouter une clé API Mistral pour activer l'extraction
                  de factures.
                </div>
              ) : null}
              {apiKeyError ? (
              ...
              {apiKeyMessage ? (
                <div className="alert-modern success" style={{ marginBottom: '1.25rem' }}>
                  <CheckCircle2 size={16} /> {apiKeyMessage}
                </div>
              ) : null}
              ...
              >
                ...
                <div className="api-key-input-container">
                  <input
                    ...
                    placeholder={
                      apiKeyConfigured
                        ? 'Nouvelle clé ou laisser vide pour supprimer'
                        : 'mistral-api-key-...'
                    }
                    value={mistralKey}
              ...
              >
                {savingApiKey
                  ? 'Enregistrement…'
                  : apiKeyConfigured && !mistralKey.trim()
                    ? 'Supprimer la clé'
                    : 'Sauvegarder la clé'}
              </button>
              ...
              >
                ...
                >
                  ...
                  <span
                    className={`badge ${resendKeyConfigured ? 'badge-reception' : 'badge-correction'}`}
                  >
                    {resendKeyConfigured ? 'Resend activé' : 'Non configuré'}
                  </span>
                </div>
                ...
                >
                  ...
                  >
                    <strong>Resend</strong> transmet les commandes fournisseurs générées par
                    ToqueHub. La clé est enregistrée pour l’organisation, masquée dans l’interface
                    et n’est jamais renvoyée au navigateur.
                  </p>
                </div>
                ...
                {resendKeyMasked ? (
                  ...
                  >
                    <ShieldCheck size={16} style={{ color: '#10b981' }} />
                    <span style={{ fontSize: '0.85rem' }}>
                      Clé API active enregistrée : <code>{resendKeyMasked}</code>
                    </span>
                  </div>
                ) : null}
                {resendKeyError ? (
                  <div className="alert-modern error" style={{ marginBottom: '1.25rem' }}>
                    <AlertCircle size={16} /> {resendKeyError}
                  </div>
                ) : null}
                {resendKeyMessage ? (
                  <div className="alert-modern success" style={{ marginBottom: '1.25rem' }}>
                    <CheckCircle2 size={16} /> {resendKeyMessage}
                  </div>
                ) : null}
                ...
                >
                  {savingResendKey
                    ? 'Enregistrement…'
                    : resendKeyConfigured && !resendKey.trim()
                      ? 'Supprimer la clé'
                      : 'Sauvegarder la clé'}
                </button>
              </section>
          ...
          {activeSubTab === 'core' && (
            <div className="card-modern" style={{ padding: '1.5rem' }}>
              ...
              >
                <Server size={18} /> Diagnostic de l'Instance
              </span>
              <p className="muted" style={{ fontSize: '0.85rem', marginBottom: '1.5rem' }}>
              ...
              <div className="tech-stack-container">
                <div className="tech-stack-visual">
                  <div className="tech-stack-node active">
                    <div
                      className="tech-stack-node-icon"
      ...
      >
        {editingSetting === 'name' ? (
          ...
          >
            {identityError ? (
              <div className="alert-modern error">
                <AlertCircle size={16} /> {identityError}
              </div>
            ) : null}
            <label>
            ...
            <div className="modal-footer" style={{ margin: '1rem -1.75rem -1.75rem' }}>
              ...
              <button
                className="btn btn-primary"
                disabled={savingIdentity || !identityDraft.name.trim()}
              >
        ...
        {editingSetting === 'establishmentType' ? (
          ...
          >
            ...
            <label>
              ...
              <select
                value={identityDraft.establishmentType}
                onChange={(event) =>
            ...
            <div className="modal-footer" style={{ margin: '1rem -1.75rem -1.75rem' }}>
              ...
              <button className="btn btn-primary" disabled={savingIdentity}>
                {savingIdentity ? 'Enregistrement...' : 'Enregistrer'}
              </button>
            </div>
          </form>
        ...
        {editingSetting === 'regulatoryCountry' ? (
          <form
            onSubmit={(event) => {
              event.preventDefault();
              void saveRegulatoryCountry(true);
            }}
            style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}
          >
            {regulatoryCountryError ? (
              <div className="alert-modern error">
                <AlertCircle size={16} /> {regulatoryCountryError}
              </div>
            ) : null}
            <label>
              Pays RH
              <select
                value={regulatoryCountryDraft}
                disabled={savingRegulatoryCountry}
                onChange={(event) => setRegulatoryCountryDraft(event.target.value)}
                autoFocus
              >
            ...
            <p className="muted" style={{ margin: 0 }}>
              Ce pays determine le solde simple utilise : conges payes en France, conges annuels en
              Finlande.
            </p>
            <div className="modal-footer" style={{ margin: '1rem -1.75rem -1.75rem' }}>
              <button
              ...
              <button
                className="btn btn-primary"
                disabled={
                  savingRegulatoryCountry ||
                  regulatoryCountryDraft === (regulatoryCountryCode ?? '')
                }
              >
                {savingRegulatoryCountry ? 'Enregistrement...' : 'Enregistrer'}
              </button>
            </div>
          </form>
        ) : null}
        ...
        {editingSetting === 'secondarySites' ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div className="settings-list">
              {secondarySites.map((site) => (
                <div key={site.id} className="planning-settings-item">
                  <div>
                    ...
                    <span>
                      {site.address || site.phone || site.responsibleName
                        ? [site.address, site.phone, site.responsibleName]
                            .filter(Boolean)
                            .join(' · ')
                  ...
                  <button
                    ...
                    onClick={() => openSiteForm(site, 'Modifier le site secondaire')}
                  >
                    Modifier
                  </button>
                </div>
              ...
              {!secondarySites.length ? (
                <p className="muted" style={{ margin: 0 }}>
                  Aucun site secondaire configuré.
                </p>
              ) : null}
            </div>
            <div className="modal-footer" style={{ margin: '1rem -1.75rem -1.75rem' }}>
              ...
              <button
                type="button"
                className="btn btn-primary"
                onClick={() => openSiteForm(null, 'Ajouter un site secondaire')}
              >
                Ajouter un site
              </button>
            </div>
        ...
        {editingSetting === 'siteForm' ? (
          ...
          >
            ...
            <section className="site-address-editor" aria-label="Adresse du site">
              <div className="site-address-editor-head">
                <div>
                  <span className="site-address-kicker">Localisation</span>
                  <h3>Adresse du site</h3>
                  <p>
                    Choisissez une suggestion pour enregistrer automatiquement l’adresse complète.
                  </p>
                </div>
                <div className="site-country-switch" role="group" aria-label="Pays de recherche">
                  <button
                    type="button"
                    className={siteCountry === 'FR' ? 'active' : ''}
                    onClick={() => setSiteCountry('FR')}
                  >
                    🇫🇷 France
                  </button>
                  <button
                    type="button"
                    className={siteCountry === 'FI' ? 'active' : ''}
                    onClick={() => setSiteCountry('FI')}
                  >
                    🇫🇮 Finlande
                  </button>
                </div>
              ...
              <AddressAutocomplete
                ...
                onChange={(address) => setSiteDraft((current) => ({ ...current, address }))}
              />
            </section>
            <div
              style={{
            ...
            >
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setSiteDraft((current) => ({ ...current, phone: event.target.value }))
                  }
                />
              </label>
              <label>
                Responsable
                <input
                  value={siteDraft.responsibleName}
                  onChange={(event) =>
                    setSiteDraft((current) => ({ ...current, responsibleName: event.target.value }))
                  }
                />
              </label>
              <label>
                ...
                <input
                  value={siteDraft.responsiblePhone}
                  onChange={(event) =>
              ...
              <label>
                ...
                <input
                  type="email"
                  value={siteDraft.responsibleEmail}
                  onChange={(event) =>
                    setSiteDraft((current) => ({
                      ...current,
            ...
            <div className="modal-footer" style={{ margin: '1rem -1.75rem -1.75rem' }}>
              ...
              <button className="btn btn-primary" disabled={savingSite || !siteDraft.name.trim()}>
...
function CategoryDetailModal({
  ...
  const linkedProducts = useMemo(
    () =>
      products.filter((product) => (product.categoryId ?? product.category?.id) === category?.id),
  ...
  async function remove() {
    ...
    const confirmed = window.confirm(
      `Supprimer la catégorie "${category.name}" ? ${linkedProducts.length} produit(s) seront déplacés dans "Sans catégorie".`,
    );
    if (!confirmed) return;
    setSubmitting(true);
    setError(undefined);
        ...
        <form onSubmit={save} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.65rem', flexWrap: 'wrap' }}>
            ...
            <button
              type="button"
              className="btn btn-outline-danger"
              disabled={submitting || isUncategorized}
              onClick={() => void remove()}
              title={
                isUncategorized
                  ? 'Catégorie système conservée pour les produits non classés'
                  : undefined
              }
            >
...
function ProductForm({
  ...
  const [supplierId, setSupplierId] = useState(
    initialProduct?.primarySupplierId ??
      initialProduct?.supplierId ??
      initialProduct?.primarySupplier?.id ??
  ...
  const [unitsPerPackage, setUnitsPerPackage] = useState(
    productFieldString(initialProduct?.unitsPerPackage),
  );
  const [unitWeightGrams, setUnitWeightGrams] = useState(
    productFieldString(initialProduct?.unitWeightGrams),
  );
  const [netWeightGrams, setNetWeightGrams] = useState(
    productFieldString(initialProduct?.netWeightGrams),
  );
  const [ingredients, setIngredients] = useState(initialProduct?.ingredients ?? '');
  const [allergensPresent, setAllergensPresent] = useState<string[]>(
    initialProduct?.allergensPresent ?? [],
  );
  const [possibleTraces, setPossibleTraces] = useState<string[]>(
    initialProduct?.possibleTraces ?? [],
  );
  const [dietaryTags, setDietaryTags] = useState<string[]>(initialProduct?.dietaryTags ?? []);
  const [nutrition, setNutrition] = useState<Record<ProductNutritionField, string>>(() =>
    initialNutritionState(initialProduct),
  );
  ...
  const [shelfLifeAfterOpening, setShelfLifeAfterOpening] = useState(
    initialProduct?.shelfLifeAfterOpening ?? '',
  );
  const [storageInstructions, setStorageInstructions] = useState(
    initialProduct?.storageInstructions ?? '',
  );
  const [preparationInstructions, setPreparationInstructions] = useState(
    initialProduct?.preparationInstructions ?? '',
  );
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string>();
          ...
          {activeFormTab === 'identity' ? (
            <div className="product-sheet-form-grid">
              <label>
                Nom du produit *
                <input
                  placeholder="ex: Beurre doux, œufs plein air..."
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  required
                  autoFocus
              ...
              <label className="product-sheet-wide">
                ...
                <textarea
                  rows={3}
                  placeholder="Notes produit, marque, informations utiles..."
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                />
              </label>
          ...
          {activeFormTab === 'supplier' ? (
            <div className="product-sheet-form-grid">
              ...
              <label>
                ...
                <input
                  placeholder="ex: KESPRO-12345, FARINE-T55..."
                  value={sku}
          ...
          {activeFormTab === 'stock' ? (
            <div className="product-sheet-form-grid">
              ...
              <div className="product-readonly-field product-sheet-wide">
                ...
                <strong>
                  {currentQuantity == null
                    ? 'Calculée depuis les mouvements'
                    : `${currentQuantity.toFixed(3).replace(/\.?0+$/, '')} ${selectedUnit?.symbol ?? ''}`}
                </strong>
                <small>
                  La quantité se modifie via réceptions, sorties, corrections ou inventaires.
                </small>
              </div>
            </div>
          ) : null}
          ...
          {activeFormTab === 'packaging' ? (
            <div className="product-sheet-form-grid">
              <label>
                Format fournisseur
                <input
                  placeholder="ex: Carton 12 x 1 L, caisse 6 pièces..."
                  value={packageLabel}
              ...
              <label>
                ...
                <input
                  ...
                  min="0"
                  step="0.001"
                  placeholder="En grammes ou millilitres selon l'unité"
              ...
              <label>
                ...
                <input
                  ...
                  placeholder={
                    computedNetWeight
                      ? `Calculé: ${computedNetWeight.toFixed(3).replace(/\.?0+$/, '')}`
                      : 'Optionnel'
                  }
              ...
              <div className="product-readonly-field">
                ...
                <strong>
                  {unitsPerPackage || '0'} x {unitWeightGrams || '0'} ={' '}
                  {(computedNetWeight ?? 0).toFixed(3).replace(/\.?0+$/, '')}
                </strong>
          ...
          {activeFormTab === 'allergens' ? (
            <div className="product-sheet-form-grid">
              <label className="product-sheet-wide">
                ...
                <textarea
                  rows={4}
                  placeholder="Liste d'ingrédients telle qu'indiquée par le fournisseur..."
                  value={ingredients}
                ...
                <div className="product-choice-grid">
                  {PRODUCT_ALLERGEN_OPTIONS.map((allergen) => (
                    <button
                      key={allergen}
                      type="button"
                      className={allergensPresent.includes(allergen) ? 'selected' : ''}
                      onClick={() => {
                ...
                <div className="product-choice-grid">
                  {PRODUCT_ALLERGEN_OPTIONS.map((allergen) => (
                    <button
                      key={allergen}
                      type="button"
                      className={possibleTraces.includes(allergen) ? 'selected' : ''}
                      onClick={() => {
                        setPossibleTraces((current) => toggleProductValue(current, allergen));
                        setAllergensPresent((current) =>
                ...
                <div className="product-choice-grid compact">
                  {PRODUCT_DIETARY_TAG_OPTIONS.map((tag) => (
                    <button
                      key={tag}
                      type="button"
                      className={dietaryTags.includes(tag) ? 'selected' : ''}
                      onClick={() => setDietaryTags((current) => toggleProductValue(current, tag))}
                    >
                      {tag}
                    </button>
                  ))}
              ...
              {PRODUCT_NUTRITION_FIELDS.map((field) => (
                <label key={field.key}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNutrition((current) => ({ ...current, [field.key]: e.target.value }))
                    }
                  />
                </label>
              ))}
            ...
            <div className="product-sheet-form-grid">
              <label>
                ...
                <select value={storageType} onChange={(e) => setStorageType(e.target.value)}>
                  <option value="">Non renseigné</option>
                  {PRODUCT_STORAGE_OPTIONS.map((option) => (
              ...
              <label>
                ...
                <input
                  placeholder="ex: 3 jours, 48 h, à consommer immédiatement..."
              ...
              <label className="product-sheet-wide">
                ...
                <textarea
                  rows={4}
                  placeholder="Température, conditions de stockage, précautions après ouverture..."
              ...
              <label className="product-sheet-wide">
                ...
                <textarea
                  rows={4}
                  placeholder="Conseils de préparation, décongélation, utilisation en production..."
...
function computeProductCompletion(product: Product) {
  ...
  const sectionInputs = [
    ...
    { label: 'Fournisseur', values: [supplierId, product.sku ?? product.reference] },
    {
      ...
      values: [
        ...
        product.unitWeightGrams ?? product.netWeightGrams,
      ],
    },
    {
      label: 'Allergènes',
      values: [
        product.ingredients,
        product.allergensPresent,
        product.possibleTraces,
        product.dietaryTags,
      ],
    },
  ...
  const sections = sectionInputs.map((section) => {
    ...
    return {
      ...
      done: completed === section.values.length,
    };
  });
...
function ProductTagBadges({
  ...
  return (
    <div className="product-tag-row">
      {values.map((item) => (
        <span key={item} className="badge badge-reception">
          {item}
        </span>
      ))}
    </div>
  ...
  const stockValue = productStocks.reduce(
    (sum, stock) =>
      ...
      numeric(
        stock.stockValue ??
          stock.value ??
          numeric(stock.currentQuantity ?? stock.quantity) * averagePrice,
      ),
    0,
  );
      ...
      {editing ? (
        ...
        <div className="product-detail">
          <div className="product-detail-hero">
            <div>
              ...
              <div className="product-detail-badges">
                ...
                <span className="badge badge-reception">
                  {product.unit?.name ?? 'Unité'} ({product.unit?.symbol ?? '—'})
                </span>
                {product.gtin ? (
                  <span className="badge badge-correction">GTIN {product.gtin}</span>
                ) : null}
              </div>
            </div>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
          ...
          <div className="product-detail-metrics">
            <Metric
              icon={<Boxes size={18} />}
              value={`${totalQuantity.toFixed(2).replace(/\.?0+$/, '')} ${product.unit?.symbol ?? ''}`}
            ...
            <Metric
              ...
              value={minimumStock ? parseFloat(minimumStock.toFixed(2)).toString() : '—'}
              label="Seuil minimum"
              tone="orange"
            />
          </div>
          ...
          <div className="product-detail-completion">
            ...
            <div className="progress-bar-bg">
              <div className="progress-bar-fill" style={{ width: `${completion.score}%` }} />
            </div>
            <div className="product-completion-sections">
              {completion.sections.map((section) => (
                <span key={section.label} className={section.done ? 'done' : ''}>
          ...
          <div className="product-detail-panel product-sheet-read-panel">
            {detailTab === 'identity' ? (
              <>
                ...
                <dl className="product-detail-list">
                  ...
                  <div>
                    <dt>GTIN / EAN</dt>
                    <dd>{productTextDisplay(product.gtin)}</dd>
                  </div>
                  <div>
                ...
                {product.description ? (
                  ...
                  <EmptyMini
                    ...
                    text="Ajoutez une note interne si nécessaire."
                  />
                )}
              </>
            ) : null}
              ...
              <>
                ...
                <dl className="product-detail-list">
                  ...
                  <div>
                    ...
                    <dd>{productTextDisplay(product.sku ?? product.reference)}</dd>
                  </div>
                </dl>
              </>
            ) : null}
              ...
              <div className="product-detail-grid">
                <div>
                  ...
                  <dl className="product-detail-list">
                    <div>
                      <dt>Catégorie</dt>
                      <dd>{product.category?.name ?? 'Non catégorisé'}</dd>
                    </div>
                    <div>
                      <dt>Unité de stock</dt>
                      <dd>
                        {product.unit?.name ?? '—'} ({product.unit?.symbol ?? '—'})
                      </dd>
                    </div>
                    <div>
                      <dt>Quantité actuelle</dt>
                      <dd>{productNumberDisplay(totalQuantity, product.unit?.symbol ?? '')}</dd>
                    </div>
                    <div>
                      <dt>Seuil minimum</dt>
                      <dd>
                        {minimumStock
                          ? productNumberDisplay(minimumStock, product.unit?.symbol ?? '')
                          : '—'}
                      </dd>
                    </div>
                    <div>
                ...
                <div className="product-sheet-wide">
                  ...
                  {productMovements.length ? (
                      ...
                      {productMovements.map((movement) => (
                        <div key={movement.id}>
                          ...
                          <strong>
                            ...
                            {productNumberDisplay(movement.quantity, product.unit?.symbol ?? '')}
                          </strong>
                        </div>
                      ))}
                    </div>
                    ...
                    <EmptyMini
                      title="Aucun mouvement"
                      text="Les réceptions et sorties apparaîtront ici."
                    />
                  )}
                </div>
              </div>
            ...
            {detailTab === 'packaging' ? (
              <>
                ...
                <dl className="product-detail-list">
                  <div>
                    <dt>Format fournisseur</dt>
                    <dd>{productTextDisplay(product.packageLabel)}</dd>
                  </div>
                  <div>
                    <dt>Quantité contenue par format</dt>
                    <dd>{productNumberDisplay(product.unitsPerPackage)}</dd>
                  </div>
                  <div>
                    <dt>Unité contenue</dt>
                    <dd>
                      {product.unit?.name ?? '—'} ({product.unit?.symbol ?? '—'})
                    </dd>
                  </div>
                  <div>
                    <dt>Poids / volume d'une unité</dt>
                    <dd>{productNumberDisplay(product.unitWeightGrams, 'g/ml')}</dd>
                  </div>
                  <div>
                    <dt>Poids net du format</dt>
                    <dd>
                      {netWeight == null
                        ? '—'
                        : `${productNumberDisplay(netWeight, 'g/ml')}${productHasValue(product.netWeightGrams) ? '' : ' calculé'}`}
                    </dd>
                  </div>
                </dl>
                <p className="product-detail-description">
                  Quantité reçue fournisseur x unités par format x contenu d'une unité = quantité
                  ajoutée au stock.
                </p>
              </>
            ) : null}
            ...
            {detailTab === 'allergens' ? (
              <div className="product-allergen-read">
                <div>
                  ...
                  <p className="product-detail-description">
                    {product.ingredients || 'Non renseigné'}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <ProductTagBadges
                    items={product.possibleTraces}
                    emptyText="Aucune trace renseignée"
                  />
                </div>
                <div>
            ...
            {detailTab === 'storage' ? (
              <>
                ...
                <dl className="product-detail-list">
                  <div>
                    <dt>Type de conservation</dt>
                    <dd>{productTextDisplay(product.storageType)}</dd>
                  </div>
                  <div>
                    <dt>Durée après ouverture</dt>
                    <dd>{productTextDisplay(product.shelfLifeAfterOpening)}</dd>
                  </div>
                </dl>
                <div className="product-storage-copy">
                  <div>
                    <span className="product-detail-section-title">
                      Instructions de conservation
                    </span>
...
function SupplierForm({
  ...
  const [orderEmail, setOrderEmail] = useState(
    initialSupplier?.purchasingProfile?.orderEmail ?? '',
  );
  ...
  const [deliveryWeekdays, setDeliveryWeekdays] = useState<number[]>(
    initialSupplier?.purchasingProfile?.deliveryWeekdays ?? [],
  );
  const [deliveryFee, setDeliveryFee] = useState(
    Number(initialSupplier?.purchasingProfile?.deliveryFee ?? 0),
  );
  const [minimumOrder, setMinimumOrder] = useState(
    Number(initialSupplier?.purchasingProfile?.minimumOrder ?? 0),
  );
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string>();
  ...
  useEffect(() => {
    ...
    setOrderEmail(initialSupplier?.purchasingProfile?.orderEmail ?? '');
    setDeliveryMode(initialSupplier?.purchasingProfile?.deliveryMode ?? 'ON_DEMAND');
    setDeliveryWeekdays(initialSupplier?.purchasingProfile?.deliveryWeekdays ?? []);
    setDeliveryFee(Number(initialSupplier?.purchasingProfile?.deliveryFee ?? 0));
    setMinimumOrder(Number(initialSupplier?.purchasingProfile?.minimumOrder ?? 0));
...
function AddressAutocomplete({
  ...
  const [suggestions, setSuggestions] = useState<
    Array<{ label: string; address: string; postalCode?: string; city?: string }>
  >([]);
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  ...
  const select = (suggestion: (typeof suggestions)[number]) => {
  ...
  return (
    <div className="address-autocomplete">
      <div className={`address-autocomplete-input ${selected ? 'selected' : ''}`}>
        <MapPin size={18} />
        <div>
          <input
            value={value}
            ...
            placeholder={
              country === 'FI' ? 'Ex. Mannerheimintie 10, Helsinki' : 'Ex. 12 rue de la Paix, Paris'
            }
          ...
          <small>
            {selected
              ...
              : loading
                ...
                : `Recherche ${country === 'FI' ? 'en Finlande' : 'en France'}`}
          </small>
        </div>
        {selected ? <CheckCircle2 size={18} /> : <Search size={17} />}
      </div>
      {open ? (
        <div className="address-autocomplete-menu" role="listbox">
          {suggestions.length ? (
            ...
            <div className="address-autocomplete-empty">
              Aucune adresse trouvée. Vous pouvez tout de même la saisir manuellement.
            </div>
          )}
        </div>
      ) : null}
    ...
    <form onSubmit={submitForm} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <label>
        ...
        <input
          value={name}
          onChange={(e) => setName(e.target.value)}
          placeholder="Restaurant principal, Cuisine centrale…"
          required
          autoFocus
        />
      </label>
...
function LocationForm({
  ...
  sites: Site[];
  onSubmit: (payload: { name: string; siteId: string; description?: string }) => Promise<void>;
  onClose: () => void;
}) {
  ...
  async function submitForm(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      await onSubmit({ name: name.trim(), siteId, description: description.trim() || undefined });
    } finally {
      setSubmitting(false);
    }
  }
  return (
    <form onSubmit={submitForm} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <label>
        Site *
        <select value={siteId} onChange={(e) => setSiteId(e.target.value)} required autoFocus>
      ...
      <div className="modal-footer" style={{ margin: '1rem -1.75rem -1.75rem' }}>
        ...
        <button className="btn btn-primary" disabled={!name.trim() || !siteId || submitting}>
          {submitting ? 'Configuration…' : 'Configurer le site'}
        </button>
      </div>
    </form>
  );
          ...
          <div className="product-sheet-form-grid">
            ...
            <label className="product-sheet-wide">
              ...
              <textarea
                ...
                placeholder="Motif et périmètre du comptage"
              />
            </label>
            <div className="product-sheet-note product-sheet-wide">
              À la validation, les écarts entre stock théorique et compté généreront des corrections
...
function StocksOcrReviewPanel({
  ...
  onCreateSupplierFromOcr,
  onReanalyzeAi,
  onClose,
}: {
  extraction: StocksOcrExtraction;
  products: Product[];
  categories: Category[];
  suppliers: Supplier[];
  units: Unit[];
  ...
  onCreateReception: (payload: StocksOcrExtraction['data']) => Promise<void>;
  onCreateProductFromLine: (line: StocksOcrLine, supplierId?: string | null) => Promise<Product>;
  onCreateSupplierFromOcr: (name: string) => Promise<Supplier>;
  ...
  useEffect(() => {
    ...
    api
      .viewStocksDocument(token, document.id)
      .then((url) => {
  ...
  const productsToCreate = activeLines.filter(
    (line) => !line.productId && line.createProduct,
  ).length;
  const blockingMissingProducts = activeLines.filter(
    (line) => !line.productId && !line.createProduct,
  ).length;
  ...
  const priceIssues = activeLines.filter(
    (line) => ocrLineStatus(line) === 'price_mismatch' || hasOcrPriceMismatch(line),
  ).length;
  ...
  const displayedLineEntries = draft.lines
    ...
    .filter(({ line }) => {
      ...
      if (lineFilter === 'missing')
        return !line.productId || ocrLineStatus(line) === 'missing_product';
      if (lineFilter === 'price')
        return (
          ocrLineStatus(line) === 'price_mismatch' ||
          hasOcrPriceMismatch(line) ||
          ocrLineStatus(line) === 'quantity_suspicious'
        );
  ...
  const supplierMatchStatus = draft.supplierId
    ? (draft.supplierMatchingStatus ?? draft.supplier?.matchingStatus ?? 'RECOGNIZED')
    : 'NOT_FOUND';
  const supplierOcrName = draft.supplier?.name || draft.supplierName || '';
  const aiWarnings = uniqueOcrMessages([
    ...(draft.warnings ?? []),
    ...(draft.aiAnalysis?.warnings ?? []),
  ]);
  const aiActions = uniqueOcrMessages([
    ...(draft.suggestedActions ?? []),
    ...(draft.aiAnalysis?.suggestedActions ?? []),
  ]);
  const totalsCheck = draft.aiAnalysis?.totalsCheck;
  const totalLinesAmount = activeLines.reduce(
  ...
  function removeLine(index: number) {
    setDraft((current) => ({
      ...current,
      lines: current.lines.map((line, i) => (i === index ? { ...line, ignored: true } : line)),
    }));
  }
  ...
  async function createAllMissingProducts() {
    ...
    for (const { line, index } of linesToCreate) {
      ...
      try {
        setCreatingProductLineId(lineKey);
        const product =
          createdByKey.get(dedupeKey) ?? (await onCreateProductFromLine(line, supplierId));
        ...
        failures.push(
          reason ? `${label || `ligne ${index + 1}`}: ${reason}` : label || `ligne ${index + 1}`,
        );
      }
    }
    setCreatingProductLineId(null);
  ...
  return (
    <div className={`stocks-ocr-review ${showPreview ? 'show-preview' : 'hide-preview'}`}>
          ...
          <div className="stocks-ocr-preview-header">
            ...
            <div style={{ display: 'flex', gap: '0.4rem' }}>
              ...
              <button
                ...
                onClick={() => setZoomScale((z) => Math.min(3, z + 0.2))}
              >
                <ZoomIn size={12} />
              </button>
              <button
      ...
      <div className="stocks-ocr-editor">
        {error ? (
          <div className="alert-modern error" style={{ marginBottom: '0.5rem' }}>
            <AlertCircle size={16} /> {error}
          </div>
        ) : null}
        ...
        <div className="ocr-dossier">
          <div className="ocr-dossier-header">
            <div>
              ...
              <p>
                {draft.aiAnalysis?.status === 'applied'
                  ? 'Analyse Mistral IA appliquée au document.'
                  : draft.aiAnalysis?.status === 'failed'
                    ? 'Analyse IA indisponible, parsing ToqueHub utilisé.'
                    : 'Analyse OCR prête à valider.'}
              </p>
            </div>
          ...
          <div className="ocr-dossier-metrics">
            <div>
              ...
              <strong>
                {formatOcrPercent(draft.documentConfidence ?? draft.aiAnalysis?.confidence)}
              </strong>
            </div>
            <div>
              <span>Lignes prêtes</span>
              <strong>
                {readyLines}/{activeLines.length}
              </strong>
            </div>
            <div>
              <span>À vérifier</span>
              <strong>{activeLines.length - readyLines}</strong>
            </div>
            <div>
              <span>Total lignes</span>
              <strong>{totalLinesAmount.toFixed(2)} €</strong>
            </div>
            <div>
              <span>Écart total</span>
              <strong className={numeric(totalsCheck?.delta) > 0.05 ? 'danger-text' : ''}>
                {totalsCheck?.delta != null ? `${numeric(totalsCheck.delta).toFixed(2)} €` : '—'}
              </strong>
            </div>
          </div>
          ...
          {aiWarnings.length || aiActions.length ? (
            <div className="ocr-dossier-alerts">
              {aiWarnings.slice(0, 4).map((warning, index) => (
                <span key={`w-${index}`}>
          ...
          <div className="ocr-line-filter-bar">
            {[
              ...
              <button
                key={key}
                type="button"
                className={`ocr-line-filter ${lineFilter === key ? 'active' : ''}`}
                onClick={() => setLineFilter(key as typeof lineFilter)}
              >
                {label}
              </button>
            ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setDraft({
                    ...
                    supplierName: (supplier?.name ?? supplierOcrName) || null,
                    supplierMatchingStatus: e.target.value ? 'RECOGNIZED' : 'NOT_FOUND',
                    supplierMatchingScore: e.target.value ? 1 : 0,
                  });
                }}
              >
                <option value="">Non renseigné</option>
          ...
          >
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
              <span className={`badge ${ocrMatchClass(supplierMatchStatus)}`}>
                {ocrMatchLabel(supplierMatchStatus)}
              </span>
              {draft.supplierId
                ? `Fournisseur lié : ${suppliers.find((supplier) => supplier.id === draft.supplierId)?.name ?? draft.supplierName ?? 'sélectionné'}`
                : `Fournisseur OCR : ${supplierOcrName || 'non détecté'}`}
            </span>
            {!draft.supplierId && supplierOcrName ? (
              <button
                ...
                onClick={() => void createSupplierFromOcr()}
              >
                  ...
                  <button
                    ...
                    onClick={() =>
                      setDraft({
                        ...
                        supplierMatchingStatus: 'RECOGNIZED',
                        supplierMatchingScore: candidate.score,
                      })
                    }
                  >
                    {candidate.name}
          ...
          <div className="form-row">
            ...
            <label style={{ fontWeight: 700, fontSize: '0.82rem', color: 'var(--text-main)' }}>
              ...
              <input
                ...
                onChange={(e) => setDraft({ ...draft, deliveryNoteNumber: e.target.value })}
                placeholder="Ex: BL-4919"
              />
            </label>
            <label style={{ fontWeight: 700, fontSize: '0.82rem', color: 'var(--text-main)' }}>
              N° Commande
              <input
                ...
                onChange={(e) => setDraft({ ...draft, purchaseOrderNumber: e.target.value })}
                placeholder="Ex: BC-8201"
              />
            </label>
          </div>
          ...
          <div className="form-row">
            <label style={{ fontWeight: 700, fontSize: '0.82rem', color: 'var(--text-main)' }}>
              ...
              <input
                type="date"
                value={dateInputValue(draft.documentDate)}
            ...
            <label style={{ fontWeight: 700, fontSize: '0.82rem', color: 'var(--text-main)' }}>
              ...
              <input
                type="date"
                value={dateInputValue(draft.deliveryDate)}
                onChange={(e) => setDraft({ ...draft, deliveryDate: e.target.value || null })}
              />
            </label>
            <label style={{ fontWeight: 700, fontSize: '0.82rem', color: 'var(--text-main)' }}>
              Total TTC (€)
              <input
                type="number"
                step="0.01"
                value={draft.totalIncludingTax ?? ''}
                onChange={(e) =>
                  setDraft({
                    ...draft,
                    totalIncludingTax: e.target.value ? Number(e.target.value) : null,
                  })
                }
              />
            </label>
          ...
          <div className="form-row">
            <label style={{ fontWeight: 700, fontSize: '0.82rem', color: 'var(--text-main)' }}>
              ...
              <select
                value={draft.siteId || ''}
                onChange={(e) =>
                  setDraft({ ...draft, siteId: e.target.value || null, locationId: null })
                }
              >
                ...
                <tr>
                  <td colSpan={8}>
                    <div className="empty-state" style={{ padding: '1.5rem' }}>
                      ...
                      <span className="empty-state-desc">
                        Changez de filtre pour afficher les autres lignes de l’analyse IA.
                      </span>
                    </div>
                  </td>
                </tr>
        ...
        >
          <button className="btn btn-secondary" onClick={onClose} style={{ borderRadius: '10px' }}>
            Annuler
          </button>
          <button
            className="btn btn-secondary"
          ...
          <button
            className="btn btn-primary"
            disabled={submitting !== null || blockingMissingProducts > 0 || invalidQuantities > 0}
      ...
      <Modal
        ...
        title={`Aperçu : ${document?.originalName || 'Document'}`}
        size="lg"
      >
        <div
          style={{
        ...
        >
            ...
            {previewUrl && document?.mimeType === 'application/pdf' ? (
              <iframe
                ...
                style={{
                  ...
                  borderRadius: '6px',
                  background: 'white',
                  pointerEvents: zoomScale > 1.0 ? 'none' : 'auto',
                }}
              />
            ) : null}
            {!previewUrl ? (
          ...
          >
            ...
            <span
              style={{ height: '12px', width: '1px', background: 'rgba(255,255,255,0.2)' }}
            ></span>
            <button
              type="button"
              className="btn btn-secondary btn-sm"
...
function ProductAssignmentModal({
  ...
  const [matchFilter, setMatchFilter] = useState<'all' | 'best' | 'supplier' | 'uncategorized'>(
    'all',
  );
    ...
    return products
      ...
      .map((product) => {
        ...
        const searchHaystack = normalizeSearchText(
          `${product.name} ${product.sku ?? product.reference ?? ''} ${supplierName} ${categoryName} ${product.unit?.symbol ?? ''}`,
        );
        const textMatch =
          !search || search.split(' ').every((part) => searchHaystack.includes(part));
      ...
      .filter(({ product, score, textMatch }) => {
        ...
        if (categoryFilter && (product.categoryId ?? product.category?.id) !== categoryFilter)
          return false;
        if (unitFilter && product.unitId !== unitFilter) return false;
        if (matchFilter === 'best' && score < 0.58) return false;
        if (
          matchFilter === 'supplier' &&
          (!supplierId || productSupplierId(product) !== supplierId)
        )
          return false;
        if (matchFilter === 'uncategorized' && (product.categoryId || product.category?.id))
          return false;
      ...
      <div className="ocr-product-selector">
        <div className="ocr-product-selector-context">
          ...
          <div className="ocr-product-selector-meta">
            ...
            {line.unit || line.matchedUnitSymbol ? (
              <span>{line.unit || line.matchedUnitSymbol}</span>
            ) : null}
          </div>
        </div>
        ...
        <div className="ocr-product-selector-toolbar">
          <div className="search-input-wrapper ocr-product-selector-search">
            ...
            <input
              ...
              placeholder="Rechercher par nom, référence, fournisseur, catégorie..."
              autoFocus
            />
          </div>
          <select
        ...
        <div className="ocr-product-selector-results">
          {rankedProducts.map(({ product, score }) => {
            const supplierName =
              product.primarySupplier?.name ?? product.supplier?.name ?? 'Sans fournisseur';
            ...
            return (
              <button
                key={product.id}
                type="button"
                className="ocr-product-result"
                onClick={() => onSelect(product, score)}
              >
                <div className="ocr-product-result-main">
                  ...
                  <span className="ocr-product-result-sub">
                    {product.sku || product.reference
                      ? `Réf. ${product.sku ?? product.reference} · `
                      : ''}
                ...
                <div className="ocr-product-result-badges">
                  ...
                  <span
                    className={`badge ${score >= 0.84 ? 'badge-reception' : score >= 0.58 ? 'badge-correction' : 'badge-production'}`}
                  >
                    {scoreLabel}
                  </span>
                </div>
          ...
          {!rankedProducts.length ? (
            <div className="ocr-product-selector-empty">
              ...
              <span>
                Modifiez la recherche ou les filtres, puis créez le produit depuis la ligne OCR si
                aucun produit existant ne correspond.
              </span>
            </div>
          ) : null}
        </div>
...
function normalizeOcrReceptionData(
  data: StocksOcrExtraction['data'],
  suppliers: Supplier[] = [],
): StocksOcrExtraction['data'] {
    ...
    lines: (data.lines || []).map((line, index) => {
      ...
      return {
        ...
        packageDescription:
          line.packageDescription ??
          line.descriptionOriginal ??
          ocrPackageDescriptionFromName(line.nameOriginal ?? line.label ?? line.ocrLabel ?? '') ??
          null,
      };
    }),
  };
...
function normalizeKnownOcrSupplier(data: StocksOcrExtraction['data'], suppliers: Supplier[]) {
  ...
  const kespro = suppliers.find(
    (supplier) =>
      normalizeLookup(supplier.name) === 'kespro' ||
      normalizeLookup(supplier.name).includes('kespro'),
  );
  const supplier = {
    ...(data.supplier ?? {}),
    name: 'Kespro',
  ...
  return {
    ...
    supplierCandidates: kespro
      ? [{ id: kespro.id, name: kespro.name, score: 1 }]
...
function isKesproOcrReception(data: StocksOcrExtraction['data']) {
  ...
  const supplierName =
    data.supplierName ?? data.supplier?.supplierName ?? data.supplier?.name ?? '';
  const lineUnits = (data.lines ?? [])
    .map((line) => line.unit)
    .filter(Boolean)
    .join(' ');
  const documentText = [
...
function isOcrStatusReady(status: StocksOcrStatus) {
  const state = normalizedOcrState(status);
  return (
    Boolean(status.extraction) ||
    state.includes('vérifier') ||
    state.includes('pret') ||
    state.includes('prêt')
  );
}
...
function isOcrLineReady(line: StocksOcrLine) {
  ...
  return (
    !line.ignored &&
    Boolean(line.productId || line.createProduct) &&
    numeric(line.quantity) > 0 &&
    !hasOcrPriceMismatch(line) &&
    !blockingStatuses.includes(status) &&
    !(status === 'missing_product' && !line.createProduct)
  );
}
...
function ocrLinePackageDescription(line: StocksOcrLine) {
  return (
    line.packageDescription ||
    line.descriptionOriginal ||
    ocrPackageDescriptionFromName(line.nameOriginal || line.ocrLabel || line.label || '')
  );
}
...
function translateOcrMessage(message?: string | null) {
  ...
  const replacements: Array<[RegExp, string]> = [
    [
      /Document date and delivery date are missing\.?/gi,
      'La date du document et la date de livraison sont manquantes.',
    ],
    [
      /No totals \(excluding tax, tax, including tax\) found on the delivery note\.?/gi,
      'Aucun total HT, TVA ou TTC n’a été trouvé sur le bon de livraison.',
    ],
    [
      /Unit price and line total are missing, preventing price verification\.?/gi,
      'Le prix unitaire et le total de ligne sont manquants, la vérification du prix est impossible.',
    ],
    [
      /Verify the delivery date and document date with the supplier\.?/gi,
      'Vérifier la date de livraison et la date du document avec le fournisseur.',
    ],
    [
      /Check if the product '([^']+)' exists in your product catalog and update the productId if necessary\.?/gi,
      'Vérifier si le produit « $1 » existe dans le catalogue et l’associer si nécessaire.',
    ],
    [
      /Confirm the unit '([^']+)' \(assumed to be '([^']+)' or '([^']+)'\) matches your internal unit of measure\.?/gi,
      'Confirmer que l’unité « $1 » correspond à votre unité interne.',
    ],
    [
      /Unit '([^']+)' mapped to '([^']+)'\s*\(([^)]+)\)\. Verify if this matches your internal unit of measure\.?/gi,
      'L’unité « $1 » a été associée à « $2 » ($3). Vérifier que cela correspond à votre unité interne.',
    ],
    [
      /Verify if this matches your internal unit of measure\.?/gi,
      'Vérifier que cela correspond à votre unité interne.',
    ],
...
function CustomizeDashboardForm({
  ...
  const themeOptions = [
    ...
    },
    {
      id: 'dark',
      name: 'Stealth Slate',
      bg: 'linear-gradient(135deg, #0b0f19 0%, #070a13 100%)',
      accent: '#64748b',
    },
  ] as const;
        ...
        <div className="customizer-toggle-list">
          <div className="customizer-toggle-row">
            <div className="customizer-toggle-info">
              ...
              <span className="customizer-toggle-desc">
                Affiche le résumé global des stocks et produits.
              </span>
            </div>
            <label className="switch-control">
              <input
          ...
          <div className="customizer-toggle-row">
            <div className="customizer-toggle-info">
              ...
              <span className="customizer-toggle-desc">
                Affiche la check-list des étapes initiales.
              </span>
            </div>
            <label className="switch-control">
              <input
          ...
          <div className="customizer-toggle-row">
            <div className="customizer-toggle-info">
              ...
              <span className="customizer-toggle-desc">
                Raccourci vers les modules actifs et le Toque Store.
              </span>
            </div>
            <label className="switch-control">
              <input
              ...
              return (
                <div className="customizer-toggle-row" key={widget.id}>
                  <div className="customizer-toggle-info">
                    ...
                    <span className="customizer-toggle-desc">
                      {widget.zone.toUpperCase()} · {dashboardWidgetModule(widget)}
                      {widget.comingSoon ? ' · À venir' : ''}
                    </span>
                  </div>
                  <button
                    className="btn btn-secondary btn-sm"
                  ...
                  >
                    <Pin size={14} /> {pinned ? 'Désépingler' : 'Épingler'}
                  </button>
                  <label className="switch-control">
                    <input
                      type="checkbox"
