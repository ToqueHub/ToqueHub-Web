...
export class ProductionService {
  ...
  async createOrder(organizationId: string, actor: Actor, dto: any) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const created = await tx.productionOrder.create({ data: { organizationId, number, name: dto.name || recipe.name, technicalSheetId: recipe.id, productionDate: new Date(dto.productionDate), plannedTime: dto.plannedTime, plannedPortions: dto.plannedPortions, serviceId: dto.serviceId || null, responsibleEmployeeId: dto.responsibleEmployeeId || null, priority: dto.priority ?? ProductionPriority.NORMAL, comments: dto.comments, createdById: actor.id } });
      await this.recalculateTx(tx, organizationId, created.id);
      await this.history(tx, organizationId, created.id, actor.id, ProductionHistoryAction.CREATED, `Création manuelle ${number}`, { technicalSheetId: recipe.id });
  ...
  async updateOrder(organizationId: string, actor: Actor, id: string, dto: any) {
    ...
    if (dto.serviceId) await this.ensureService(organizationId, dto.serviceId); if (dto.responsibleEmployeeId) await this.ensureEmployee(organizationId, dto.responsibleEmployeeId);
    return this.prisma.$transaction(async (tx) => { await tx.productionOrder.update({ where: { id, organizationId }, data: { ...dto, productionDate: dto.productionDate ? new Date(dto.productionDate) : undefined, updatedById: actor.id } }); if (dto.plannedPortions) await this.recalculateTx(tx, organizationId, id); await this.refreshAlertsTx(tx, organizationId, id); await this.history(tx, organizationId, id, actor.id, ProductionHistoryAction.UPDATED, 'Modification ordre'); return this.serializeOrder(await tx.productionOrder.findUnique({ where: { id }, include: this.orderInclude(true) })); });
  }

  async recalculateOrder(organizationId: string, actor: Actor, id: string) { await this.assertInstalled(organizationId); this.assertWrite(actor); return this.prisma.$transaction(async (tx) => { await this.recalculateTx(tx, organizationId, id); await this.history(tx, organizationId, id, actor.id, ProductionHistoryAction.UPDATED, 'Recalcul besoins matières'); return this.serializeOrder(await tx.productionOrder.findUnique({ where: { id }, include: this.orderInclude(true) })); }); }
  ...
  async changeStatus(organizationId: string, actor: Actor, id: string, dto: any) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (critical.length && dto.confirmCriticalOverride) { if (!dto.overrideReason) throw new BadRequestException('Motif requis'); for (const a of critical) await tx.productionAlertOverride.create({ data: { organizationId, alertId: a.id, confirmedById: actor.id, reason: dto.overrideReason, snapshot: a } }); await this.history(tx, organizationId, id, actor.id, ProductionHistoryAction.ALERT_OVERRIDE_CONFIRMED, 'Contournement alertes critiques confirmé', { reason: dto.overrideReason }); }
      ...
      await this.history(tx, organizationId, id, actor.id, dto.status === 'CANCELLED' ? ProductionHistoryAction.CANCELLED : dto.status === 'VALIDATED' ? ProductionHistoryAction.VALIDATED : ProductionHistoryAction.STATUS_CHANGED, `Statut ${dto.status}`); await this.refreshAlertsTx(tx, organizationId, id); return this.serializeOrder(updated);
    });
  }
  ...
  private history(tx: any, organizationId: string, orderId: string | null, actorUserId: string | null, action: any, summary: string, details?: any) { return tx.productionHistory.create({ data: { organizationId, orderId, actorUserId, action, summary, details } }); }
