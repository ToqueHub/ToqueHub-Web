...
export class ProductionService {
  ...
  async confirmDestocking(organizationId: string, actor: Actor, proposalId: string, dto: any) {
    await this.assertInstalled(organizationId); this.assertWrite(actor);
    return this.prisma.$transaction(async (tx) => {
      const proposal = await tx.productionDestockingProposal.findFirst({
      ...
      if (proposal.status === ProductionDestockingStatus.CONFIRMED)
        throw new BadRequestException('Déstockage déjà confirmé');
      if (!proposal.order.siteId)
        throw new BadRequestException('Le site de production doit être renseigné avant le déstockage.');
      const movementIds = [];
      for (const line of proposal.lines) {
        const qty = new Prisma.Decimal(line.confirmedQuantity ?? line.proposedQuantity);
        const movement = await tx.stockMovement.create({
          data: {
            ...
            inputQuantity: qty,
            unitId: line.unitId,
            unitSymbolSnapshot: line.unitSymbolSnapshot,
            reason: `Production ${proposal.order.number}`,
            sourceSiteId: proposal.order.siteId,
            sourceEntityType: 'ProductionOrder',
            sourceEntityId: proposal.orderId,
        ...
        await this.applyStock(
          ...
          organizationId,
          line.productId,
          proposal.order.siteId,
          qty.neg(),
        );
        await tx.productionDestockingMovement.create({
          data: { organizationId, proposalId, stockMovementId: movement.id },
        });
        movementIds.push(movement.id);
      }
      const updated = await tx.productionDestockingProposal.update({
        where: { id: proposalId },
        data: {
          status: ProductionDestockingStatus.CONFIRMED,
      ...
      await this.recordHistory(
        ...
        proposal.orderId,
        actor.id,
        ProductionHistoryAction.DESTOCKING_CONFIRMED,
        'Déstockage confirmé manuellement via Stocks',
        { movementIds, siteId: proposal.order.siteId },
      );
      return updated;
    });
  }
  ...
  private async recalculateTx(tx: any, organizationId: string, orderId: string) {
    ...
    if (!order.siteId)
      throw new BadRequestException('Le site de production doit être renseigné.');
    await tx.productionMaterialRequirement.deleteMany({ where: { orderId } });
    const assignments = await tx.productSite.findMany({
      where: {
        organizationId,
        siteId: order.siteId,
        productId: { in: order.technicalSheet.ingredients.map((item) => item.productId) },
        isActive: true,
      },
    });
    ...
    for (const ing of order.technicalSheet.ingredients) {
      ...
      const available = ing.product.stocks
        .filter((stock) => stock.siteId === order.siteId)
        .reduce((sum, stock) => sum.add(stock.quantity), new Prisma.Decimal(0));
      const minimumStock =
        assignmentByProduct.get(ing.productId)?.minimumStock ?? ing.product.minimumStock;
      const status = ing.product.isArchived
        ...
        : available.isZero()
          ...
          : available.lt(required)
            ...
            : available.sub(required).lte(minimumStock)
      ...
      await tx.productionMaterialRequirement.create({
        data: {
          ...
          details: { sourceTechnicalSheetId: order.technicalSheetId, siteId: order.siteId },
        },
      });
    }
    await tx.productionOrder.update({ where: { id: orderId }, data: { estimatedCost } });
    await this.refreshAlertsTx(tx, organizationId, orderId);
  }
  ...
  private async applyStock(tx: any, organizationId: string, productId: string, siteId: string, delta: any) {
    ...
    const stock = await tx.stock.findFirst({
      where: { organizationId, productId, siteId, lotId: null, locationId: null },
      orderBy: { createdAt: 'asc' },
    });
    if (stock)
      await tx.stock.update({
        where: { id: stock.id },
        data: { quantity: stock.quantity.add(delta) },
      });
    else
      await tx.stock.create({
        data: { organizationId, productId, siteId, quantity: delta },
      });
  }
  ...
  private async resolveOperationalSite(organizationId: string, requestedSiteId?: string) {
    ...
    if (requestedSiteId) {
      ...
      if (!selected) throw new BadRequestException('Le site de production est invalide ou archivé.');
      return selected;
    }
    if (sites.length === 1) return sites[0];
    if (!sites.length) throw new BadRequestException('Aucun site actif n’est configuré.');
    throw new BadRequestException('Sélectionnez le site où la production sera réalisée.');
  }
