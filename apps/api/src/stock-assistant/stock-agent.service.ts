...
export type StockAgentChoiceType = 'product_select' | 'location_select' | 'supplier_select' | 'proposal_review' | 'confirm_duplicate' | 'clarification';
...
export class StockAgentService {
  ...
  private inferToolCall(content: string, state: AgentState): AgentToolCall | null {
    ...
    if (/\bstock\b/.test(normalized) && /\b(de|du|d|pour)\b/.test(normalized) && /[?]\s*$/.test(content.trim())) return { tool: 'clarification', message: 'Quel produit souhaitez-vous vérifier ?\n\nExemple : “Y a-t-il du stock de café ?”', args: {} };
  ...
  private genericActionClarification(normalized: string): AgentToolCall | null {
    ...
    if (actionIntent === 'transfer') return { tool: 'clarification', message: 'Transfert de stock\n\nIndiquez le produit, la quantité, l’emplacement source et la destination.\n\nExemple : “Transfère 5 kg de café de la réserve vers la cuisine”.', args: { pendingIntent: 'transfer' } };
    if (actionIntent === 'waste') return { tool: 'clarification', message: 'Retrait / perte\n\nIndiquez le produit, la quantité et l’emplacement.\n\nExemple : “Retire 2 kg de tomates en réserve”.', args: { pendingIntent: 'waste' } };
    if (actionIntent === 'receipt') return { tool: 'clarification', message: 'Ajout de stock\n\nIndiquez le produit, la quantité et l’emplacement.\n\nExemple : “Ajoute 10 kg de café dans la réserve”.', args: { pendingIntent: 'receipt' } };
    return null;
  }
  ...
  private async decideToolCallWithMistral(organizationId: string, conversationId: string, content: string, state: AgentState): Promise<AgentToolCall> {
    ...
    try {
      ...
      return { tool: 'clarification', message: parsed?.message || 'Je peux vous aider sur les produits, stocks, emplacements, fournisseurs, propositions et factures. Que voulez-vous faire ?', args: {}, decision: 'mistral', confidence: 0.4 };
    } catch {
