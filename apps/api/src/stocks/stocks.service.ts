...
export class StocksService {
  ...
  archiveCategory(organizationId: string, actor: Actor, id: string) { return this.archive('category', organizationId, actor, id, AuditAction.CATEGORY_ARCHIVED, 'Category'); }
  ...
  async createProduct(organizationId: string, actor: Actor, dto: UpsertProductDto) { this.assertWrite(actor); await this.ensureUnit(organizationId, dto.unitId); if (dto.categoryId) await this.ensureCategory(organizationId, dto.categoryId); if (dto.primarySupplierId) await this.ensureSupplier(organizationId, dto.primarySupplierId); const item = await this.prisma.product.create({ data: { ...dto, organizationId }, include: { category: true, unit: true, primarySupplier: true } }); await this.log(organizationId, actor.id, AuditAction.PRODUCT_CREATED, 'Product', item.id, item.name); return item; }
  async updateProduct(organizationId: string, actor: Actor, id: string, dto: UpsertProductDto) { this.assertWrite(actor); const item = await this.prisma.product.update({ where: { id, organizationId }, data: dto, include: { category: true, unit: true, primarySupplier: true } }); await this.log(organizationId, actor.id, AuditAction.PRODUCT_UPDATED, 'Product', item.id, item.name); return item; }
