...
export class TechnicalSheetsController {
  ...
  @Patch('categories/:id') updateCategory(@CurrentUser() user: AuthenticatedUser, @Param('id') id: string, @Body() dto: UpsertRecipeCategoryDto) { return this.service.updateCategory(this.org(user), id, dto); }
  @Post('categories/:id/archive') archiveCategory(@CurrentUser() user: AuthenticatedUser, @Param('id') id: string) { return this.service.archiveCategory(this.org(user), id); }

  @Get('allergens') listAllergens(@CurrentUser() user: AuthenticatedUser, @Query() q: TechnicalSheetListQueryDto) { return this.service.listAllergens(this.org(user), q); }
  @Post('allergens') createAllergen(@CurrentUser() user: AuthenticatedUser, @Body() dto: UpsertAllergenDto) { return this.service.createAllergen(this.org(user), dto); }
  @Patch('allergens/:id') updateAllergen(@CurrentUser() user: AuthenticatedUser, @Param('id') id: string, @Body() dto: UpsertAllergenDto) { return this.service.updateAllergen(this.org(user), id, dto); }
  @Post('allergens/:id/archive') archiveAllergen(@CurrentUser() user: AuthenticatedUser, @Param('id') id: string) { return this.service.archiveAllergen(this.org(user), id); }
  ...
  @Get('recipes/:id/history') history(@CurrentUser() user: AuthenticatedUser, @Param('id') id: string, @Query() q: TechnicalSheetListQueryDto) { return this.service.historyList(this.org(user), id, q); }
