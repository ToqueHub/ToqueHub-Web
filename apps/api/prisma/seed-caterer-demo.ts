...
async function main() {
  ...
  const primarySite = organization.primarySiteId
  ...
  await prisma.organization.update({
    ...
    data: { menusInstalledAt: organization.menusInstalledAt ?? new Date() },
  });
  ...
  console.log(
    `Démo Traiteur prête pour « ${organization.name} » sur le site « ${site.name} » : ${counts[0]} clients, ${counts[1]} événements, ${counts[2]} prestations et ${counts[3]} recettes.`,
  );
}
